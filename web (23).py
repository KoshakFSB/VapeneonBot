...
async def login(body: LoginIn, response: Response):
    ...
    try:
        c = db()
        c.execute(
            """INSERT INTO admin_sessions (token, username, can_review_admin_complaints, expires_at)
               VALUES (%s,%s,%s,%s)
               ON CONFLICT (token) DO UPDATE SET
...
async def get_users(request: Request, q: str = ""):
    ...
    for u in data:
        ...
        try:
            ...
            u["muted"]     = ((one(conn,"SELECT COUNT(*) as c FROM mutes WHERE user_id=%s AND is_active=1",(uid,)) or {}).get("c", 0)) > 0
            u["banned"]    = ((one(conn,"SELECT COUNT(*) as c FROM bans WHERE user_id=%s AND is_active=1",(uid,)) or {}).get("c", 0)) > 0
