...
# Токены авторизации пользователей: token -> {username, confirmed, tg_id}
USER_TOKENS: dict = {}
USER_SESSIONS: dict = {}  # session_token -> {username, tg_id}
...
async def create_user_token(body: dict):
    """Создать токен для авторизации пользователя через бота"""
...
async def poll_user_token(token: str, response: Response):
    """Сайт спрашивает: подтверждён ли токен ботом?"""
...
async def confirm_user_token(body: dict):
    """Бот подтверждает токен после /start webauth_XXX"""
    token = body.get("token")
    username = body.get("username")
    tg_id = body.get("tg_id")
    secret = body.get("secret")
    # Простая защита — бот передаёт BOT_TOKEN как secret
    if secret != BOT_TOKEN:
...
async def user_auth_check(username: str):
    """Проверить авторизовался ли пользователь через бота"""
    if not username:
        raise HTTPException(400, "username required")
    conn = db()
    u = one(conn, "SELECT * FROM bot_users WHERE lower(ltrim(username,'@'))=?",
