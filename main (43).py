...
async def cmd_addjbadm(message: Message, command: CommandObject):
    ...
    if not target_username:
        await message.answer(
            "❌ Не удалось определить username пользователя.\n"
            "Пользователь должен иметь username и хотя бы раз писать в чат.",
            parse_mode="HTML"
        )
        return

    site_username = target_username.lstrip("@").lower()
    ...
    try:
        ...
        async with _httpx.AsyncClient(timeout=8) as client:
            ...
            if r.status_code == 404:
                await message.answer(
                    f"❌ @{site_username} не найден среди администраторов сайта.\n"
                    "Сначала назначьте его администратором через бота.",
                    parse_mode="HTML"
                )
                return
    ...
    await message.answer(
        f"✅ @{site_username} теперь может рассматривать жалобы на администраторов на сайте.",
        parse_mode="HTML"
    )
