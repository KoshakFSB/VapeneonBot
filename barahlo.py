...
async def sd_check_payment(callback: CallbackQuery):
    ...
    if payment_confirmed:
        ...
        # Уведомляем обоих участников в личных сообщениях
        ...
        # Уведомляем в групповой чат сделки
        if deal.get("group_chat_id"):
            try:
                ...
                logger.error(f"Ошибка уведомления в групповой чат сделки {deal_id}: {e}")
...
async def sd_open_dispute(callback: CallbackQuery):
    ...
    # Если оплата не поступила — продавец не может открыть спор (возвращать нечего)
    if not deal.get("payment_confirmed") and user_id == deal["seller_id"]:
        await callback.answer(
            "❌ Покупатель ещё не оплатил сделку — открыть спор невозможно.",
            show_alert=True
        )
        return
