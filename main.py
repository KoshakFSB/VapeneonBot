...
async def resolve_user_only(message: Message, command_args: str = None) -> Optional[int]:
    ...
    if command_args and command_args.strip():
        ...
        return await get_user_id_from_message(user_identifier)
...
async def cmd_warn(message: Message, command: CommandObject):
    ...
    if not result:
        await message.answer(
            ...
            "📌 <b>Используйте:</b>\n"
            "• <code>/warn [причина]</code> - ответом на сообщение\n"
...
async def cmd_warns(message: Message, command: CommandObject):
    ...
    if not user_id:
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Используйте:</b>\n"
            "• <code>/warns</code> - ответом на сообщение\n"
...
async def cmd_mute(message: Message, command: CommandObject):
    ...
    if not result:
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Используйте:</b>\n"
            "• <code>/mute [причина]</code> - ответом на сообщение\n"
            "• <code>/mute @username [причина]</code>\n"
            "• <code>/mute ID [причина]</code>\n\n"
            "⏱️ <i>Мут по умолчанию: {MUTE_DURATION_DAYS} день</i>".format(MUTE_DURATION_DAYS=MUTE_DURATION_DAYS),
            parse_mode="HTML"
        )
        return
...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    if not args and not message.reply_to_message:
        await message.answer(
            ...
            "📌 <b>Используйте:</b>\n"
            "• <code>/tmute [время] [причина]</code> - ответом на сообщение\n"
    ...
    # Обрабатываем случай с ответом на сообщение
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
    ...
    # Обрабатываем случай с аргументами
    elif args:
        ...
        if len(parts) < 2:
            await message.answer(
                "❌ <b>Неверный формат!</b>\n"
                "Используйте: <code>/tmute @username 1d [причина]</code>",
                parse_mode="HTML"
            )
            return
...
async def cmd_ban(message: Message, command: CommandObject):
    ...
    if not result:
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Используйте:</b>\n"
            "• <code>/ban [причина]</code> - ответом на сообщение\n"
...
async def cmd_tban(message: Message, command: CommandObject):
    ...
    if not args and not message.reply_to_message:
        await message.answer(
            "❌ <b>Не указан пользователь и время!</b>\n\n"
            "📌 <b>Используйте:</b>\n"
            "• <code>/tban [время] [причина]</code> - ответом на сообщение\n"
    ...
    # Обрабатываем случай с аргументами
        ...
        if len(parts) < 2:
            await message.answer(
                "❌ <b>Неверный формат!</b>\n"
                "Используйте: <code>/tban @username 1d [причина]</code>",
                parse_mode="HTML"
            )
            return
