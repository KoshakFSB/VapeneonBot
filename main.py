...
def _repair_db_if_needed(db_path: str):
    """
    Если БД содержит повреждённые таблицы (malformed schema / invalid rootpage),
    пересобирает её через dump → новый файл → замена.
    Повреждённые таблицы при этом теряются (они всё равно нечитаемы),
    остальные данные сохраняются.

    ВАЖНО: файл БД лежит на сетевом диске. На сетевых ФС проверка целостности
    может ЛОЖНО провалиться из-за временного сбоя блокировки файла, а не из-за
    реальной порчи данных. Если в такой момент слепо запустить пересборку через
    iterdump(), можно прочитать неполный/устаревший снимок файла и откатить
    недавние изменения назад. Поэтому:
      1) Проверка повторяется несколько раз с паузой — так отсеиваются
         единичные сетевые сбои.
      2) Реальная пересборка запускается только если ошибка стабильно
         повторяется на всех попытках.
      3) Всё логируется явно (WARNING/ERROR), чтобы при повторном срабатывании
         это сразу было видно в логах бота и не терялось незаметно.
    """
    ...
    for attempt in range(1, 6):
        try:
            ...
            try:
                ...
                conn.execute("PRAGMA integrity_check(1)").fetchall()
            finally:
                conn.close()
            return  # БД здорова
            ...
            last_error = e
            logger.warning(
                f"Проверка БД не прошла (попытка {attempt}/5): {e}. "
                f"Возможно, это временный сбой сетевого диска — повторяю проверку..."
            )
            time.sleep(2 * attempt)
        except Exception as e:
            # Не sqlite-специфичная ошибка (например, файл ещё не смонтирован) —
            # не считаем это порчей БД, просто ждём и пробуем снова
            last_error = e
            logger.warning(f"Не удалось открыть БД (попытка {attempt}/5): {e}. Повторяю...")
            time.sleep(2 * attempt)

    logger.error(
        f"БД стабильно не проходит проверку целостности после 5 попыток: {last_error}. "
        f"Запускаю восстановление через dump. Старая копия будет сохранена как {db_path}.broken"
    )
    ...
    try:
        ...
        # iterdump пропускает повреждённые таблицы с ошибкой, остальные дампит
        dump_lines = []
        ...
        logger.warning(
            f"БД восстановлена через пересборку. Старая версия сохранена как {backup_path}. "
            f"Если данные за последнее время пропали — они там."
        )
    except Exception as e:
        logger.error(f"Ошибка при восстановлении БД: {e} — создаём пустую БД")
        try:
            src.close()
        except Exception:
            pass
        try:
            ...
            pass
        # Если даже dump не помог — переименовываем и начинаем с чистого листа
...
def init_db():
    ...
    # Проверяем и восстанавливаем БД если повреждена
    _repair_db_if_needed(db_path)
    ...
    cursor.execute("INSERT OR IGNORE INTO admin_quest_state (id, is_open, max_applications, applications_count) VALUES (1, 0, NULL, 0)")
...
try:
    _conn = sqlite3.connect("data/bot_database.db", timeout=30)
...
def register_bot_user(user):
    ...
    try:
        conn = sqlite3.connect("data/bot_database.db", timeout=30)
...
def get_db_connection():
    ...
    timeout + busy_timeout снижают риск ложных 'database is locked'/повреждена
    при конкурентном доступе на сетевом диске."""
    conn = sqlite3.connect("data/bot_database.db", timeout=30)
    conn.execute("PRAGMA busy_timeout=30000")
    return conn
...
def set_admin_quest_open(max_applications: Optional[int] = None):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE admin_quest_state SET is_open = 1, max_applications = ?, applications_count = 0 WHERE id = 1",
        (max_applications,)
    )
    conn.commit()
    conn.close()
...
@dp.message(Command("dbinfo"), F.chat.type == ChatType.PRIVATE)
async def cmd_dbinfo(message: Message):
    """ВРЕМЕННАЯ диагностическая команда — удалить после диагностики.
    Показывает, какой файл БД реально видит процесс бота."""
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ У вас нет прав для использования этой команды.")
        return
    ...
    lines = [
        f"<b>cwd:</b> <code>{os.getcwd()}</code>",
        f"<b>resolved_path:</b> <code>{abs_path}</code>",
        f"<b>exists:</b> {exists}",
    ]
    if exists:
        st = os.stat(abs_path)
        lines.append(f"<b>size_bytes:</b> {st.st_size}")
        lines.append(f"<b>mtime:</b> {datetime.fromtimestamp(st.st_mtime).strftime('%Y-%m-%d %H:%M:%S')}")
        try:
            conn = sqlite3.connect(abs_path)
            cur = conn.cursor()
            cur.execute("SELECT COUNT(*) FROM shop_products WHERE is_active = 1")
            count = cur.fetchone()[0]
            conn.close()
            lines.append(f"<b>active_products_seen:</b> {count}")
        except Exception as e:
            lines.append(f"<b>query_error:</b> {e}")
...
async def cmd_addjbadm(message: Message, command: CommandObject):
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        ...
        if arg.lstrip("-").isdigit():
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT user_id, username FROM bot_users WHERE lower(username)=?", (uname,)
            )
            row = cursor.fetchone()
            conn.close()
            if row:
    ...
    if not target_username and target_user_id:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT username FROM bot_users WHERE user_id=?", (target_user_id,))
        row = cursor.fetchone()
        conn.close()
...
async def cmd_amnist(message: Message):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT user_id FROM bans WHERE chat_id = ? AND is_active = TRUE",
        (message.chat.id,)
    )
...
def get_product_list_for_delete(cat_index: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name FROM shop_products WHERE category = ? AND is_active = 1 ORDER BY id DESC",
        (category,)
    )
...
def add_product_to_db(category: str, name: str, description: str, photo_file_id: str, added_by: int, extra_photos: list = None) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO shop_products (category, name, description, photo_file_id, added_by) VALUES (?, ?, ?, ?, ?)",
    ...
    for i, fid in enumerate(all_photos):
        cursor.execute(
            "INSERT INTO shop_product_photos (product_id, file_id, sort_order) VALUES (?, ?, ?)",
            (product_id, fid, i)
        )
    conn.commit()
    conn.close()
...
def delete_product_from_db(product_id: int):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE shop_products SET is_active = 0 WHERE id = ?", (product_id,))
    conn.commit()
    conn.close()
...
async def adm_delete_confirm(callback: CallbackQuery):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM shop_products WHERE id = ?", (product_id,))
    row = cursor.fetchone()
    conn.close()
