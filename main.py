...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    # Получаем данные пользователя из аргументов или ответа
    user_data = await resolve_user_reference(message, command.args)
    if not user_data:
        return
    ...
    # Проверяем время, если оно есть в аргументах
    duration = MUTE_DURATION  # значение по умолчанию
    
    # Если в команде было указано время (для tmute)
    if command.args:
        args = command.args.split()
        if len(args) >= 2:  # есть и пользователь и время
    ...
    # Выдаем мут (функция mute_user сама проверит, бот это или нет)
