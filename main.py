...
def init_db():
    ...
    cursor.execute(
        ...
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER NOT NULL,
        photos TEXT NOT NULL DEFAULT '[]', -- JSON массив с file_id фотографий, по умолчанию пустой массив
        description TEXT NOT NULL,
        price TEXT NOT NULL,
        username TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    # Таблица для блокировки публикаций объявлений
    cursor.execute(
    ...
    # Индексы для таблицы блокировок объявлений
...
def check_database_tables():
    """Проверяет существование всех необходимых таблиц и выводит статистику"""
...
async def restore_active_punishments():
    ...
    try:
        ...
        print(f"🔄 Восстановление активных наказаний... Текущее время: {current_time}")
        ...
        print(f"📊 Найдено активных мутов: {len(active_mutes)}")
        ...
        for user_id, chat_id, expires_at, reason in active_mutes:
            try:
                ...
                if member.status in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.CREATOR]:
                    ...
                    print(f"⚠️ Мут администратора {user_id} деактивирован")
                    continue
                ...
                restored_count += 1
                print(f"✅ Восстановлен мут для пользователя {user_id} в чате {chat_id}")
                
            except Exception as e:
                print(f"❌ Ошибка восстановления мута для {user_id}: {e}")
        ...
        print(f"📊 Найдено активных банов: {len(active_bans)}")
        ...
        for user_id, chat_id, expires_at, reason in active_bans:
            try:
                ...
                restored_bans += 1
                print(f"✅ Восстановлен бан для пользователя {user_id} в чате {chat_id}")
            except Exception as e:
                print(f"❌ Ошибка восстановления бана для {user_id}: {e}")
                cursor.execute(
                    "UPDATE bans SET is_active = FALSE WHERE user_id = ? AND chat_id = ?",
                    (user_id, chat_id)
                )
        
        # Восстанавливаем активные блокировки объявлений
        cursor.execute("""
            SELECT user_id, chat_id, expires_at, reason, issued_by
            FROM ad_blocks 
            WHERE is_active = TRUE AND (expires_at IS NULL OR expires_at > ?)
        """, (current_time,))
        
        active_ad_blocks = cursor.fetchall()
        print(f"📊 Найдено активных блокировок объявлений: {len(active_ad_blocks)}")
        
        conn.commit()
        conn.close()
        
        print(f"✅ Восстановлено наказаний: {restored_count} мутов, {restored_bans} банов")
        
    except Exception as e:
        print(f"❌ Ошибка восстановления наказаний: {e}")
...
def get_user_warns(user_id: int, chat_id: int) -> List[Dict[str, Any]]:
    ...
    current_time = datetime.now()
    cursor.execute(
        "SELECT id, reason, issued_at, expires_at FROM warns WHERE user_id = ? AND chat_id = ? AND (expires_at IS NULL OR expires_at > ?)",
...
def get_active_complaints() -> List[Dict[str, Any]]:
    ...
    for row in cursor.fetchall():
        complaints.append({
            "id": row[0],
            "user_id": row[1],
            "username": row[2] if row[2] else "Неизвестный",
            "admin_username": row[3] if row[3] else "Неизвестный",
            "description": row[4] if row[4] else "Не указано",
            "complaint_text": row[5] if row[5] else "Не указано",
            "evidence": row[6] if row[6] else "Не предоставлены",
            "created_at": row[7],
            "status": row[8],
            "handled_by": row[9],
...
def save_admin_complaint(user_id: int, username: str, admin_username: str, description: str, complaint_text: str, evidence: str = None) -> int:
    ...
    evidence = evidence or "Не предоставлены"
...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    """
    Разрешает ссылку на пользователя.
    ПОЛНОСТЬЮ ИГНОРИРУЕМ reply_to_message, если есть аргументы!
    """
    reason = "Не указана"
    
    logger.info(f"resolve_user_reference called with command_args: {command_args}")
    logger.info(f"Message has reply_to_message: {message.reply_to_message is not None} (ИГНОРИРУЕМ!)")
    
    # ЕСЛИ ЕСТЬ АРГУМЕНТЫ - ИСПОЛЬЗУЕМ ТОЛЬКО ИХ, reply НАС НЕ ИНТЕРЕСУЕТ
    if command_args:
        # Разбиваем аргументы
        args_parts = command_args.strip().split(maxsplit=1)
        
        if len(args_parts) >= 1:
            user_part = args_parts[0].strip()
            logger.info(f"Processing user_part: '{user_part}'")
            
            # Получаем ID из аргументов
            if user_part.isdigit():
                user_id = int(user_part)
                logger.info(f"Found user_id from args (digit): {user_id}")
            elif user_part.startswith('@'):
                try:
                    user = await bot.get_chat(user_part)
                    user_id = user.id
                    logger.info(f"Found user_id from args (username): {user_id}")
                except Exception as e:
                    logger.error(f"Error getting user by username: {e}")
                    await message.answer(f"❌ Пользователь {user_part} не найден.")
                    return None
            else:
                try:
                    user_id = int(user_part)
                    logger.info(f"Found user_id from args (converted): {user_id}")
                except ValueError:
                    await message.answer(f"❌ Неверный формат пользователя: {user_part}")
                    return None
            
            # Берем причину если есть
            if len(args_parts) > 1:
                reason = args_parts[1]
                logger.info(f"Reason from args: {reason}")
            
            # Проверяем, что это не бот
            try:
                user = await bot.get_chat(user_id)
                logger.info(f"User type: {user.type}, is_bot: {getattr(user, 'is_bot', False)}")
                
                if hasattr(user, 'is_bot') and user.is_bot:
                    await message.answer("❌ Нельзя выдавать наказания ботам.")
                    return None
            except Exception as e:
                logger.warning(f"Could not verify user {user_id}: {e}")
                # Если не можем проверить, всё равно продолжаем
            
            return (user_id, reason)
    
    # ЕСЛИ АРГУМЕНТОВ НЕТ - только тогда проверяем reply
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        logger.info(f"No args, using reply: {user_id}")
...
async def resolve_user_only(message: Message, command_args: str = None) -> Optional[int]:
    ...
    ПОЛНОСТЬЮ ИГНОРИРУЕМ reply_to_message, если есть аргументы!
    """
    logger.info(f"resolve_user_only called with command_args: {command_args}")
    logger.info(f"Message has reply_to_message: {message.reply_to_message is not None} (ИГНОРИРУЕМ!)")
    
    # ЕСЛИ ЕСТЬ АРГУМЕНТЫ - ИСПОЛЬЗУЕМ ТОЛЬКО ИХ
    if command_args:
        args = command_args.strip().split()
        if args:
            user_part = args[0].strip()
            logger.info(f"Processing user_part: '{user_part}'")
...
async def cmd_unwarn(message: Message, command: CommandObject):
    ...
    user_id = await resolve_user_only(message, command.args)
    if not user_id:
        await message.answer("❌ Укажите пользователя (ответом на сообщение или @username/id).")  # Убрано message_thread_id
        return
    ...
    if not warns:
        user_mention = await get_user_mention(user_id)
        await message.answer(  # Убрано message_thread_id
...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    # Получаем ID пользователя через resolve_user_reference
    user_data = await resolve_user_reference(message, args)
    if not user_data:
        return
    ...
    # Проверяем, есть ли время в аргументах
    if args:
        parts = args.split()
        if len(parts) >= 2:
            # Второй аргумент — это время
            potential_time = parts[1]
            if parse_time(potential_time):
                time_str = potential_time
                # Если есть третий аргумент и больше — это причина
                if len(parts) > 2:
                    reason = ' '.join(parts[2:])
            else:
                # Если второй аргумент не время — значит причина без времени
                reason = args
...
async def cmd_unmute(message: Message, command: CommandObject):
    ...
    if await unmute_user(message.chat.id, user_id, message.message_thread_id):
        # Сообщение уже отправляется в unmute_user
        pass
    else:
...
async def cmd_tban(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        ...
        if message.reply_to_message.from_user.is_bot:
            await message.answer("❌ Нельзя банить ботов.")
            return
...
async def cmd_unban(message: Message, command: CommandObject):
    ...
    if await unban_user(message.chat.id, user_id, message.message_thread_id):
        # Сообщение уже отправляется в unban_user
        pass
    else:
...
async def diagnostic_check():
    """Диагностическая функция для проверки работы базы данных"""
    print("\n" + "="*50)
...
async def main():
    ...
    print("🔄 Восстановление активных наказаний...")
