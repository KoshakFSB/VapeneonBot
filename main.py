...
def get_ad_rating_keyboard(ad_id: int, user_id: int):
    ...
    keyboard.row(InlineKeyboardButton(text="📝 Отзывы о продавце", callback_data=f"user_reviews:{user_id}"))
...
async def process_ad_photos(message: Message, state: FSMContext):
    ...
    if text.lower() in ['/done', 'готово', 'done']:
        if not photos:
            await message.answer("❌ Вы не отправили ни одной фотографии. Отправьте фото или нажмите /cancel для отмены.")
            return
    ...
    if message.photo:
        ...
        await message.answer("❌ Пожалуйста, отправьте фотографию или напишите 'готово' если закончили.")
...
async def publish_ad_handler(callback: types.CallbackQuery):
    ...
    caption = (
        ...
        f"💰 <b>Цена:</b> {ad['price']}\n"
        f"👤 <b>Продавец:</b> {ad['username']}\n\n"
        f"<i>Оцените объявление и оставьте отзыв о продавце!</i>"
    )
    ...
    try:
        ...
        main_message_id = sent_messages[0].message_id
        
        # Отправляем клавиатуру для оценок отдельным сообщением
        await bot.send_message(
            chat_id=CHAT_ID,
            text="⚡️ <b>Оцените объявление:</b>",
            parse_mode="HTML",
            reply_markup=get_ad_rating_keyboard(ad_id, ad['user_id']),
            message_thread_id=BARAHOLKA_THREAD_ID,
...
async def rate_ad_handler(callback: types.CallbackQuery):
    ...
    keyboard.row(InlineKeyboardButton(text="📝 Отзывы о продавце", callback_data=f"user_reviews:{ad['user_id']}"))
