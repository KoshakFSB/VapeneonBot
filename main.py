...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    # СНАЧАЛА проверяем аргументы команды
    if command_args:
        ...
        if len(args_parts) >= 1:
            ...
            if user_part.isdigit():
                user_id = int(user_part)  # Берем ID из аргументов!
                if len(args_parts) > 1:
                    reason = args_parts[1]
                logger.info(f"Found user via args: {user_id}")
                ...
                try:
                    ...
                    if len(args_parts) > 1:
                        reason = args_parts[1]
                    logger.info(f"Found user via args username: {user_id}")
                    return (user_id, reason)
                except:
                    pass
    
    # ТОЛЬКО если в аргументах не было ID, проверяем reply
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        logger.info(f"Found user via reply (fallback): {user_id}")
        if command_args and not reason:
            reason = command_args
        return (user_id, reason)
    ...
    # Затем проверяем аргументы команды
        ...
        if args:
            ...
            if user_part.isdigit():
                ...
                # ИСПРАВЛЕНИЕ: Проверяем на бота ТОЛЬКО если user_id похож на ID бота
                # (ID ботов обычно начинаются с определенных цифр, но лучше просто убрать проверку)
                # Просто возвращаем ID, проверку на бота делаем позже
                return user_id
            elif user_part.startswith('@'):
                try:
                    user = await bot.get_chat(user_part)
