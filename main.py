...
async def get_user_id_from_message(text: str) -> Optional[int]:
    ...
    """
    logger.info(f"=== get_user_id_from_message ===")
    ...
    logger.info(f"После обработки: '{text}' (было: '{original_text}')")
    
    # Проверяем, является ли это числом (ID)
    if text.isdigit():
        user_id = int(text)
        logger.info(f"Это числовой ID: {user_id}")
        return user_id
    
    # Иначе пробуем получить по юзернейму
    try:
        logger.info(f"Пробуем получить пользователя по юзернейму @{text}")
        user = await bot.get_chat(f"@{text}")
        logger.info(f"Успешно получен пользователь: {user.id} (@{user.username})")
        return user.id
    except Exception as e:
...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    """
    logger.info(f"=== resolve_user_reference ===")
    logger.info(f"command_args: '{command_args}'")
    logger.info(f"Есть reply_to_message: {message.reply_to_message is not None}")
    
    user_id = None
    reason = "Не указана"
    
    # СЛУЧАЙ 1: ВСЕГДА проверяем ответ на сообщение
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        logger.info(f"Получен ID из ответа: {user_id}")
        
        # Если есть аргументы команды - используем их как причину
        if command_args and command_args.strip():
            reason = command_args.strip()
            logger.info(f"Причина из аргументов: '{reason}'")
        else:
            reason = "Не указана"
            logger.info("Причина не указана, используем значение по умолчанию")
            
        logger.info(f"Возвращаем: ({user_id}, '{reason}')")
        return (user_id, reason)
    
    # СЛУЧАЙ 2: Если нет ответа, пробуем аргументы команды
    if command_args and command_args.strip():
        parts = command_args.strip().split(maxsplit=1)
        user_identifier = parts[0]
        reason = parts[1] if len(parts) > 1 else "Не указана"
        
        logger.info(f"user_identifier: '{user_identifier}', reason: '{reason}'")
        ...
        if user_id:
            ...
            logger.warning(f"Не удалось получить ID из '{user_identifier}'")
...
async def resolve_user_only(message: Message, command_args: str = None) -> Optional[int]:
    ...
    """
    logger.info(f"=== resolve_user_only ===")
    logger.info(f"command_args: '{command_args}'")
    logger.info(f"Есть reply_to_message: {message.reply_to_message is not None}")
    ...
    # СЛУЧАЙ 2: Если нет ответа, пробуем аргументы команды
    if command_args and command_args.strip():
        user_identifier = command_args.strip().split()[0]
        logger.info(f"user_identifier из аргументов: '{user_identifier}'")
        
        user_id = await get_user_id_from_message(user_identifier)
        if user_id:
            logger.info(f"Получен ID из аргументов: {user_id}")
            return user_id
        else:
            logger.warning(f"Не удалось получить ID из '{user_identifier}'")
...
async def cmd_warn(message: Message, command: CommandObject):
    ...
    logger.info(f"=== КОМАНДА /warn ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Тип чата: {message.chat.type}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    
    if message.reply_to_message:
        logger.info(f"ID пользователя в ответе: {message.reply_to_message.from_user.id}")
        logger.info(f"Username в ответе: @{message.reply_to_message.from_user.username}")
    
    if not await is_chat_admin(message.from_user.id, message.chat.id):
        logger.warning(f"Пользователь {message.from_user.id} не является администратором, применяем наказание")
    ...
    # Используем resolve_user_reference для получения user_id и причины
    result = await resolve_user_reference(message, command.args)
    
    logger.info(f"Результат resolve_user_reference: {result}")
    ...
    if not result:
        ...
        await message.answer(
            ...
            "   Пример: <code>/warn Спам</code>\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/warn ID [причина]</code>\n"
            "   Пример: <code>/warn 123456789 Спам</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
            "   <code>/warn @username [причина]</code>\n"
            "   Пример: <code>/warn @spammer Спам</code>\n\n"
            "📝 <i>Причина необязательна, но желательна</i>",
            parse_mode="HTML"
        )
        return
    
    user_id, reason = result
    logger.info(f"Команда /warn: пользователь {message.from_user.id} выдаёт предупреждение {user_id} с причиной: '{reason}'")

    # Проверяем, не является ли пользователь администратором
    if await is_chat_admin(user_id, message.chat.id):
        logger.warning(f"Пользователь {user_id} является администратором, нельзя выдать предупреждение через /warn")
        await message.answer("❌ Нельзя выдавать предупреждения администраторам через эту команду. Используйте /admin_warn")
        return

    # Вызываем функцию warn_user
    logger.info(f"Вызываем warn_user для {user_id} в чате {message.chat.id}")
    success = await warn_user(message.chat.id, user_id, reason, message.message_thread_id)
    logger.info(f"Результат warn_user: {success}")
...
async def cmd_unwarn(message: Message, command: CommandObject):
    ...
    user_id = await resolve_user_only(message, command.args)
    logger.info(f"Результат resolve_user_only: {user_id}")
    
    if not user_id:
        logger.warning("Не удалось определить пользователя для снятия предупреждения")
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Форматы использования:</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
            "   <code>/unwarn</code>\n"
            "   (без аргументов, просто ответьте на сообщение пользователя)\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/unwarn ID</code>\n"
            "   Пример: <code>/unwarn 123456789</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
            "   <code>/unwarn @username</code>\n"
            "   Пример: <code>/unwarn @user</code>",
            parse_mode="HTML"
        )
        return

    logger.info(f"Команда /unwarn: пользователь {message.from_user.id} снимает предупреждение с {user_id}")

    # Получаем все активные варны пользователя
    warns = get_user_warns(user_id, message.chat.id)
    logger.info(f"Активные предупреждения пользователя {user_id}: {len(warns)}")
    
    if not warns:
        user_mention = await get_user_mention(user_id)
        logger.info(f"У пользователя {user_id} нет активных предупреждений")
        await message.answer(
            f"✅ У пользователя {user_mention} нет активных предупреждений.",
            parse_mode="HTML"
        )
        return
    ...
    for i, warn in enumerate(warns):
        logger.info(f"Предупреждение #{i+1}: ID={warn['id']}, причина='{warn['reason']}', выдано={warn['issued_at']}")
    
    # Удаляем последнее предупреждение (с наибольшим ID)
    last_warn = max(warns, key=lambda x: x['id'])
    logger.info(f"Последнее предупреждение: ID={last_warn['id']}, причина='{last_warn['reason']}'")
    
    logger.info(f"Вызываем remove_warn для ID {last_warn['id']}")
    remove_warn(last_warn['id'])
    logger.info(f"Предупреждение {last_warn['id']} удалено")
    
    user_mention = await get_user_mention(user_id)
    remaining_warns = len(warns) - 1
    
    logger.info(f"У пользователя {user_id} осталось предупреждений: {remaining_warns}")
    ...
    # Дополнительно проверяем, обновились ли предупреждения в БД
    updated_warns = get_user_warns(user_id, message.chat.id)
    logger.info(f"После удаления, активных предупреждений: {len(updated_warns)}")

@dp.message(Command("mute"))
async def cmd_mute(message: Message, command: CommandObject):
    """Мут пользователя на 1 день"""
    logger.info(f"=== КОМАНДА /mute ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    ...
    if not result:
        ...
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Форматы использования (мут на 1 день):</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
            "   <code>/mute [причина]</code>\n"
            "   Пример: <code>/mute Оскорбления</code>\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/mute ID [причина]</code>\n"
            "   Пример: <code>/mute 123456789 Оскорбления</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
            "   <code>/mute @username [причина]</code>\n"
            "   Пример: <code>/mute @user Оскорбления</code>\n\n"
            f"⏱️ <i>Мут по умолчанию: {MUTE_DURATION_DAYS} день</i>",
            parse_mode="HTML"
        )
        return
    
    user_id, reason = result
    logger.info(f"Команда /mute: пользователь {message.from_user.id} мутит {user_id} с причиной: '{reason}'")

    if await is_chat_admin(user_id, message.chat.id):
        logger.warning(f"Пользователь {user_id} является администратором, нельзя замутить")
        await message.answer("❌ Нельзя мутить администраторов.")
        return

    # Вызываем функцию mute_user
    logger.info(f"Вызываем mute_user для {user_id} в чате {message.chat.id} на {MUTE_DURATION_DAYS} день")
    success = await mute_user(message.chat.id, user_id, MUTE_DURATION, reason, message_thread_id=message.message_thread_id)
    logger.info(f"Результат mute_user: {success}")

@dp.message(Command("tmute"))
async def cmd_tmute(message: Message, command: CommandObject):
    """Мут пользователя на указанное время"""
    logger.info(f"=== КОМАНДА /tmute ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    ...
    args = command.args
    logger.info(f"Разбираем аргументы: {args}")
    
    user_id = None
    time_str = None
    reason = "Не указана"

    # СЛУЧАЙ 1: Ответ на сообщение
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        logger.info(f"Получен ID из ответа: {user_id}")
        
        if args:
            # Разделяем аргументы: время и причина
            parts = args.strip().split(maxsplit=1)
            time_str = parts[0]
            reason = parts[1] if len(parts) > 1 else "Не указана"
            logger.info(f"Из аргументов: время={time_str}, причина='{reason}'")
        else:
            logger.warning("Не указано время при ответе на сообщение")
            await message.answer("❌ Укажите время: /tmute [время] [причина] при ответе на сообщение")
            return
            
    # СЛУЧАЙ 2: Аргументы команды
    elif args:
        parts = args.strip().split(maxsplit=2)
        logger.info(f"Разбитые аргументы: {parts}")
        
        if len(parts) < 2:
            logger.warning("Недостаточно аргументов")
            await message.answer(
                "❌ <b>Неверный формат!</b>\n"
                "Используйте: <code>/tmute @username 1d [причина]</code> или <code>/tmute 123456789 2h [причина]</code>",
                parse_mode="HTML"
            )
            return
        ...
        logger.info(f"Ищем пользователя: {user_identifier}, время={time_str}, причина='{reason}'")
        
        user_id = await get_user_id_from_message(user_identifier)
        logger.info(f"Результат get_user_id_from_message: {user_id}")
        
        if not user_id:
            logger.warning(f"Пользователь {user_identifier} не найден")
            await message.answer("❌ Пользователь не найден.")
            return
    else:
        logger.warning("Нет аргументов и нет ответа на сообщение")
        await message.answer(
            "❌ <b>Не указан пользователь и время!</b>\n\n"
            "📌 <b>Форматы использования:</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
            "   <code>/tmute [время] [причина]</code>\n"
            "   Пример: <code>/tmute 2h Флуд</code>\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/tmute ID [время] [причина]</code>\n"
            "   Пример: <code>/tmute 123456789 1d Оскорбления</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
            "   <code>/tmute @username [время] [причина]</code>\n"
            "   Пример: <code>/tmute @user 30m Спам</code>",
            parse_mode="HTML"
        )
        return

    # Парсим время
    logger.info(f"Парсим время: {time_str}")
    duration = parse_time(time_str)
    logger.info(f"Результат parse_time: {duration}")
    
    if not duration:
        logger.warning(f"Неверный формат времени: {time_str}")
        await message.answer(
            "❌ <b>Неверный формат времени.</b>\n"
            "Используйте: <code>30m, 2h, 1d, 1w</code>",
            parse_mode="HTML"
        )
        return

    logger.info(f"Команда /tmute: пользователь {message.from_user.id} мутит {user_id} на {time_str} ({duration}) с причиной: '{reason}'")
    ...
    # Вызываем функцию mute_user
    logger.info(f"Вызываем mute_user для {user_id} в чате {message.chat.id} на {duration}")
    success = await mute_user(message.chat.id, user_id, duration, reason, message_thread_id=message.message_thread_id)
    logger.info(f"Результат mute_user: {success}")
...
async def cmd_unmute(message: Message, command: CommandObject):
    ...
    if not user_id:
        ...
        await message.answer(
            ...
            "   Пример: <code>/unmute @user</code>",
            parse_mode="HTML"
        )
        return

    logger.info(f"Команда /unmute: пользователь {message.from_user.id} снимает мут с {user_id}")

    # Вызываем функцию unmute_user
    logger.info(f"Вызываем unmute_user для {user_id} в чате {message.chat.id}")
    success = await unmute_user(message.chat.id, user_id, message.message_thread_id)
    logger.info(f"Результат unmute_user: {success}")

@dp.message(Command("ban"))
async def cmd_ban(message: Message, command: CommandObject):
    """Бан пользователя навсегда"""
    logger.info(f"=== КОМАНДА /ban ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    ...
    if not result:
        ...
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Форматы использования (бан навсегда):</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
            "   <code>/ban [причина]</code>\n"
            "   Пример: <code>/ban Реклама</code>\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/ban ID [причина]</code>\n"
            "   Пример: <code>/ban 123456789 Реклама</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
    ...
    user_id, reason = result
    logger.info(f"Команда /ban: пользователь {message.from_user.id} банит {user_id} с причиной: '{reason}'")

    if await is_chat_admin(user_id, message.chat.id):
        logger.warning(f"Пользователь {user_id} является администратором, нельзя забанить")
        await message.answer("❌ Нельзя банить администраторов.")
        return

    # Вызываем функцию ban_user
    logger.info(f"Вызываем ban_user для {user_id} в чате {message.chat.id} (навсегда)")
    success = await ban_user(message.chat.id, user_id, None, reason, message_thread_id=message.message_thread_id)
    logger.info(f"Результат ban_user: {success}")

@dp.message(Command("tban"))
async def cmd_tban(message: Message, command: CommandObject):
    """Бан пользователя на указанное время"""
    logger.info(f"=== КОМАНДА /tban ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    ...
    if message.reply_to_message:
        ...
        if args:
            ...
            logger.warning("Не указано время при ответе на сообщение")
            await message.answer("❌ Укажите время: /tban [время] [причина] при ответе на сообщение")
            return
    ...
    # СЛУЧАЙ 2: Аргументы команды
        ...
        if len(parts) < 2:
            ...
            await message.answer(
                "❌ <b>Неверный формат!</b>\n"
                "Используйте: <code>/tban @username 1d [причина]</code> или <code>/tban 123456789 2h [причина]</code>",
                parse_mode="HTML"
            )
            return
        ...
        await message.answer(
            ...
            "📌 <b>Форматы использования:</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
            "   <code>/tban [время] [причина]</code>\n"
            "   Пример: <code>/tban 3d Реклама</code>\n\n"
            "2️⃣ <b>По ID пользователя:</b>\n"
            "   <code>/tban ID [время] [причина]</code>\n"
            "   Пример: <code>/tban 123456789 1w Оскорбления</code>\n\n"
            "3️⃣ <b>По @username:</b>\n"
            "   <code>/tban @username [время] [причина]</code>\n"
            "   Пример: <code>/tban @user 2d Спам</code>",
            parse_mode="HTML"
        )
        return
    ...
    logger.info(f"Команда /tban: пользователь {message.from_user.id} банит {user_id} на {time_str} ({duration}) с причиной: '{reason}'")
    ...
    # Вызываем функцию ban_user
    logger.info(f"Вызываем ban_user для {user_id} в чате {message.chat.id} на {duration}")
    success = await ban_user(message.chat.id, user_id, duration, reason, message_thread_id=message.message_thread_id)
    logger.info(f"Результат ban_user: {success}")

@dp.message(Command("unban"))
async def cmd_unban(message: Message, command: CommandObject):
    """Снятие бана с пользователя"""
    logger.info(f"=== КОМАНДА /unban ===")
    logger.info(f"Отправитель: {message.from_user.id} (@{message.from_user.username})")
    logger.info(f"Чат: {message.chat.id}")
    logger.info(f"Аргументы команды: {command.args}")
    logger.info(f"Есть ответ на сообщение: {message.reply_to_message is not None}")
    ...
    if not user_id:
        logger.warning("Не удалось определить пользователя для разбана")
        await message.answer(
            "❌ <b>Не указан пользователь!</b>\n\n"
            "📌 <b>Форматы использования:</b>\n\n"
            "1️⃣ <b>Ответ на сообщение:</b>\n"
    ...
    logger.info(f"Команда /unban: пользователь {message.from_user.id} снимает бан с {user_id}")

    # Проверяем, есть ли активный бан в базе данных
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM bans WHERE user_id = ? AND chat_id = ? AND is_active = TRUE",
        (user_id, message.chat.id)
    )
    active_ban = cursor.fetchone()
    conn.close()
    
    logger.info(f"Активный бан в БД: {active_ban is not None}")
    
    if not active_ban:
        logger.info(f"У пользователя {user_id} нет активного бана в БД, но пробуем разбанить в Telegram")
        # Всё равно пробуем разбанить в Telegram

    # Вызываем функцию unban_user
    logger.info(f"Вызываем unban_user для {user_id} в чате {message.chat.id}")
    success = await unban_user(message.chat.id, user_id, message.message_thread_id)
    logger.info(f"Результат unban_user: {success}")
    
    if success:
        # Деактивируем бан в базе данных
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE bans SET is_active = FALSE WHERE user_id = ? AND chat_id = ? AND is_active = TRUE",
        ...
        conn.commit()
        conn.close()
        logger.info(f"Бан деактивирован в БД для пользователя {user_id}")
