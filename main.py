...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    Если есть аргументы команды - используем ТОЛЬКО их, полностью игнорируем reply!
    """
    reason = "Не указана"
    ...
    if command_args:
        ...
        if len(args_parts) >= 1:
            ...
            if user_part.isdigit():
                ...
                logger.info(f"Using user_id from args: {user_id}")
            elif user_part.startswith('@'):
                try:
                    user = await bot.get_chat(user_part)
                    user_id = user.id
                    logger.info(f"Using user_id from args (username): {user_id}")
                except Exception as e:
                ...
                try:
                    user_id = int(user_part)
                    logger.info(f"Using user_id from args (converted): {user_id}")
                except ValueError:
            ...
            # ВАЖНО: Проверяем что это не бот, но с правильной обработкой
            try:
                user = await bot.get_chat(user_id)
                # Проверяем наличие атрибута is_bot
                if hasattr(user, 'is_bot') and user.is_bot:
                    await message.answer("❌ Нельзя выдавать наказания ботам.")
                    return None
                # Если это пользователь или группа - всё ок
                logger.info(f"User type: {user.type}, is_bot: {getattr(user, 'is_bot', False)}")
            except Exception as e:
                # Если не можем получить информацию о пользователе, всё равно продолжаем
                # Возможно пользователь скрыл аккаунт или удалился
                logger.warning(f"Cannot get chat info for {user_id}: {e}")
                pass
            
            return (user_id, reason)
    
    # ЕСЛИ АРГУМЕНТОВ НЕТ - тогда проверяем reply
