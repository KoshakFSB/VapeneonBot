...
async def cmd_admin_add(message: Message, command: CommandObject):
    ...
    try:
        ...
        try:
            async with __import__('httpx').AsyncClient(timeout=8) as _client:
                await _client.post(f"{SITE_URL}/api/site-admins/add", json={
                    "secret": BOT_TOKEN,
                    "tg_id": user_id,
...
"
    ...
    target_username = command.args.strip().lstrip("@").lower()
    ...
    try:
        async with __import__('httpx').AsyncClient(timeout=8) as client:
            ...
            if r.status_code == 404:
                await message.answer(f"❌ @{target_username} не найден среди администраторов сайта.
Сначала назначьте его администратором через бота.")
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT user_id FROM bot_users WHERE lower(username)=?", (target_username,))
    row = cursor.fetchone()
    conn.close()
    if row:
        try:
            await bot.send_message(
                row[0],
                f"⚖️ <b>Вам выданы расширенные права на сайте VapeNeon</b>

"
                f"Теперь вы можете рассматривать жалобы на администраторов.
...
"
            ...
            logger.error(f"Could not notify {target_username}: {e}")

    await message.answer(
        f"✅ @{target_username} теперь может рассматривать жалобы на администраторов на сайте.",
        parse_mode="HTML"
    )
