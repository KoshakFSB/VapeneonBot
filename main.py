...
async def cmd_clearwarns(message: Message, command: CommandObject):
    ...
    await message.answer(f"✅ Предупреждения пользователя {user_mention} очищены.")
...
async def cmd_unmute(message: Message, command: CommandObject):
    ...
    if await unmute_user(message.chat.id, user_id):
        user_mention = await get_user_mention(user_id)
        await message.answer(f"✅ Пользователь {user_mention} размучен.")
    else:
...
async def cmd_activate(message: Message):
    ...
    if is_user_activated(message.from_user.id):
        await message.answer("✅ Вы уже активированы в системе.")
        return
    ...
    if is_ad_message(text):
        ...
        if not is_user_activated(user_id):
            ...
            activation_msg = await message.answer(
                f"❌ <b>Объявление удалено</b>\n\n"
                f"Пользователь {await get_user_mention(user_id)} не активирован в системе.\n"
                f"Для активации напишите боту в ЛС: @{(await bot.get_me()).username}",
                parse_mode="HTML"
            )
            
            # Удаляем сообщение об активации через 30 секунд
            await asyncio.sleep(30)
            await delete_message(message.chat.id, activation_msg.message_id)
            return
...
async def handle_chat_member_update(update: ChatMemberUpdated):
    ...
    if (old_status == ChatMemberStatus.LEFT and 
        ...
        ⚠️ <b>Внимание:</b> перед отправкой объявлений необходимо активировать аккаунт!
        ...
        # Удаляем приветствие через 5 минут
