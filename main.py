...
def init_db():
    ...
    cursor.execute(
        ...
        published_message_ids TEXT -- JSON массив с ID опубликованных сообщений в чате
    )"""
    )
    # Миграция: добавляем колонку published_message_ids если её нет (для существующих БД)
    try:
        cursor.execute("ALTER TABLE ads ADD COLUMN published_message_ids TEXT")
    except sqlite3.OperationalError:
        pass  # Колонка уже существует
...
def get_ad_by_id(ad_id: int) -> Optional[Dict[str, Any]]:
    ...
    cursor.execute(
        "SELECT id, user_id, photos, description, price, username, created_at, status, published_at, published_message_ids FROM ads WHERE id = ?",
        (ad_id,)
    )
    row = cursor.fetchone()
    conn.close()
    ...
    if row:
        ...
        return {
            ...
            "status": row[7],
            "published_at": row[8],
            "published_message_ids": json.loads(row[9]) if row[9] else []
        }
    return None
...
def publish_ad(ad_id: int, message_ids: list = None):
    """Публикует объявление и сохраняет ID сообщений в чате"""
    import json
    conn = get_db_connection()
    cursor = conn.cursor()
    message_ids_json = json.dumps(message_ids) if message_ids else None
    cursor.execute(
        "UPDATE ads SET status = 'published', published_at = CURRENT_TIMESTAMP, published_message_ids = ? WHERE id = ?",
        (message_ids_json, ad_id)
    )
    conn.commit()
    conn.close()

def get_user_published_ad(user_id: int) -> Optional[Dict[str, Any]]:
    """Получает последнее опубликованное объявление пользователя с сохранёнными ID сообщений"""
    import json
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        """SELECT id, published_message_ids FROM ads 
           WHERE user_id = ? AND status = 'published' AND published_message_ids IS NOT NULL
    ...
    conn.close()
    if row:
        return {"id": row[0], "published_message_ids": json.loads(row[1]) if row[1] else []}
    return None
...
def get_main_keyboard(user_id: int = None):
    ...
    builder.row(KeyboardButton(text="📦 Создать объявление"))
    ...
    builder.row(KeyboardButton(text="📝 Мои объявления"), KeyboardButton(text="👤 Мой профиль"))
...
async def cmd_start_with_reviews(message: Message, command: CommandObject, state: FSMContext):
    ...
    # Проверяем режим тех.работ (владельцы обходят его)
    ...
    # Обработка ссылки ?start=appeal_<тип>_<user_id> (обжалование наказания)
    if command.args and command.args.startswith("appeal_"):
        try:
            # Формат: appeal_<тип>_<user_id>
            parts = command.args.split("_")
            punishment_type = parts[1]          # warn / mute / ban
            punished_user_id = int(parts[2])    # ID наказанного
    ...
    # Обработка ссылки ?start=onboarding (fallback когда ЛС не открыты)
    if command.args == "onboarding":
        if not is_onboarding_completed(user_id):
            start_onboarding_db(user_id)  # создаём запись, если ещё нет
            builder = InlineKeyboardBuilder()
            ...
            return
        # Онбординг уже пройден — показываем обычное меню
    ...
    welcome_text = (
        ...
        "Здесь ты можешь:\n"
        "• 📦 Публиковать объявления о продаже\n"
        "• ⭐️ Оставлять отзывы о продавцах\n"
        "• 📢 Подавать жалобы на администраторов\n\n"
        "Используй кнопки меню ниже 👇"
    )
    await message.answer_animation(
        animation="CgACAgIAAxkBAAFF16Vpx4afDK5BxqzI25JHNeYHRV6QsQACA0oAAoM0oUupvCegy90oZzoE",
...
@dp.message(F.chat.type == ChatType.PRIVATE, F.text == "📦 Создать объявление")
async def btn_create_ad(message: Message):
    """Кнопка «Создать объявление» — запускает тот же флоу, что и /public"""
    await cmd_public(message)

@dp.message(F.chat.type == ChatType.PRIVATE, F.text == "📝 Мои объявления")
async def btn_my_ads(message: Message):
    user_id = message.from_user.id
    ads = get_user_ads(user_id)
    if not ads:
        await message.answer("📭 У вас нет объявлений.", reply_markup=get_main_keyboard(message.from_user.id))
        return
    keyboard = get_my_ads_keyboard(user_id)
    await message.answer("📋 <b>Ваши объявления:</b>", parse_mode="HTML", reply_markup=keyboard)

@dp.message(F.chat.type == ChatType.PRIVATE, F.text == "👤 Мой профиль")
...
async def btn_start_onboarding(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if is_onboarding_completed(user_id):
        await message.answer("✅ Ты уже прошёл обучение! Если хочешь повторить — напиши /start.", reply_markup=get_main_keyboard(message.from_user.id))
        return
    remaining = get_onboarding_cooldown_remaining(user_id)
    if remaining:
        ...
        await message.answer(
            f"⏳ <b>Обучение временно недоступно</b>\n\nПовторная попытка через: <b>{hours} ч. {minutes} мин.</b>",
    ...
    await message.answer(
        "👋 <b>Привет!</b>\n\n"
        "Чтобы начать писать в чат — пройди короткое обучение. Это займёт <b>2-3 минуты</b>.\n\n"
        "Готов начать?",
        parse_mode="HTML",
        reply_markup=builder.as_markup()
    )
...
async def publish_ad_handler(callback: types.CallbackQuery):
    ...
    try:
        # Удаляем предыдущее опубликованное объявление пользователя (если есть)
        prev_ad = get_user_published_ad(callback.from_user.id)
        if prev_ad and prev_ad['published_message_ids']:
            for msg_id in prev_ad['published_message_ids']:
                try:
                    await bot.delete_message(chat_id=CHAT_ID, message_id=msg_id)
                except Exception as del_err:
                    logger.warning(f"Не удалось удалить старое объявление (msg_id={msg_id}): {del_err}")
        ...
        if ad['photos'] and len(ad['photos']) > 0:
            ...
            if len(ad['photos']) == 1:
                ...
                published_message_ids.append(rating_msg.message_id)
        else:
        ...
        # Обновляем статус объявления и сохраняем ID сообщений
        publish_ad(ad_id, published_message_ids)
...
DAILY_TOPIC_ID = 405760  # ID темы для ежедневной рассылки

DAILY_RULES_TEXT = """📜 Правила чата 📜

🚫 1. Нарушение правил отправки объявлений (НПОО)
   • Разрешено отправлять объявление только через бота
   • Запрещено писать в объявлении «Username» человека

   🔥 Нарушение: запрет на публикацию объявлений + мут на 1 - 2 дня (В зависимости от нарушения) по причине «НПОО»
...
⛔ 5. Конкуренция и перепродажа
   ...
   🔥 Нарушение: запрет на публикацию объявлений + мут на 7 дней
...
🔧 10. Технические нарушения
   • Запрещены попытки взлома или обхода ограничений
   • Запрещено создание нескольких аккаунтов для обхода наказаний
   • Запрещено писать в темы не по назначению
   🔥 Нарушение: варн / бан от 30 дней до перманентного

⚖️ 11. Политика и религия
...
💎 Администрация оставляет за собой право наказывать за другие нарушения, не указанные в правилах!"""
...
📊 Прайс: https://t.me/c/3879084610/3"""
...
async def send_daily_info():
    """Ежедневная отправка правил, информации о заказах и инструкции по жалобам в тему DAILY_TOPIC_ID в 12:00 по МСК."""
    while True:
        try:
            now = get_moscow_time()
            # Вычисляем время до следующего 12:00 МСК
            next_run = now.replace(hour=12, minute=0, second=0, microsecond=0)
            if now >= next_run:
                next_run += timedelta(days=1)
            seconds_until = (next_run - now).total_seconds()
            logger.info(f"Ежедневная рассылка: следующая отправка через {seconds_until:.0f} секунд ({next_run.strftime('%d.%m.%Y %H:%M')} МСК)")
            await asyncio.sleep(seconds_until)

            # Отправляем три сообщения в тему
            for text in [DAILY_RULES_TEXT, DAILY_ORDERS_TEXT, DAILY_COMPLAINTS_TEXT]:
                try:
                    await bot.send_message(
                        chat_id=CHAT_ID,
                        text=text,
                        message_thread_id=DAILY_TOPIC_ID,
                        disable_web_page_preview=True
                    )
                    await asyncio.sleep(1)  # небольшая пауза между сообщениями
                except Exception as e:
                    logger.error(f"Ошибка при отправке ежедневного сообщения: {e}")

            logger.info("Ежедневная рассылка успешно отправлена")
            # Пауза 60 секунд, чтобы не отправить дважды за одну минуту
            await asyncio.sleep(60)
        except Exception as e:
            logger.error(f"Ошибка в задаче ежедневной рассылки: {e}")
            await asyncio.sleep(300)
...
async def main():
    ...
    asyncio.create_task(send_daily_info())  # Ежедневная рассылка в 12:00 МСК
