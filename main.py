...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    """
    user_id = None
    reason = "Не указана"  # Значение по умолчанию
    
    # Сначала проверяем ответ на сообщение
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        # Причина - все аргументы команды после команды
        if command_args:
            reason = command_args
    ...
    # Затем проверяем аргументы команды
        ...
        # Разбиваем аргументы: первый - пользователь, остальное - причина
        parts = command_args.split(maxsplit=1)
        
        if len(parts) >= 1:
            user_part = parts[0]
            # Получаем ID пользователя
            user_id = await get_user_id_from_message(user_part)
...
async def resolve_user_only(message: Message, command_args: str = None) -> Optional[int]:
    ...
    # Затем проверяем аргументы команды
        ...
        # Берем ТОЛЬКО первый аргумент (разделяем по пробелу)
        args = command_args.strip().split()
        if args:
            return await get_user_id_from_message(args[0])
