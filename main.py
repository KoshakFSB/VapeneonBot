...
# Состояние набора администраторов
# is_open: bool — открыт ли набор
# limit: int | None — максимум заявок (None = без лимита)
# count: int — сколько заявок уже подано
admin_quest_state: Dict[str, Any] = {
    "is_open": False,
    "limit": None,
    "count": 0,
}
...
async def cmd_admin_quest(message: Message, command: CommandObject):
    """
    Управление набором администраторов.

    /admin_quest         — открыть набор без лимита
    /admin_quest 10      — открыть набор, максимум 10 заявок
    /admin_quest close   — закрыть набор вручную
    """
    if not await is_owner(message.from_user.id):
    ...
    # --- Закрытие набора ---
    if arg == "close":
        if not admin_quest_state["is_open"]:
            await message.answer("ℹ️ Набор администраторов и так закрыт.")
            return
        admin_quest_state["is_open"] = False
        count = admin_quest_state["count"]
        admin_quest_state["count"] = 0
        admin_quest_state["limit"] = None
        await message.answer(
            f"🔒 <b>Набор администраторов закрыт.</b>\n\n"
            f"Всего подано заявок: <b>{count}</b>",
            parse_mode="HTML"
        )
        return
    ...
    if arg:
        if not arg.isdigit() or int(arg) < 1:
            await message.answer("❌ Укажите корректное число заявок, например: <code>/admin_quest 10</code>", parse_mode="HTML")
            return
        limit = int(arg)

    admin_quest_state["is_open"] = True
    admin_quest_state["limit"] = limit
    admin_quest_state["count"] = 0

    limit_text = f"Принимается максимум <b>{limit}</b> заявок — набор закроется автоматически." if limit else "Лимит заявок не установлен."

    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="📝 Подать заявку", callback_data="admin_quest_apply")
    ]])

    await message.answer(
        f"📢 <b>Открыт набор администраторов!</b> 📢\n\n"
        f"Хотите стать частью команды модерации чата? Нажмите кнопку ниже, чтобы заполнить анкету.\n\n"
        f"ℹ️ {limit_text}\n\n"
        f"❗️ Заявка заполняется в личных сообщениях с ботом.",
        parse_mode="HTML",
        reply_markup=kb
    )


@dp.callback_query(F.data == "admin_quest_apply")
async def admin_quest_apply_start(callback: CallbackQuery, state: FSMContext):
    await callback.answer()

    # Проверяем, открыт ли набор
    if not admin_quest_state["is_open"]:
        await callback.answer("❌ Набор администраторов уже закрыт.", show_alert=True)
        return

    # Проверяем лимит
    limit = admin_quest_state["limit"]
    if limit is not None and admin_quest_state["count"] >= limit:
        await callback.answer("❌ Все места заняты, набор закрыт.", show_alert=True)
        return

    try:
        await bot.send_message(
            chat_id=callback.from_user.id,
            text="📝 <b>Анкета на должность администратора</b>\n\n"
        ...
        await callback.message.answer(
            f"⚠️ {callback.from_user.mention_html()}, не удалось написать вам в личные сообщения. "
            f"Откройте диалог с ботом (нажмите /start в личке) и повторите попытку.",
            parse_mode="HTML"
        )
...
@dp.message(AdminQuestStates.waiting_for_age)
...
@dp.message(AdminQuestStates.waiting_for_time)
async def admin_quest_process_time(message: Message, state: FSMContext):
    ...
    admin_quest_state["count"] += 1
    current_count = admin_quest_state["count"]
    limit = admin_quest_state["limit"]
    ...
    limit_info = f"{current_count}/{limit}" if limit else str(current_count)

    summary = (
        f"📋 <b>Новая заявка на администратора</b> [{limit_info}]\n\n"
        f"👤 Кандидат: {applicant_mention} (ID: <code>{applicant.id}</code>)\n\n"
        f"1️⃣ Имя: {data.get('quest_name')}\n"
        f"2️⃣ Возраст: {data.get('quest_age')}\n"
        f"3️⃣ Знание команд бота: {data.get('quest_time')}/10"
    )
    ...
    if limit is not None and current_count >= limit:
        ...
        for owner_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    chat_id=owner_id,
                    text=f"🔒 <b>Набор администраторов автоматически закрыт.</b>\n\n"
                         f"Достигнут лимит: <b>{limit}</b> заявок.",
                    parse_mode="HTML"
                )
            except Exception as _e:
                logger.error(f"Не удалось уведомить владельца {owner_id} о закрытии набора: {_e}")
