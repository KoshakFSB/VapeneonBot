...
async def broadcast_to_users(message_text: str, parse_mode: str = "HTML") -> tuple:
    """Рассылает сообщение всем пользователям, запустившим бота"""
    conn = sqlite3.connect("data/bot_database.db")
    cursor = conn.cursor()
...
async def cmd_broadcast(message: Message, command: CommandObject, state: FSMContext):
    """Команда для массовой рассылки — только для владельцев"""
    ...
    if not command.args:
        await message.answer(
            ...
            "⚠️ Сообщение будет отправлено <b>ВСЕМ</b> пользователям, запустившим бота.",
            parse_mode="HTML"
        )
        return
    ...
    confirm_keyboard.row(
        InlineKeyboardButton(text="✅ Да, отправить", callback_data=f"confirm_broadcast:{len(text)}"),
    ...
    await message.answer(
        ...
        f"Нажмите «Да, отправить» для запуска рассылки.",
        parse_mode="HTML",
...
async def confirm_broadcast(callback: CallbackQuery, state: FSMContext):
    ...
    sent, failed = await broadcast_to_users(text)
    
    await status_msg.edit_text(
        f"✅ <b>Рассылка завершена!</b>\n\n"
        f"📤 Отправлено: {sent}\n"
        f"❌ Ошибок: {failed}\n"
        f"📊 Всего пользователей: {sent + failed}",
        parse_mode="HTML"
    )
    await state.clear()
...
async def cmd_users_count(message: Message):
    """Показывает количество пользователей, запустивших бота"""
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ У вас нет прав для этой команды.")
        return
    ...
    # За последние 7 дней
    cursor.execute(
        "SELECT COUNT(*) FROM bot_users WHERE last_seen > datetime('now', '-7 days')"
    )
    active_week = cursor.fetchone()[0]
    
    # За последние 30 дней
    cursor.execute(
        "SELECT COUNT(*) FROM bot_users WHERE last_seen > datetime('now', '-30 days')"
    )
    ...
    await message.answer(
        f"📊 <b>Статистика пользователей бота</b>\n\n"
        f"👥 Всего запустили бота: <b>{total}</b>\n"
        f"🟢 Активны за 7 дней: <b>{active_week}</b>\n"
        f"🟡 Активны за 30 дней: <b>{active_month}</b>",
        parse_mode="HTML"
    )
...
async def safe_deal_create_redirect(callback: CallbackQuery, state: FSMContext):
    """Создание новой сделки — сначала показываем пользовательское соглашение"""
    await callback.answer()
...
async def safe_deal_create_confirmed(callback: CallbackQuery, state: FSMContext):
    """Пользователь принял соглашение — переходим к выбору роли"""
    await callback.answer()
