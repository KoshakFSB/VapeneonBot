...
async def cmd_report(message: Message, command: CommandObject):
    ...
    report_text = (
        f"🚨 <b>Новая жалоба из чата {message.chat.title}</b>\n\n"
    ...
    # Получаем список администраторов бота (из базы данных и владельцев)
    admin_ids = get_all_admins() + ADMIN_IDS
    
    # Отправляем жалобу всем администраторам в ЛС
    success_count = 0
    for admin_id in admin_ids:
        try:
            # Проверяем, что бот может писать администратору
            try:
                await bot.send_chat_action(admin_id, "typing")
            except:
                continue  # Пропускаем администраторов, которым бот не может писать
