...
async def cmd_clear_chat(message: Message):
    """Полная очистка чата (кроме сообщений от канала и закрепленных)"""
    if not await is_chat_admin(message.from_user.id, message.chat.id):
        clown_msg = await message.reply("🤡")
        await asyncio.sleep(2)
    ...
    try:
        deleted_count = 0
        skipped_count = 0
        max_messages = 1000  # Ограничение для безопасности
        batch_size = 100     # Размер пачки для удаления
        
        # Получаем ID последних сообщений
        last_messages = []
        current_message_id = message.message_id - 1  # Начинаем с сообщения перед командой
        
        while len(last_messages) < max_messages and current_message_id > 0:
            try:
                # Пытаемся получить информацию о сообщении
                chat_msg = await bot.forward_message(
                    chat_id=message.from_user.id,  # Пересылаем себе для проверки
                    from_chat_id=message.chat.id,
                    message_id=current_message_id
                )
                
                # Проверяем, нужно ли пропустить это сообщение
                skip_message = False
                
                # Пропускаем закрепленные сообщения
                try:
                    chat_info = await bot.get_chat(message.chat.id)
                    if chat_info.pinned_message and chat_info.pinned_message.message_id == current_message_id:
                        skip_message = True
                except:
                    pass
                
                # Пропускаем сообщения от каналов
                if chat_msg.sender_chat and chat_msg.sender_chat.type == ChatType.CHANNEL:
                    skip_message = True
                
                # Пропускаем сообщения от ботов
                if chat_msg.from_user and chat_msg.from_user.is_bot:
                    skip_message = True
                
                if not skip_message:
                    last_messages.append(current_message_id)
                else:
                    skipped_count += 1
                
                # Удаляем пересланное сообщение у себя
                await delete_message(message.from_user.id, chat_msg.message_id)
                
            except Exception as e:
                # Если сообщение не существует или недоступно, пропускаем
                skipped_count += 1
        ...
        for i in range(0, len(last_messages), batch_size):
            batch = last_messages[i:i + batch_size]
            
            for msg_id in batch:
                try:
                    success = await delete_message(message.chat.id, msg_id)
                    if success:
                        deleted_count += 1
                    else:
                        skipped_count += 1
        ...
        await confirm_msg.edit_text(
            ...
            f"<i>Пропущены сообщения от каналов, закрепленные сообщения и сообщения ботов</i>",
            parse_mode="HTML"
        )
