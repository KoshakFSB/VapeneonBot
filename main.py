...
SITE_URL = os.getenv("SITE_URL", "https://2nsf-it7o-587d.gw-1a.dockhost.net")
...
async def cmd_start_with_reviews(message: Message, command: CommandObject, state: FSMContext):
    ...
    if command.args == "admin_quest":
        register_bot_user(message.from_user)
        quest_state = get_admin_quest_state()
        if not quest_state["is_open"]:
            await message.answer(
                "😔 Набор администраторов сейчас закрыт. Следите за объявлениями в группе.",
                parse_mode="HTML"
            )
            return
        ...
        await state.clear()
        await message.answer(
            "📝 <b>Анкета на должность администратора</b>\n\n"
            "Вопрос 1/3: Как вас зовут?",
            parse_mode="HTML"
        )
        await state.set_state(AdminQuestStates.waiting_for_name)
        return
...
async def cmd_admin_quest(message: Message, command: CommandObject):
    """Управляет набором администраторов: открыть / открыть с лимитом / закрыть"""
    if not await is_owner(message.from_user.id):
        await message.answer("❌ Только владелец может управлять набором администраторов.")
        return
    ...
    # ── Закрытие набора ──
    if arg == "close":
        state = get_admin_quest_state()
        if not state["is_open"]:
            await message.answer("ℹ️ Набор администраторов и так закрыт.")
            return
        set_admin_quest_closed()
        try:
            await bot.send_message(
                CHAT_ID,
                "🔒 <b>Набор администраторов закрыт.</b>\n\n"
                "Спасибо всем, кто подал заявку! Новые заявки больше не принимаются.",
                parse_mode="HTML"
            )
            await message.answer("✅ Набор администраторов закрыт.")
        except Exception as e:
    ...
    if arg:
        if not arg.isdigit() or int(arg) <= 0:
            await message.answer(
                ...
                "Использование:\n"
                "<code>/admin_quest</code> — открыть набор без лимита\n"
                "<code>/admin_quest 10</code> — открыть набор с лимитом заявок\n"
                "<code>/admin_quest close</code> — закрыть набор",
                parse_mode="HTML"
            )
            return
        max_applications = int(arg)

    set_admin_quest_open(max_applications)

    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="📝 Подать заявку",
            url=f"https://t.me/{bot_username}?start=admin_quest"
        )
    ]])
    ...
    try:
        await bot.send_message(
            CHAT_ID,
            "📢 <b>Открыт набор администраторов!</b> 📢\n\n"
            "Хотите стать частью команды модерации чата? Нажмите кнопку ниже — она откроет чат с ботом, "
            "где нужно нажать «Старт» и ответить на 3 вопроса анкеты."
            f"{limit_note}",
            parse_mode="HTML",
            reply_markup=kb
        )
        await message.answer("✅ Объявление о наборе администраторов отправлено в группу.")
    except Exception as e:
        await message.answer(f"❌ Не удалось отправить сообщение в группу: {e}")


@dp.message(AdminQuestStates.waiting_for_name, F.chat.type == ChatType.PRIVATE, F.text)
async def admin_quest_process_name(message: Message, state: FSMContext):
    await state.update_data(quest_name=message.text.strip())
    await message.answer("Вопрос 2/3: Сколько вам лет?")
    await state.set_state(AdminQuestStates.waiting_for_age)


@dp.message(AdminQuestStates.waiting_for_name, F.chat.type == ChatType.PRIVATE)
async def admin_quest_process_name_invalid(message: Message):
    await message.answer("Пожалуйста, ответьте текстом. Вопрос 1/3: Как вас зовут?")


@dp.message(AdminQuestStates.waiting_for_age, F.chat.type == ChatType.PRIVATE, F.text)
async def admin_quest_process_age(message: Message, state: FSMContext):
    await state.update_data(quest_age=message.text.strip())
    await message.answer("Вопрос 3/3: Насколько хорошо вы знаете команды бота от 1 до 10?")
    await state.set_state(AdminQuestStates.waiting_for_time)


@dp.message(AdminQuestStates.waiting_for_age, F.chat.type == ChatType.PRIVATE)
async def admin_quest_process_age_invalid(message: Message):
    await message.answer("Пожалуйста, ответьте текстом. Вопрос 2/3: Сколько вам лет?")


@dp.message(AdminQuestStates.waiting_for_time, F.chat.type == ChatType.PRIVATE, F.text)
async def admin_quest_process_time(message: Message, state: FSMContext):
    data = await state.update_data(quest_time=message.text.strip())
    await state.clear()

    quest_state = get_admin_quest_state()
    if not quest_state["is_open"]:
        await message.answer("😔 Набор администраторов уже закрыт, заявка не принята. Следите за новыми объявлениями.")
        return
    max_apps = quest_state["max_applications"]
    if max_apps is not None and quest_state["applications_count"] >= max_apps:
        await message.answer("😔 Лимит заявок уже исчерпан, ваша заявка не принята. Следите за новыми объявлениями.")
        return

    applicant = message.from_user
    applicant_mention = (
        f"<a href='tg://user?id={applicant.id}'>{applicant.first_name}</a>"
        + (f" (@{applicant.username})" if applicant.username else "")
    )

    summary = (
        f"📋 <b>Новая заявка на администратора</b>\n\n"
        f"👤 Кандидат: {applicant_mention} (ID: <code>{applicant.id}</code>)\n\n"
        f"1️⃣ Имя: {data.get('quest_name')}\n"
        f"2️⃣ Возраст: {data.get('quest_age')}\n"
        f"3️⃣ Знание команд бота (1-10): {data.get('quest_time')}"
    )

    _sent_to_any_owner = False
    for owner_id in ADMIN_IDS:
        try:
            await bot.send_message(chat_id=owner_id, text=summary, parse_mode="HTML")
            _sent_to_any_owner = True
        except Exception as _e:
            logger.error(f"Не удалось отправить анкету владельцу {owner_id}: {_e}")

    if not _sent_to_any_owner:
        await message.answer("⚠️ Заявка заполнена, но не удалось доставить её владельцам. Свяжитесь с администрацией напрямую.")
        return

    new_count = increment_admin_quest_applications()
    await message.answer("✅ Спасибо! Ваша заявка отправлена на рассмотрение.")

    if max_apps is not None and new_count >= max_apps:
        set_admin_quest_closed()
        try:
            await bot.send_message(
                CHAT_ID,
                "🔒 <b>Набор администраторов закрыт — лимит заявок достигнут.</b>\n\n"
                "Спасибо всем, кто откликнулся!",
                parse_mode="HTML"
            )
        except Exception as _e:
            logger.error(f"Не удалось отправить сообщение о закрытии набора в группу: {_e}")


@dp.message(AdminQuestStates.waiting_for_time, F.chat.type == ChatType.PRIVATE)
async def admin_quest_process_time_invalid(message: Message):
    await message.answer("Пожалуйста, ответьте текстом. Вопрос 3/3: Насколько хорошо вы знаете команды бота от 1 до 10?")
...
async def handle_message(message: Message):
    ...
    if text.startswith('/'):
        ...
        bot_commands = [
            ...
            '/ban', '/tban', '/unban', '/cc', '/admin_add', '/admin_remove', '/admin_quest',
