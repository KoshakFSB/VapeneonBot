...
async def cmd_warn(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        ...
        if not args:
            await message.answer("❌ Укажите пользователя: /warn @username [причина]")
            return
...
async def cmd_mute(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        ...
        if not args:
            await message.answer("❌ Укажите пользователя: /mute @username [причина]")
            return
        parts = args.split(maxsplit=1)
        user_identifier = parts[0]
        reason = parts[1] if len(parts) > 1 else "Не указана"
...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        ...
        if args:
            ...
            await message.answer("❌ Укажите время: /tmute [время] [причина]")
            return
    else:
        if not args:
...
async def cmd_tban(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        ...
        if args:
            ...
            await message.answer("❌ Укажите время: /tban [время] [причина]")
            return
    else:
        if not args:
