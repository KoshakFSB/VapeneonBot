...
def add_product_to_db(category: str, name: str, description: str, photo_file_id: str, added_by: int) -> int:
    """Добавляет товар в БД и возвращает его ID"""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
...
async def adm_product_photo(message: Message, state: FSMContext):
    """Получить фото и сохранить товар"""
    data = await state.get_data()
