...
async def send_rules_and_order_message():
    ...
    try:
        # Проверяем текущее время - не отправляем с 00:00 до 06:00
        ...
        if night_start <= current_time <= night_end:
            logger.info("Ночное время (00:00-06:00) - пропускаем отправку правил")
            return
...
async def rules_scheduler():
    """Планировщик для отправки правил каждые 2.5 часа (кроме ночного времени 00:00-06:00)"""
    # Ждем немного перед первым запуском, чтобы избежать отправки в ночное время
...
async def handle_message(message: Message):
    ...
    if is_ad_message(text):
        ...
        # Если проверка пройдена, добавляем объявление в базу
...
async def main():
    ...
    # Проверяем время при запуске - не отправляем правила если ночное время
    current_time = datetime.now().time()
    night_start = datetime.strptime("00:00", "%H:%M").time()
    night_end = datetime.strptime("06:00", "%H:%M").time()
    
    if not (night_start <= current_time <= night_end):
        logger.info("Дневное время - отправляем правила при запуске")
        await send_rules_and_order_message()
    else:
        logger.info("Ночное время - пропускаем отправку правил при запуске")
