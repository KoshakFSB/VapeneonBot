...
def set_admin_quest_open(max_applications: Optional[int] = None):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE admin_quest_state SET is_open = 1, max_applications = %s, applications_count = 0 WHERE id = 1",
        (max_applications,)
    )
    conn.commit()
    conn.close()
...
def set_admin_quest_closed():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE admin_quest_state SET is_open = 0 WHERE id = 1")
    conn.commit()
    conn.close()
