...
async def cmd_admin_add(message: Message, command: CommandObject):
    ...
    try:
        ...
        # Если ЛС не дошло — отправляем логин/пароль владельцу
        if not _dm_sent:
            _site_info = (
                f"\n\n⚠️ <b>Не удалось отправить данные в ЛС пользователю</b> (бот не начат).\n"
                f"Передайте вручную:\n"
                f"🌐 {SITE_URL}\n"
                f"👤 Логин: <code>{_site_username}</code>\n"
                f"🔐 Пароль: <code>{_site_password}</code>"
            ) if _site_registered else (
                f"\n\n⚠️ <b>Не удалось отправить ЛС и зарегистрировать на сайте.</b>\n"
                f"Попробуйте /addjbadm позже или добавьте вручную."
            )
            await message.answer(
                f"✅ {user_mention} назначен, но данные для сайта не удалось отправить в ЛС.{_site_info}",
                parse_mode="HTML"
            )
