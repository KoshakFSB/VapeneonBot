...
SITE_URL = os.getenv("SITE_URL", "https://vapeneon.online")
ADMIN_IDS = list(map(int, os.getenv("ADMIN_IDS").split(','))) if os.getenv("ADMIN_IDS") else []
CHAT_ID = int(os.getenv("CHAT_ID"))

# Счётчик сообщений в основном чате — нужен для соблюдения паузы
# между автопостами объявлений (не менее 30 сообщений между постами).
_autopost_msg_counter = 0
...
def format_moscow_time(db_time_str):
    """Конвертирует время из БД в московское время и форматирует.
    Принимает и строку (как отдавал sqlite3), и datetime (как отдаёт psycopg2 —
    для колонок TIMESTAMP значения приходят уже готовыми объектами datetime,
    без необходимости парсить их из строки)."""
    if not db_time_str:
        return "не указано"
    
    try:
        if isinstance(db_time_str, datetime):
            utc_time = db_time_str
        else:
...
# Функции для работы с базой данных (должны быть определены ДО init_db(),
# так как init_db() вызывается на верхнем уровне модуля при импорте —
# порядок объявлений в файле здесь важен)
...
class HybridRow(dict):
    """Строка результата запроса, ведущая себя как sqlite3.Row:
    поддерживает и доступ по имени колонки (row["username"]), и по числовому
    индексу (row[0]), и dict(row). Обычный psycopg2.extras.RealDictRow
    поддерживает только доступ по имени — этого гибрида не хватало бы
    для кода, написанного изначально под sqlite3.Row.

    ВАЖНО: обычный dict при переборе (for x in row / for a, b in row)
    отдаёт КЛЮЧИ. sqlite3.Row вместо этого отдаёт ЗНАЧЕНИЯ по порядку
    колонок (как обычный tuple) — именно на это рассчитан весь код вида
    `for user_id, chat_id in cursor.fetchall():`. Поэтому __iter__ здесь
    обязательно переопределён — без этого такой код молча получает вместо
    значений имена столбцов ("user_id" вместо, например, 123456789)."""
...
class HybridCursor(psycopg2.extensions.cursor):
    """Обычный psycopg2-курсор (строки — plain tuples), но fetch-методы
    оборачивают каждую строку в HybridRow, используя cursor.description
    для имён колонок. Не зависит от приватных методов RealDictCursor."""

    def _wrap(self, raw_row):
        if raw_row is None:
            return None
        cols = [d[0] for d in self.description]
        return HybridRow(zip(cols, raw_row))
    ...
    def __iter__(self):
        for raw_row in super().__iter__():
            yield self._wrap(raw_row)


class _PooledConn:
    """Обёртка над соединением из пула psycopg2: даёт тот же интерфейс,
    которым пользовался весь остальной код бота при работе со sqlite3
    (conn.cursor(), conn.execute(), conn.commit(), conn.close()),
    но close() возвращает соединение в пул вместо разрыва TCP-сессии."""

    def __init__(self, raw):
        self._raw = raw
        raw.cursor_factory = HybridCursor
    ...
    def close(self):
        _get_pg_pool().putconn(self._raw)


def get_db_connection():
    """Возвращает соединение с базой данных (из пула PostgreSQL)."""
...
def init_db():
    ...
    cursor.execute("ALTER TABLE admins ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'moderator'")
    cursor.execute("ALTER TABLE admins ADD COLUMN IF NOT EXISTS display_name TEXT")
    ...
    # Таблица для нескольких фото товара
    cursor.execute('''
    ...
    # Таблица состояния набора администраторов
    cursor.execute(
    ...
    cursor.execute(
        "INSERT INTO admin_quest_state (id, is_open, max_applications, applications_count) "
        "VALUES (1, FALSE, NULL, 0) ON CONFLICT (id) DO NOTHING"
    )

    # Таблица состояния наборов волонтёров/администраторов (заявки теперь подаются на сайте)
    cursor.execute(
    ...
    cursor.execute(
        ...
        "VALUES ('volunteer', FALSE, NULL, 0), ('admin', FALSE, NULL, 0) ON CONFLICT (role) DO NOTHING"
    )

    # Очередь объявлений о новых публичных комнатах «Просмотр фильмов» — сайт (web.py)
    # кладёт сюда строку при создании комнаты, а бот сам забирает и публикует в чат.
    cursor.execute(
    ...
    # ─── ПЛАТНЫЕ УСЛУГИ: закрепление объявлений ───
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS paid_services_orders (
        id SERIAL PRIMARY KEY,
    ...
    cursor.execute(
        ...
        id SERIAL PRIMARY KEY,
        order_id INTEGER REFERENCES paid_services_orders(id),
        user_id BIGINT NOT NULL,
    ...
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS autopost_subscriptions (
    ...
    cursor.execute(
        ...
        last_post_counter INTEGER DEFAULT 0,
    ...
    # ─── ПЛАТНАЯ УСЛУГА: «УОО» — урезанные ограничения на отправку объявлений ───
    # Продаётся на сайте (web.py). Пока активна — участнику выдаётся тег «УОО»
    # рядом с именем в чате (Bot API setChatMemberTag, без выдачи прав админа),
    # по истечении срока тег снимается автоматически (см. uoo_expiry_checker).
    cursor.execute(
        """CREATE TABLE IF NOT EXISTS uoo_subscriptions (
        user_id BIGINT PRIMARY KEY,
        username TEXT,
        expires_at TIMESTAMP NOT NULL,
        active BOOLEAN DEFAULT TRUE,
...
def find_bot_user_by_username(username: str) -> Optional[dict]:
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM bot_users WHERE username = %s",
...
def get_safe_deal(deal_id: str) -> Optional[dict]:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM safe_deals WHERE id = %s", (deal_id,))
    row = cursor.fetchone()
    conn.close()
    return dict(row) if row else None
...
def update_safe_deal_status(deal_id: str, status: str):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE safe_deals SET status = %s WHERE id = %s", (status, deal_id))
    conn.commit()
    conn.close()
...
def set_user_safe_confirmed(deal_id: str, user_type: str):
    ...
    cursor = conn.cursor()
    if user_type == 'buyer':
        cursor.execute("UPDATE safe_deals SET buyer_confirmed = TRUE WHERE id = %s", (deal_id,))
    else:
        cursor.execute("UPDATE safe_deals SET seller_confirmed = TRUE WHERE id = %s", (deal_id,))
    conn.commit()
    conn.close()
...
async def restore_active_punishments():
    ...
    try:
        ...
        cursor.execute("""
            ...
            FROM mutes 
            WHERE is_active = TRUE AND (expires_at IS NULL OR expires_at > %s)
        """, (current_time,))
        
        active_mutes = cursor.fetchall()
        restored_count = 0
        
        for row in active_mutes:
            user_id, chat_id, expires_at, reason = row["user_id"], row["chat_id"], row["expires_at"], row["reason"]
            try:
                ...
                if member.status in [ChatMemberStatus.ADMINISTRATOR, ChatMemberStatus.CREATOR]:
                    ...
                    cursor.execute(
                        "UPDATE mutes SET is_active = FALSE WHERE user_id = %s AND chat_id = %s",
                        (user_id, chat_id)
                    )
                    logger.info(f"Мут администратора {user_id} деактивирован")
                    continue
                
                # expires_at из Postgres уже приходит как datetime (или None),
                # а не строкой, как было в sqlite — парсить не нужно
        ...
        for row in active_bans:
            ...
            try:
                ...
                cursor.execute(
                    "UPDATE bans SET is_active = FALSE WHERE user_id = %s AND chat_id = %s",
                    (user_id, chat_id)
                )
        ...
        # Соединение обязательно возвращается в пул при любом исходе —
        # иначе при повторяющихся ошибках пул постепенно пустеет
        if conn is not None:
            conn.close()
    ...
    while True:
        ...
        try:
            ...
            for row in expired_mutes:
                user_id, chat_id, mute_id = row["user_id"], row["chat_id"], row["id"]
                try:
                    # Снимаем мут
                    permissions = ChatPermissions(
                    ...
                    # Деактивируем в БД
                    cursor.execute(
                        "UPDATE mutes SET is_active = FALSE WHERE id = %s",
                        (mute_id,)
                    )
            ...
            for row in expired_bans:
                user_id, chat_id, ban_id = row["user_id"], row["chat_id"], row["id"]
                try:
                    # Разбаниваем
                    ...
                    # Деактивируем в БД
                    cursor.execute(
                        "UPDATE bans SET is_active = FALSE WHERE id = %s",
                        (ban_id,)
                    )
            ...
            # Соединение обязательно возвращается в пул при любом исходе —
            # именно отсутствие этого раньше приводило к постепенному
            # исчерпанию пула (после чего требовался перезапуск бота)
            if conn is not None:
                conn.close()
...
def add_warn(user_id: int, chat_id: int, reason: str, issued_by: int):
    ...
    cursor.execute(
        "INSERT INTO warns (user_id, chat_id, reason, issued_by, expires_at) VALUES (%s, %s, %s, %s, %s)",
...
def get_user_warns(user_id: int, chat_id: int) -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, reason, issued_at, expires_at FROM warns WHERE user_id = %s AND chat_id = %s AND expires_at > %s",
...
def clear_user_warns(user_id: int, chat_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "DELETE FROM warns WHERE user_id = %s AND chat_id = %s", (user_id, chat_id)
    )
    conn.commit()
    conn.close()
...
def add_mute(user_id: int, chat_id: int, reason: str, issued_by: int, duration: timedelta = None):
    ...
    cursor.execute(
        "INSERT INTO mutes (user_id, chat_id, reason, issued_by, expires_at, is_active) VALUES (%s, %s, %s, %s, %s, TRUE)",
        (user_id, chat_id, reason, issued_by, expires_at),
    )
    conn.commit()
    conn.close()
...
def add_ban(user_id: int, chat_id: int, reason: str, issued_by: int, duration: timedelta = None):
    ...
    expires_at = datetime.now() + duration if duration else None
    cursor.execute(
        "INSERT INTO bans (user_id, chat_id, reason, issued_by, expires_at, is_active) VALUES (%s, %s, %s, %s, %s, TRUE)",
        (user_id, chat_id, reason, issued_by, expires_at),
    )
    conn.commit()
    conn.close()
...
def add_admin_warn(user_id: int, reason: str, issued_by: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO admin_warns (user_id, reason, issued_by) VALUES (%s, %s, %s)",
...
def get_admin_warns(user_id: int) -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, reason, issued_at, issued_by FROM admin_warns WHERE user_id = %s AND is_active = TRUE",
        (user_id,),
    )
    warns = [
...
def remove_admin_warn(warn_id: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE admin_warns SET is_active = FALSE WHERE id = %s", (warn_id,))
    conn.commit()
    conn.close()
...
def remove_last_admin_warn(user_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM admin_warns WHERE user_id = %s AND is_active = TRUE ORDER BY id DESC LIMIT 1",
        (user_id,)
    )
    result = cursor.fetchone()
    if result:
        cursor.execute("UPDATE admin_warns SET is_active = FALSE WHERE id = %s", (result[0],))
    conn.commit()
    conn.close()
...
def clear_admin_warns(user_id: int):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE admin_warns SET is_active = FALSE WHERE user_id = %s", (user_id,))
    conn.commit()
    conn.close()
...
def add_bot_warn(user_id: int, reason: str, issued_by: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO bot_warns (user_id, reason, issued_by) VALUES (%s, %s, %s)",
        (user_id, reason, issued_by),
    )
    conn.commit()
    conn.close()
...
def get_bot_warns(user_id: int) -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, reason, issued_at, issued_by FROM bot_warns WHERE user_id = %s AND is_active = TRUE",
        (user_id,),
    )
    warns = [
        {"id": row[0], "reason": row[1], "issued_at": row[2], "issued_by": row[3]}
...
def remove_bot_warn(warn_id: int):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE bot_warns SET is_active = FALSE WHERE id = %s", (warn_id,))
    conn.commit()
    conn.close()
...
def clear_bot_warns(user_id: int):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE bot_warns SET is_active = FALSE WHERE user_id = %s", (user_id,))
    conn.commit()
    conn.close()
...
ADMIN_ROLES = {
    ...
    "orders": {
        ...
        "can_ban": False,
        "can_adblock": False,
        # Роль только для выдачи заказов — модераторских прав в Telegram не требуется
        "tg_rights": {
            "can_manage_chat": False,
...
def add_admin(user_id: int, added_by: int, role: str = "moderator", display_name: str = None):
    ...
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO admins (user_id, added_by, role, display_name) VALUES (%s, %s, %s, %s)
           ON CONFLICT (user_id) DO UPDATE SET
               added_by=excluded.added_by, role=excluded.role, display_name=excluded.display_name""",
...
def get_admin_info(user_id: int) -> Optional[Dict[str, Any]]:
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT user_id, role, display_name, added_at FROM admins WHERE user_id = %s", (user_id,))
    result = cursor.fetchone()
    conn.close()
    if result:
...
def get_admin_quest_state() -> Dict[str, Any]:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT is_open, max_applications, applications_count FROM admin_quest_state WHERE id = 1")
    row = cursor.fetchone()
    conn.close()
    if not row:
        return {"is_open": False, "max_applications": None, "applications_count": 0}
    return {"is_open": bool(row[0]), "max_applications": row[1], "applications_count": row[2]}

def set_admin_quest_open(max_applications: Optional[int] = None):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE admin_quest_state SET is_open = TRUE, max_applications = %s, applications_count = 0 WHERE id = 1",
        (max_applications,)
    )
    conn.commit()
    conn.close()

def set_admin_quest_closed():
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE admin_quest_state SET is_open = FALSE WHERE id = 1")
    conn.commit()
    conn.close()

# ── Наборы волонтёров/администраторов (заявки заполняются на сайте) ──

def get_role_quest_state(role: str) -> Dict[str, Any]:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT is_open, max_applications, applications_count FROM role_quest_state WHERE role = %s",
        (role,)
    )
    row = cursor.fetchone()
    conn.close()
...
def set_role_quest_open(role: str, max_applications: Optional[int] = None):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE role_quest_state SET is_open = TRUE, max_applications = %s, applications_count = 0 WHERE role = %s",
        (max_applications, role)
    )
    conn.commit()
    conn.close()

def set_role_quest_closed(role: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE role_quest_state SET is_open = FALSE WHERE role = %s", (role,))
    conn.commit()
    conn.close()

def increment_admin_quest_applications() -> int:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE admin_quest_state SET applications_count = applications_count + 1 WHERE id = 1")
    conn.commit()
    cursor.execute("SELECT applications_count FROM admin_quest_state WHERE id = 1")
    row = cursor.fetchone()
    conn.close()
...
def add_user_ad(user_id: int, message_text: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO user_ads (user_id, message_text) VALUES (%s, %s)",
...
def get_today_ads_count(user_id: int) -> int:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(*) AS c FROM user_ads WHERE user_id = %s AND DATE(sent_at) = CURRENT_DATE",
        (user_id,),
    )
...
def get_last_ad_time(user_id: int) -> Optional[datetime]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT sent_at FROM user_ads WHERE user_id = %s ORDER BY sent_at DESC LIMIT 1",
        (user_id,),
    )
    result = cursor.fetchone()
    conn.close()
...
def add_ad_violation(user_id: int):
    ...
    cursor.execute(
        "SELECT id, violation_count FROM ad_limit_violations WHERE user_id = %s AND violation_date = %s",
        (user_id, today)
    )
    result = cursor.fetchone()
    ...
    if result:
        ...
        cursor.execute(
            "UPDATE ad_limit_violations SET violation_count = %s WHERE id = %s",
            (count + 1, violation_id)
        )
    else:
        # Создаем новую запись
        cursor.execute(
            "INSERT INTO ad_limit_violations (user_id, violation_date) VALUES (%s, %s)",
            (user_id, today)
        )
...
def get_active_uoo(user_id: int) -> Optional[Dict[str, Any]]:
    """Возвращает данные активной подписки «УОО» (урезанные ограничения на
    объявления), если она сейчас действует, иначе None. Тегом «УОО» в чате
    и его снятием управляет сайт (web.py) — см. uoo_expiry_checker там же."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_id, username, expires_at FROM uoo_subscriptions "
        "WHERE user_id = %s AND active = TRUE AND expires_at > %s",
...
def get_today_violations_count(user_id: int) -> int:
    ...
    cursor.execute(
        "SELECT violation_count FROM ad_limit_violations WHERE user_id = %s AND violation_date = %s",
        (user_id, today)
    )
    result = cursor.fetchone()
    conn.close()
...
def update_complaint_status(complaint_id: int, status: str, handled_by: int = None, handling_result: str = None):
    ...
    cursor.execute(
        """UPDATE admin_complaints 
           SET status = %s, handled_by = %s, handling_result = %s, handled_at = CURRENT_TIMESTAMP
           WHERE id = %s""",
...
def has_accepted_tos(user_id: int) -> bool:
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT user_id FROM tos_accepted WHERE user_id = %s", (user_id,))
        result = cursor.fetchone()
        conn.close()
        return result is not None
    except Exception as e:
...
def save_tos_acceptance(user_id: int):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO tos_accepted (user_id, accepted_at) VALUES (%s, %s) ON CONFLICT (user_id) DO NOTHING",
            (user_id, datetime.now())
        )
        conn.commit()
        conn.close()
...
class ProductStates(StatesGroup):
    """Состояния для добавления товара через админ-панель"""
...
def is_user_blocked(user_id: int) -> bool:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM bot_blocks WHERE user_id = %s AND is_active = TRUE",
        (user_id,)
    )
    result = cursor.fetchone()
    conn.close()
...
def block_user(user_id: int, reason: str, blocked_by: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        """INSERT INTO bot_blocks (user_id, reason, blocked_by) VALUES (%s, %s, %s)
           ON CONFLICT (user_id) DO UPDATE SET
               reason=excluded.reason, blocked_by=excluded.blocked_by,
               blocked_at=CURRENT_TIMESTAMP, is_active=TRUE""",
...
def unblock_user(user_id: int):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE bot_blocks SET is_active = FALSE WHERE user_id = %s",
        (user_id,)
    )
    conn.commit()
    conn.close()
...
def add_user_review(from_user_id: int, to_user_id: int, rating: int, review_text: str) -> int:
    ...
    cursor.execute(
        "INSERT INTO user_reviews (from_user_id, to_user_id, rating, review_text) VALUES (%s, %s, %s, %s) RETURNING id",
...
def get_user_rating_stats(user_id: int) -> Tuple[float, int]:
    ...
    cursor.execute(
        "SELECT AVG(rating), COUNT(*) FROM user_reviews WHERE to_user_id = %s",
        (user_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
def get_user_review_from_user(from_user_id: int, to_user_id: int) -> Optional[Dict[str, Any]]:
    ...
    cursor.execute(
        "SELECT id, rating, review_text, created_at FROM user_reviews WHERE from_user_id = %s AND to_user_id = %s",
...
async def mute_user(chat_id: int, user_id: int, duration: timedelta = None, reason: str = None, is_auto: bool = False, message_thread_id: int = None) -> bool:
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE mutes SET is_active = FALSE WHERE user_id = %s AND chat_id = %s AND is_active = TRUE",
            (user_id, chat_id)
        )
        conn.commit()
        conn.close()
        ...
        if is_auto:
            message_text = (
                ...
                f"💎 Не хотите ждать окончания срока? На сайте можно приобрести досрочное снятие наказания."
            )
        else:
            message_text = (
...
async def ban_user(chat_id: int, user_id: int, duration: timedelta = None, reason: str = None, message_thread_id: int = None) -> bool:
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE bans SET is_active = FALSE WHERE user_id = %s AND chat_id = %s AND is_active = TRUE",
            (user_id, chat_id)
        )
        conn.commit()
        conn.close()
...
TERMS_OF_SERVICE_TEXT = (
    ...
    "━━━━━━━━━━━━━━━━━━━━━━\n"
    "3.1. За использование сервиса безопасных сделок взимается комиссия в размере <b>5% от суммы "
...
def has_active_autopost_subscription(user_id: int) -> bool:
    """Проверяет, есть ли у пользователя действующая подписка на автопостинг."""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
            "SELECT 1 FROM autopost_subscriptions WHERE user_id = %s AND active = TRUE AND expires_at > %s",
            (user_id, get_moscow_time())
        )
        result = cursor.fetchone()
        conn.close()
        return result is not None
    except Exception as e:
        logger.error(f"Ошибка проверки подписки на автопостинг: {e}")
        return False


def get_main_keyboard(user_id: int = None):
    """Стандартная Reply-клавиатура для ЛС с ботом.
    Кнопка «Просмотреть жалобы» и «Админ-панель» показываются только администраторам.
    """
    ...
    if user_id is not None and has_active_autopost_subscription(user_id):
        builder.row(KeyboardButton(text="🚀 Настройки автопостинга"))
...
@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "🔐 Открыть систему безопасных сделок")
async def btn_open_safe_deal(message: Message):
    ...
    await message.answer(
        ...
        "• Комиссия всего <b>5%</b> от суммы сделки\n\n"
        "Выберите действие 👇",
        parse_mode="HTML",
...
async def safe_deal_about(callback: CallbackQuery):
    ...
    about_text = (
        ...
        "💼 <b>Комиссия сервиса: 5%</b> от суммы сделки\n\n"
...
async def safe_deal_balance_redirect(callback: CallbackQuery):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT balance FROM safe_deal_balances WHERE user_id = %s", (user_id,))
    row = cursor.fetchone()
    conn.close()
...
async def sd_view_invite(callback: CallbackQuery):
    ...
    text = (
        ...
        f"💼 <b>Комиссия гаранта (5%):</b> {guarantor_fee:.2f} руб.\n"
...
async def sd_accept_deal(callback: CallbackQuery):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT buyer_confirmed, seller_confirmed FROM safe_deals WHERE id = %s", (deal_id,))
    row = cursor.fetchone()
    conn.close()
...
async def _show_safe_deal_details(callback: CallbackQuery, deal_id: str):
    ...
    text = (
        ...
        f"💰 <b>Сумма:</b> {deal.get('amount')} руб.\n"
        f"💼 <b>Комиссия (5%):</b> {guarantor_fee:.2f} руб.\n"
        f"💵 <b>Итого к оплате:</b> {total_amount:.2f} руб.\n"
        f"📝 <b>Описание:</b> {deal.get('description')}\n"
        f"⏰ <b>Срок:</b> {deal.get('deadline_days')} дней\n"
        f"👤 <b>Покупатель:</b> @{deal.get('buyer_username')}\n"
...
async def sd_initiate_payment(callback: CallbackQuery):
    ...
    try:
        ...
        payment_params = {
            ...
            "need-phone": "false",
            "need-address": "false",
            "paymentType": "AC"   # AC = банковская карта; PC для кошелька ЮMoney
        }
        # ВАЖНО: "shop" требует верифицированного магазина — label не сохраняется.
        # Для обычного кошелька используем "button".
        ...
        cursor = conn.cursor()
        cursor.execute("UPDATE safe_deals SET payment_url = %s WHERE id = %s", (payment_url, deal_id))
        conn.commit()
        conn.close()
...
async def sd_check_payment(callback: CallbackQuery):
    ...
    try:
        ...
        async with _aiohttp.ClientSession() as session:
            ...
            # Запрашиваем последние операции без серверного фильтра по label —
            # ЮMoney фильтрует label надёжно только для верифицированных магазинов.
            # Фильтрацию делаем сами ниже.
            params = {
                "records": 50,
                "details": "true"
            }
            async with session.post(f"{YOO_BASE_URL}/operation-history", headers=headers, data=params) as resp:
                if resp.status == 200:
                    ...
                    logger.info(
                        f"[Deal {deal_id}] ЮMoney вернул {len(operations)} операций. "
                        f"Ищем label=deal_{deal_id}, direction=in, status=success"
                    )
                    for op in operations:
                        logger.info(
                            f"[Deal {deal_id}] Операция: id={op.get('operation_id')} "
                            f"label={op.get('label')!r} direction={op.get('direction')} "
                            f"status={op.get('status')} amount={op.get('amount')}"
                        )
                        if (op.get("label") == f"deal_{deal_id}" and
                                op.get("status") == "success" and
                                op.get("direction") == "in"):
                            # Label однозначно идентифицирует платёж — проверка суммы не нужна.
                            # ЮMoney при оплате картой (AC) удерживает ~3% своей комиссии,
                            # поэтому на кошелёк поступает меньше, чем заплатил покупатель.
                            op_amount = float(op.get("amount", 0))
                            expected = float(deal.get("total_amount") or 0)
                            logger.info(
                                f"[Deal {deal_id}] Платёж найден: "
                                f"получено={op_amount}, ожидалось={expected:.2f}"
                            )
    ...
    if payment_confirmed:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE safe_deals SET payment_confirmed = TRUE, status = 'payment_received' WHERE id = %s",
            (deal_id,)
        )
        conn.commit()
        conn.close()
...
async def sd_confirm_receipt(callback: CallbackQuery):
    ...
    cursor.execute("UPDATE safe_deals SET status = 'completed' WHERE id = %s", (deal_id,))
    conn.commit()
    conn.close()
...
async def cmd_link_deal(message: Message, command: CommandObject):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id FROM safe_deals WHERE group_chat_id = %s AND id != %s",
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE safe_deals SET group_chat_id = %s WHERE id = %s",
                   (message.chat.id, deal_id))
    conn.commit()
    conn.close()
...
async def sd_process_bank(callback: CallbackQuery, state: FSMContext):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE safe_deal_balances SET balance = balance - %s WHERE user_id = %s",
        (amount, user_id)
    )
    cursor.execute("""
...
async def wd_mark_done(callback: CallbackQuery):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE safe_deal_withdrawals SET status = 'completed' WHERE id = %s",
        (withdrawal_id,)
    )
    conn.commit()
    conn.close()
...
async def wd_mark_rejected(callback: CallbackQuery):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE safe_deal_withdrawals SET status = 'rejected' WHERE id = %s",
        (withdrawal_id,)
    )
    cursor.execute(
        "UPDATE safe_deal_balances SET balance = balance + %s WHERE user_id = %s",
...
async def cmd_start_with_reviews(message: Message, command: CommandObject, state: FSMContext):
    ...
    if is_user_blocked(message.from_user.id):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT reason, blocked_at FROM bot_blocks WHERE user_id = %s AND is_active = TRUE ORDER BY blocked_at DESC LIMIT 1",
    ...
    # ── Авторизация с сайта VapeNeon ──
    if command.args and command.args.startswith("webauth_"):
        user = message.from_user
        token = command.args[len("webauth_"):]

        if not user.username:
            await message.answer(
                "⚠️ <b>Для входа на сайт нужен username в Telegram</b>\n\n"
                "Как установить username:\n"
                "1️⃣ Откройте <b>Настройки</b> Telegram\n"
                "2️⃣ Нажмите на своё имя / аватар\n"
                "3️⃣ Нажмите <b>Изменить профиль</b>\n"
                "4️⃣ В поле <b>Имя пользователя</b> введите username\n"
                "5️⃣ Сохраните и вернитесь на сайт, нажмите «Войти» снова\n\n"
                "🔗 <a href='https://okbob.app/blog/telegram-set-username'>Подробная инструкция</a>",
                parse_mode="HTML",
        ...
        try:
            ...
            logger.error(f"Ошибка подтверждения токена: {e}")

        await message.answer(
            f"✅ <b>Вы успешно вошли на сайт VapeNeon!</b>\n\n"
            f"👤 Аккаунт: @{user.username}\n\n"
            f"Вернитесь на сайт — вы авторизованы.\n"
            f"В разделе <b>«Мои жалобы»</b> отображаются все ваши жалобы.",
            parse_mode="HTML"
        )
        return

    # ── Привязка 2FA к профилю на сайте VapeNeon ──
    if command.args and command.args.startswith("tfaauth_"):
        user = message.from_user
        token = command.args[len("tfaauth_"):]

        if not user.username:
            await message.answer(
                "⚠️ <b>Для привязки 2FA нужен username в Telegram</b>\n\n"
                "Установите username в настройках Telegram и повторите привязку на сайте.",
                parse_mode="HTML"
            )
            return
        ...
        try:
            async with httpx.AsyncClient(timeout=5) as client:
                await client.post(f"{SITE_URL}/api/profile/tfa/confirm", json={
                    "token": token,
                    "username": user.username,
                    "tg_id": user.id,
                    "secret": BOT_TOKEN,
            ...
            logger.error(f"Ошибка подтверждения 2FA-токена: {e}")

        await message.answer(
            f"✅ <b>2FA привязана к вашему профилю на сайте VapeNeon!</b>\n\n"
            f"👤 Аккаунт: @{user.username}\n\n"
            f"Теперь при каждом входе и при смене IP сайт будет присылать код подтверждения сюда, в этот чат.",
            parse_mode="HTML"
        )
        return

    if command.args in ("admin_quest_info", "volunteer_quest_info"):
        register_bot_user(message.from_user)
        role = "admin" if command.args == "admin_quest_info" else "volunteer"
        # (набор_склонение_кого, вкладка_на_сайте)
        role_texts = {
            "admin":     {"genitive": "администраторов", "tab": "«Заявка на роль администратора»"},
            "volunteer": {"genitive": "волонтёров",       "tab": "«Заявка на роль волонтёра»"},
        }
        rt = role_texts[role]
        quest_state = get_role_quest_state(role)
        if not quest_state["is_open"]:
            await message.answer(
                f"😔 Набор {rt['genitive']} сейчас закрыт. Следите за объявлениями в группе.",
                parse_mode="HTML"
            )
            return
        max_apps = quest_state["max_applications"]
        if max_apps is not None and quest_state["applications_count"] >= max_apps:
            await message.answer(
                "😔 К сожалению, лимит заявок на набор уже исчерпан. Следите за новыми объявлениями.",
                parse_mode="HTML"
            )
            return
        await message.answer(
            f"📝 <b>Набор {rt['genitive']} открыт!</b>\n\n"
            f"Анкету нужно заполнить на сайте:\n"
            f"🌐 {SITE_URL}\n\n"
            f"1️⃣ Войдите на сайте через Telegram (кнопка «Авторизация»).\n"
            f"2️⃣ Откройте вкладку {rt['tab']}.\n"
            f"3️⃣ Заполните анкету и отправьте — она сразу попадёт на рассмотрение.",
            parse_mode="HTML"
        )
        return
    ...
    if command.args and command.args.startswith("appeal_"):
        await message.answer(
            ...
            f"Для подачи жалобы перейдите на официальный сайт:\n"
            f"🌐 {SITE_URL}\n\n"
            f"Авторизуйтесь и заполните форму жалобы.",
            parse_mode="HTML",
            disable_web_page_preview=True
        )
        return
    ...
    welcome_text = (
        ...
        "📢 Жалобы на администраторов подаются через сайт:\n"
        f"🌐 {SITE_URL}\n\n"
...
@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "📢 Оставить жалобу на админа")
...
@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "🛠️ Админ-панель")
async def btn_admin_panel(message: Message):
    """Открытие главной админ-панели"""
    if not await is_bot_admin(message.from_user.id):
        await message.answer("❌ У вас нет доступа к админ-панели.")
        return
    await message.answer(
        "🛠️ <b>Админ-панель VapeNeon</b>\n\nВыберите раздел:",
        parse_mode="HTML",
        reply_markup=get_admin_panel_keyboard()
    )


@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "📋 Просмотреть жалобы")
...
@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "📣 Рассылка в боте")
...
@dp.message(StateFilter(None), F.chat.type == ChatType.PRIVATE, F.text == "💬 Рассылка в чате")
...
async def cmd_stats(message: Message):
    ...
    total_warns = cursor.fetchone()["c"]

    cursor.execute("SELECT COUNT(*) AS c FROM warns WHERE expires_at > %s", (datetime.now(),))
    active_warns = cursor.fetchone()["c"]

    # Статистика мутов
    cursor.execute("SELECT COUNT(*) AS c FROM mutes")
    total_mutes = cursor.fetchone()["c"]

    cursor.execute("SELECT COUNT(*) AS c FROM mutes WHERE is_active = TRUE AND (expires_at IS NULL OR expires_at > %s)", (datetime.now(),))
    active_mutes = cursor.fetchone()["c"]

    # Статистика банов
    cursor.execute("SELECT COUNT(*) AS c FROM bans")
    total_bans = cursor.fetchone()["c"]

    cursor.execute("SELECT COUNT(*) AS c FROM bans WHERE is_active = TRUE AND (expires_at IS NULL OR expires_at > %s)", (datetime.now(),))
    active_bans = cursor.fetchone()["c"]

    # Статистика объявлений
    cursor.execute("SELECT COUNT(*) AS c FROM user_ads")
    total_ads = cursor.fetchone()["c"]

    cursor.execute("SELECT COUNT(*) AS c FROM user_ads WHERE DATE(sent_at) = CURRENT_DATE")
    today_ads = cursor.fetchone()["c"]

    # Статистика администраторов
    cursor.execute("SELECT COUNT(*) AS c FROM admins")
    total_admins = cursor.fetchone()["c"]

    cursor.execute("SELECT COUNT(*) AS c FROM admin_warns WHERE is_active = TRUE")
    active_admin_warns = cursor.fetchone()["c"]
...
async def cmd_admin_add(message: Message, command: CommandObject):
    ...
    if parsed['error']:
        await message.answer(
            ...
            "• <code>orders</code> — Заказы (только выдача заказов, без модерации)",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        # Генерируем пароль для сайта
        import string as _str, random as _rnd
        _chars = _str.ascii_letters + _str.digits + "!@#$%"
        _site_password = "".join(_rnd.choices(_chars, k=12))
        _site_username = (user.username or str(user_id)).lower()

        # Регистрируем на сайте (права на /report жалобы, БЕЗ прав на жалобы на администраторов)
        _site_registered = False
        try:
            import httpx as _httpx
            async with _httpx.AsyncClient(timeout=8) as _client:
                _resp = await _client.post(f"{SITE_URL}/api/site-admins/add", json={
                    "secret": BOT_TOKEN,
                    "tg_id": user_id,
                    "username": _site_username,
                    "password": _site_password,
                    "added_by": message.from_user.id,
                    "can_review_admin_complaints": 0,
                })
                if _resp.status_code == 200:
                    _site_registered = True
                else:
                    logger.error(f"Сайт вернул ошибку при регистрации администратора: {_resp.status_code} {_resp.text}")
        except Exception as _e:
            logger.error(f"Ошибка регистрации администратора на сайте: {_e}")

        # Уведомляем нового администратора в ЛС
        _rules_text = ORDERS_ADMIN_RULES_TEXT if role_str == "orders" else ADMIN_RULES_TEXT
        _dm_sent = False
        try:
            await bot.send_message(
                chat_id=user_id,
                text=f"🎉 <b>Вам назначены права администратора</b>\n\n"
                     ...
                     f"🌐 <b>Доступ к сайту:</b>\n"
                     f"🔗 {SITE_URL}\n"
                     f"👤 Логин: <code>{_site_username}</code>\n"
                     f"🔐 Пароль: <code>{_site_password}</code>\n\n"
            ...
            logger.error(f"Не удалось отправить ЛС новому администратору {user_id}: {_e}")

        # Если ЛС не дошло — отправляем логин/пароль ВЛАДЕЛЬЦУ В ЛИЧКУ (никогда не в группу!)
        if not _dm_sent:
            _site_info = (
                f"🌐 {SITE_URL}\n"
                f"👤 Логин: <code>{_site_username}</code>\n"
                f"🔐 Пароль: <code>{_site_password}</code>"
            ) if _site_registered else (
                "⚠️ Не удалось зарегистрировать на сайте. Попробуйте /addjbadm позже или добавьте вручную."
            )
            _owner_notified = False
            try:
                await bot.send_message(
                    chat_id=message.from_user.id,
                    text=f"⚠️ <b>Не удалось отправить данные в ЛС пользователю {user_mention}</b> "
                         f"(бот не начат — попросите его нажать /start в личке с ботом).\n\n"
                         f"Передайте вручную:\n{_site_info}",
                    parse_mode="HTML"
                )
                _owner_notified = True
            except Exception as _e:
                logger.error(f"Не удалось отправить ЛС владельцу {message.from_user.id}: {_e}")

            # В группе — НИКОГДА не показываем логин/пароль, только статус
            if _owner_notified:
                await message.answer(
                    f"✅ {user_mention} назначен. Логин и пароль от сайта отправлены вам в личные сообщения "
                    f"(боту у нового админа не открыт диалог).",
                    parse_mode="HTML"
                )
            else:
                await message.answer(
                    f"✅ {user_mention} назначен, но не удалось отправить данные для сайта ни ему, ни вам в ЛС.\n"
                    f"Напишите боту в личку /start и повторите /addjbadm позже.",
                    parse_mode="HTML"
                )
...
async def cmd_admin_remove(message: Message, command: CommandObject):
    ...
    try:
        ...
        # Деактивируем доступ на сайте (снимает is_active и стирает пароль)
        _site_deactivated = False
        try:
            import httpx as _httpx
            async with _httpx.AsyncClient(timeout=8) as _client:
                _resp = await _client.post(f"{SITE_URL}/api/site-admins/deactivate", json={
                    "secret": BOT_TOKEN,
                    "tg_id": user_id,
                })
                if _resp.status_code == 200:
                    _site_deactivated = True
                else:
                    logger.error(f"Сайт вернул ошибку при деактивации администратора: {_resp.status_code} {_resp.text}")
        except Exception as _e:
            logger.error(f"Ошибка деактивации администратора на сайте: {_e}")
        
        # Отправляем подтверждение
        _site_note = " Доступ к сайту также отключён." if _site_deactivated else " ⚠️ Не удалось отключить доступ к сайту — сделайте это вручную (/removejbadm или база)."
        await message.answer(
            f"✅ Пользователь {user_mention} удален из администраторов бота и лишен прав администратора группы.{_site_note}",
            parse_mode="HTML"
        )
...
async def _cmd_role_quest(message: Message, command: CommandObject, role: str):
    """Управляет набором волонтёров/администраторов: открыть / открыть с лимитом / закрыть.
    Анкета теперь заполняется на сайте — команда только рассылает объявление в чат."""
    if not await is_owner(message.from_user.id):
        await message.answer("❌ Только владелец может управлять этим набором.")
        return

    role_texts = {
        "admin":     {"genitive": "администраторов", "cmd": "adminquest", "tab": "«Заявка на роль администратора»"},
        "volunteer": {"genitive": "волонтёров",       "cmd": "volquest",  "tab": "«Заявка на роль волонтёра»"},
    }
    rt = role_texts[role]
    ...
    # ── Закрытие набора ──
    if arg == "close":
        state = get_role_quest_state(role)
        if not state["is_open"]:
            await message.answer(f"ℹ️ Набор {rt['genitive']} и так закрыт.")
            return
        set_role_quest_closed(role)
        try:
            await bot.send_message(
                CHAT_ID,
                f"🔒 <b>Набор {rt['genitive']} закрыт.</b>\n\n"
                "Спасибо всем, кто подал заявку! Новые заявки больше не принимаются.",
                parse_mode="HTML"
            )
            await message.answer(f"✅ Набор {rt['genitive']} закрыт.")
        except Exception as e:
            await message.answer(f"❌ Набор закрыт в боте, но не удалось отправить сообщение в группу: {e}")
        return

    # ── Открытие набора (с лимитом или без) ──
    ...
    if arg:
        if not arg.isdigit() or int(arg) <= 0:
            await message.answer(
                ...
                "Использование:\n"
                f"<code>/{rt['cmd']}</code> — открыть набор без лимита\n"
                f"<code>/{rt['cmd']} 10</code> — открыть набор с лимитом заявок\n"
                f"<code>/{rt['cmd']} close</code> — закрыть набор",
                parse_mode="HTML"
            )
            return
        max_applications = int(arg)

    set_role_quest_open(role, max_applications)

    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="📝 Подать заявку",
            url=f"https://t.me/{bot_username}?start={role}_quest_info"
        )
    ]])

    limit_note = f"\n\n🎟 Принимается не более <b>{max_applications}</b> заявок." if max_applications else ""

    try:
        await bot.send_message(
            CHAT_ID,
            f"📢 <b>Открыт набор {rt['genitive']}!</b> 📢\n\n"
            "Хотите стать частью команды? Нажмите кнопку ниже — она откроет чат с ботом, "
            f"где вы получите инструкцию, как подать заявку на сайте.{limit_note}",
            parse_mode="HTML",
            reply_markup=kb
        )
        await message.answer(f"✅ Объявление о наборе {rt['genitive']} отправлено в группу.")
    except Exception as e:
        await message.answer(f"❌ Не удалось отправить сообщение в группу: {e}")


@dp.message(Command("admin_quest"))
async def cmd_admin_quest(message: Message, command: CommandObject):
    await _cmd_role_quest(message, command, "admin")


@dp.message(Command("vol_quest"))
async def cmd_vol_quest(message: Message, command: CommandObject):
    await _cmd_role_quest(message, command, "volunteer")
...
async def cmd_ban_info(message: Message, command: CommandObject):
    ...
    cursor.execute("SELECT COUNT(*) AS c FROM warns WHERE user_id = %s AND chat_id = %s", 
                  (user_id, message.chat.id))
    warn_count = cursor.fetchone()["c"]
    
    cursor.execute("SELECT COUNT(*) AS c FROM mutes WHERE user_id = %s AND chat_id = %s AND is_active = TRUE", 
                  (user_id, message.chat.id))
    mute_count = cursor.fetchone()["c"]
    
    cursor.execute("SELECT COUNT(*) AS c FROM bans WHERE user_id = %s AND chat_id = %s AND is_active = TRUE", 
                  (user_id, message.chat.id))
    ban_count = cursor.fetchone()["c"]
...
@dp.message(Command("addjbadm"))
async def cmd_addjbadm(message: Message, command: CommandObject):
    """Выдать администратору право рассматривать жалобы на администраторов на сайте"""
    if message.from_user.id not in ADMIN_IDS:
        await message.answer("❌ Нет прав.")
        return

    # Определяем цель: reply > ID > @username
    ...
    if message.reply_to_message and message.reply_to_message.from_user:
        ...
        if arg.lstrip("-").isdigit():
            ...
            # Режим @username — ищем в БД
            uname = arg.lstrip("@").lower()
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(
                "SELECT user_id, username FROM bot_users WHERE lower(username)=%s", (uname,)
            )
            row = cursor.fetchone()
            conn.close()
            if row:
                ...
                await message.answer(
                    f"❌ Пользователь @{uname} не найден в базе.\n"
                    "Убедитесь, что он писал в чат, или используйте ID.",
                    parse_mode="HTML"
                )
                return
    else:
        await message.answer(
            "📋 <b>Использование:</b>\n\n"
            "• Ответьте на сообщение: <code>/addjbadm</code>\n"
            "• По ID: <code>/addjbadm 123456789</code>\n"
            "• По username: <code>/addjbadm @username</code>\n\n"
            "Выдаёт администратору сайта право рассматривать жалобы на администраторов.\n"
            "Пользователь уже должен быть назначен администратором через бота.",
            parse_mode="HTML"
        )
        return

    # Получаем username из БД если не знаем его ещё
    if not target_username and target_user_id:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT username FROM bot_users WHERE user_id=%s", (target_user_id,))
        row = cursor.fetchone()
        conn.close()
        target_username = row[0] if row else None

    site_username = target_username.lstrip("@").lower() if target_username else ""

    try:
        import httpx as _httpx
        async with _httpx.AsyncClient(timeout=8) as client:
            r = await client.post(f"{SITE_URL}/api/site-admins/grant-complaints", json={
                "secret": BOT_TOKEN,
                "username": site_username,
                "tg_id": target_user_id or 0,
            })
            if r.status_code == 404:
                label = f"@{site_username}" if site_username else f"ID {target_user_id}"
                await message.answer(
                    f"❌ {label} не найден среди администраторов сайта.\n"
                    "Сначала назначьте его администратором через бота командой /admin_add.",
                    parse_mode="HTML"
                )
                return
            if r.status_code != 200:
                await message.answer(f"❌ Ошибка сайта: {r.text}")
                return
    except Exception as e:
        await message.answer(f"❌ Ошибка подключения к сайту: {e}")
        return
    ...
    if target_user_id:
        try:
            await bot.send_message(
                target_user_id,
                "⚖️ <b>Вам выданы расширенные права на сайте VapeNeon</b>\n\n"
                "Теперь вы можете рассматривать жалобы на администраторов.\n\n"
                f"🌐 {SITE_URL}\n"
                f"👮 Выдал: {message.from_user.mention_html()}",
                parse_mode="HTML"
            )
        except Exception as e:
            logger.error(f"Could not notify {site_username}: {e}")

    label = f"@{site_username}" if site_username else f"ID {target_user_id}"
    await message.answer(
        f"✅ {label} теперь может рассматривать жалобы на администраторов на сайте.",
        parse_mode="HTML"
    )
...
async def cmd_report(message: Message, command: CommandObject):
    ...
    # Сохраняем жалобу в БД для отображения на сайте
    reporter_username = message.from_user.username or str(message.from_user.id)
    reported_username_str = ""
    try:
        _u = await bot.get_chat(reported_user_id)
        reported_username_str = _u.username or str(reported_user_id)
    except Exception:
        reported_username_str = str(reported_user_id)
    reported_msg_text = ""
    reported_msg_link = ""
    if message.reply_to_message:
        reported_msg_text = message.reply_to_message.text or message.reply_to_message.caption or ""
        chat_id_for_link = str(message.chat.id).replace("-100", "")
        reported_msg_link = f"https://t.me/c/{chat_id_for_link}/{message.reply_to_message.message_id}"
    try:
        _conn = get_db_connection()
        ...
        _cursor.execute(
            """INSERT INTO user_reports
               (reporter_id, reporter_username, reported_id, reported_username,
                reason, message_text, message_link, chat_id, status)
               VALUES (%s,%s,%s,%s,%s,%s,%s,%s,'pending')""",
            (message.from_user.id, reporter_username, reported_user_id, reported_username_str,
             reason, reported_msg_text[:500] if reported_msg_text else None,
             reported_msg_link or None, message.chat.id)
        )
        _conn.commit()
        _conn.close()
    except Exception as _e:
        logger.error(f"Не удалось сохранить жалобу в БД: {_e}")

    # Короткое уведомление для администраторов — без деталей жалобы и без кнопок
    # с действиями. Рассмотрение жалобы происходит только на сайте.
    report_text = (
        f"🚨 <b>Новая жалоба</b>\n\n"
        f"Поступила новая жалоба на пользователя. Рассмотрите её в панели администратора на сайте."
    )

    # Клавиатура — только ссылка на сайт, без кнопок быстрых действий
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(
    ...
    if success_count > 0:
        await confirm_msg.edit_text(
            f"✅ Ваша жалоба на {reported_mention} отправлена на сайт.",
            parse_mode="HTML"
        )
    else:
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        if action == "warn":
            ...
            # warn_user() уже сам отправляет сообщение о предупреждении в чат — второе не шлём
            ...
            # mute_user() уже сам отправляет сообщение о муте в чат — второе не шлём
            ...
            # ban_user() уже сам отправляет сообщение о бане в чат — второе не шлём
...
async def process_evidence(message: Message, state: FSMContext):
    ...
    # Получаем все данные из состояния и СРАЗУ сбрасываем его,
    # чтобы повторные сообщения не создавали дублирующие жалобы
    data = await state.get_data()
    await state.clear()
    ...
    complaint_keyboard = keyboard.as_markup()

    # Отправляем жалобу владельцам бота (одно сообщение с клавиатурой)
    sent_count = 0
    for admin_id in ADMIN_IDS:
        try:
...
async def process_approve_actions(message: Message, state: FSMContext):
    ...
    actions = message.text.strip() if message.text else ""
    if not actions:
        actions = "Причина не указана"
...
async def auto_punish_non_admin(message: Message):
    ...
    await delete_message(message.chat.id, message.message_id)
    # mute_user сам отправляет в чат сообщение об автоматическом наказании,
    # поэтому здесь дополнительное сообщение не отправляем — иначе получается 2 сообщения
    await mute_user(
        message.chat.id,
        message.from_user.id,
        MUTE_DURATION,
...
@dp.message(Command("amnist"))
async def cmd_amnist(message: Message):
    """Амнистия — снятие бана со всех пользователей в чёрном списке группы"""
    if not await is_chat_admin(message.from_user.id, message.chat.id):
        await auto_punish_non_admin(message)
        return
    ...
    status_msg = await message.answer(
        "⏳ <b>Амнистия запущена...</b>\n\nПолучаю список заблокированных пользователей.",
        parse_mode="HTML"
    )

    # Получаем список забаненных из БД
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT user_id FROM bans WHERE chat_id = %s AND is_active = TRUE",
        (message.chat.id,)
    )
    ...
    if not banned_rows:
        await status_msg.edit_text(
            "✅ <b>Чёрный список пуст</b>\n\nНет активных банов в базе данных.",
            parse_mode="HTML"
        )
        return

    total = len(banned_rows)
    success_count = 0
    fail_count = 0
    unbanned_ids = []

    await status_msg.edit_text(
        f"⏳ <b>Амнистия в процессе...</b>\n\n"
        f"🔓 Снимаю бан с <b>{total}</b> пользователей...",
        parse_mode="HTML"
    )

    for (user_id,) in banned_rows:
        try:
            await bot.unban_chat_member(message.chat.id, user_id, only_if_banned=True)
            unbanned_ids.append(user_id)
            success_count += 1
        except Exception as e:
            logger.warning(f"Амнистия: не удалось разбанить {user_id}: {e}")
            fail_count += 1
        await asyncio.sleep(0.05)  # небольшая пауза, чтобы не упереться в лимиты Telegram

    # Деактивируем все баны в БД для успешно разбаненных
    if unbanned_ids:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.executemany(
            "UPDATE bans SET is_active = FALSE WHERE user_id = %s AND chat_id = %s AND is_active = TRUE",
            [(uid, message.chat.id) for uid in unbanned_ids]
        )
        conn.commit()
        conn.close()

    # Итоговый отчёт
    issuer_mention = await get_user_mention(message.from_user.id)
    result_text = (
        f"✅ <b>Амнистия завершена</b>\n\n"
        f"👮 <b>Инициатор:</b> {issuer_mention}\n"
        f"🔓 <b>Разбанено:</b> {success_count} из {total}\n"
    )
    if fail_count:
        result_text += f"⚠️ <b>Не удалось разбанить:</b> {fail_count}\n"
...
# ===== ХЭНДЛЕРЫ ДОБАВЛЕНИЯ ТОВАРА (должны быть ДО глобального F.text) =====

@dp.message(StateFilter(ProductStates.waiting_for_name))
async def adm_product_name(message: Message, state: FSMContext):
    """Получить название товара"""
    if message.chat.type != ChatType.PRIVATE:
        return
    if not message.text:
        await message.answer("⚠️ Пожалуйста, введите текстовое <b>название</b> товара:", parse_mode="HTML")
        return
    name = message.text.strip()
    if len(name) < 2:
        await message.answer("⚠️ Название слишком короткое. Введите ещё раз:")
        return
    await state.update_data(product_name=name)
    await state.set_state(ProductStates.waiting_for_price)
    await message.answer(
        f"✅ Название: <b>{name}</b>\n\nТеперь введите <b>цену</b> товара (например: 500 или 500 руб):",
        parse_mode="HTML"
    )


@dp.message(StateFilter(ProductStates.waiting_for_price))
async def adm_product_price(message: Message, state: FSMContext):
    """Получить цену товара"""
    if message.chat.type != ChatType.PRIVATE:
        return
    if not message.text:
        await message.answer("⚠️ Пожалуйста, введите <b>цену</b> товара текстом:", parse_mode="HTML")
        return
    price = message.text.strip()
    await state.update_data(product_price=price)
    await state.set_state(ProductStates.waiting_for_description)
    await message.answer(
        f"✅ Цена: <b>{price}</b>\n\nТеперь введите <b>описание</b> товара (объём, вкус и т.д.):",
        parse_mode="HTML"
    )


@dp.message(StateFilter(ProductStates.waiting_for_description))
async def adm_product_description(message: Message, state: FSMContext):
    """Получить описание товара"""
    if message.chat.type != ChatType.PRIVATE:
        return
    if not message.text:
        await message.answer("⚠️ Пожалуйста, введите текстовое <b>описание</b> товара:", parse_mode="HTML")
        return
    description = message.text.strip()
    await state.update_data(product_description=description)
    await state.set_state(ProductStates.waiting_for_photo)
    await message.answer(
        "✅ Описание сохранено.\n\nТеперь отправьте <b>фото</b> товара (изображение):",
        parse_mode="HTML"
    )


@dp.message(StateFilter(ProductStates.waiting_for_photo))
async def adm_product_photo(message: Message, state: FSMContext):
    """Получить первое фото товара"""
    if message.chat.type != ChatType.PRIVATE:
        return
    if not message.photo:
        await message.answer("⚠️ Пожалуйста, отправьте именно <b>фото</b> (изображение) товара:", parse_mode="HTML")
        return
    photo_file_id = message.photo[-1].file_id
    await state.update_data(main_photo=photo_file_id, extra_photos=[])
    await state.set_state(ProductStates.waiting_for_more_photos)
    await message.answer(
        "✅ Первое фото сохранено.\n\n"
        "Отправьте ещё фото (до 9 штук), или напишите <b>/done</b> чтобы завершить добавление товара.",
        parse_mode="HTML"
    )


@dp.message(StateFilter(ProductStates.waiting_for_more_photos))
async def adm_product_more_photos(message: Message, state: FSMContext):
    """Принимать дополнительные фото или команду /done"""
    if message.chat.type != ChatType.PRIVATE:
        return
    ...
    if message.text and message.text.strip().lower() in ("/done", "done"):
        ...
        if not data.get("main_photo"):
            await message.answer("⚠️ Ошибка: основное фото не найдено. Начните добавление заново через админ-панель.")
            await state.clear()
            return
        ...
        # Формируем итоговое описание с ценой
        full_description = f"💰 Цена: {price}\n{description}" if price else description

        product_id = add_product_to_db(
            category=category,
            name=name,
            description=full_description,
            photo_file_id=main_photo,
            added_by=message.from_user.id,
        ...
        builder.row(InlineKeyboardButton(text="◀️ Назад в категорию", callback_data=f"adm:cat:{cat_index}"))
        builder.row(InlineKeyboardButton(text="🏠 Главное меню", callback_data="adm:back_panel"))
        ...
        await message.answer_photo(
            ...
            caption=(
                ...
                f"🖼 <b>Фотографий:</b> {total_photos}\n\n"
                f"Товар появится на сайте в разделе «Товары»."
            ),
            parse_mode="HTML",
    ...
    if message.photo:
        data = await state.get_data()
        extra = data.get("extra_photos", [])
        if len(extra) >= 9:
            await message.answer("⚠️ Максимум 9 дополнительных фото. Напишите /done для завершения.")
            return
        extra.append(message.photo[-1].file_id)
        await state.update_data(extra_photos=extra)
        total = 1 + len(extra)
        await message.answer(
            f"✅ Фото {total} добавлено. Всего: <b>{total}</b>.\n"
            f"Отправьте ещё или напишите <b>/done</b>.",
            parse_mode="HTML"
        )
        return
    ...
    await message.answer(
        "⚠️ Отправьте <b>фото</b> или напишите <b>/done</b> для завершения.",
        parse_mode="HTML"
    )


# ===== КОНЕЦ ХЭНДЛЕРОВ ДОБАВЛЕНИЯ ТОВАРА =====

# ============================================================
#        ПЛАТНЫЕ УСЛУГИ: закрепление объявлений + автопостинг
# ============================================================

class AutopostSettingsStates(StatesGroup):
    waiting_text = State()
    waiting_photos = State()


async def pin_queue_monitor():
    """Фоновая задача: открепляет объявления по истечении срока и запускает
    следующее из очереди закрепление, если место свободно."""
    while True:
        try:
            now = get_moscow_time()
            conn = get_db_connection()
            ...
            # 1. Открепляем всё, чей срок истёк
            cursor.execute(
                "SELECT * FROM pin_queue WHERE status = 'active' AND unpin_at <= %s",
                (now,)
            )
            ...
            for row in expired:
                try:
                    ...
                    logger.warning(f"Не удалось открепить сообщение {row['message_id']}: {e}")
                cursor.execute("UPDATE pin_queue SET status = 'done' WHERE id = %s", (row["id"],))
                conn.commit()
                try:
                    await bot.send_message(
                        row["user_id"],
                        "📌 Срок платного закрепления вашего объявления истёк, оно откреплено.",
                        parse_mode="HTML"
                    )
                except Exception:
                    pass

            # 2. Если сейчас ничего активного не закреплено платно — запускаем следующего из очереди
            cursor.execute("SELECT 1 FROM pin_queue WHERE status = 'active' LIMIT 1")
            has_active = cursor.fetchone() is not None

            if not has_active:
                cursor.execute(
                    "SELECT * FROM pin_queue WHERE status = 'queued' ORDER BY created_at ASC LIMIT 1"
                )
                nxt = cursor.fetchone()
                if nxt:
                    success = False
                    if nxt["message_id"]:
                        try:
                            await bot.pin_chat_message(
                                chat_id=nxt["chat_id"], message_id=nxt["message_id"], disable_notification=False
                            )
                            success = True
                        except Exception as e:
                            logger.error(f"Не удалось закрепить сообщение {nxt['message_id']}: {e}")
                    unpin_at = now + timedelta(hours=float(nxt["hours"]))
                    if success:
                        cursor.execute(
                            "UPDATE pin_queue SET status='active', pinned_at=%s, unpin_at=%s WHERE id=%s",
                            (now, unpin_at, nxt["id"])
                        )
                        conn.commit()
                        try:
                            await bot.send_message(
                                nxt["user_id"],
                                f"📌 Ваше объявление закреплено на <b>{float(nxt['hours']):g} ч.</b>",
                                parse_mode="HTML"
                            )
                        except Exception:
                            pass
                    else:
                        # Не удалось закрепить (например, сообщение удалено) — закрываем заявку
                        cursor.execute("UPDATE pin_queue SET status='failed' WHERE id=%s", (nxt["id"],))
                        conn.commit()
                        try:
                            await bot.send_message(
                                nxt["user_id"],
                                "⚠️ Не удалось закрепить ваше объявление (возможно, сообщение было удалено). "
            ...
            logger.error(f"Ошибка pin_queue_monitor: {e}")

        await asyncio.sleep(20)


def _build_autopost_menu_text_kb(settings: dict):
    photos_count = len(settings.get("photos") or [])
    has_text = bool((settings.get("text") or "").strip())
    start_h = settings.get("start_hour") or 0
    end_h = settings.get("end_hour") or 0
    enabled = settings.get("enabled")

    status_line = "🟢 Автопостинг активен" if enabled else "⚪ Автопостинг ещё не запущен"
    txt = (
        "🚀 <b>Настройки автопостинга</b>\n\n"
        f"{status_line}\n\n"
        f"📸 Фотографии: <b>{photos_count}</b> шт.\n"
        f"📝 Текст объявления: {'установлен ✅' if has_text else 'не задан ❌'}\n"
        f"🕒 Время публикаций (МСК): <b>{start_h:02d}:00–{end_h:02d}:00</b>\n\n"
        "Заполните все пункты и нажмите «Принять», чтобы бот начал публиковать "
        "ваше объявление автоматически: не чаще 1 раза в 1.5 часа, максимум 5 раз "
        "в сутки, с паузой не менее 30 сообщений в чате между публикациями."
    )
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text=f"📸 Добавить фотографии ({photos_count})", callback_data="ap_add_photo"))
    kb.row(InlineKeyboardButton(text=f"📝 Добавить текст {'✅' if has_text else ''}", callback_data="ap_add_text"))
    kb.row(InlineKeyboardButton(text=f"🕒 Время: {start_h:02d}:00–{end_h:02d}:00", callback_data="ap_time_menu"))
    kb.row(InlineKeyboardButton(text="✅ Принять", callback_data="ap_accept"))
    return txt, kb.as_markup()


def _get_or_create_autopost_settings(user_id: int) -> dict:
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM autopost_settings WHERE user_id = %s", (user_id,))
    row = cursor.fetchone()
    if not row:
        cursor.execute(
            "INSERT INTO autopost_settings (user_id) VALUES (%s) ON CONFLICT (user_id) DO NOTHING",
            (user_id,)
        )
        conn.commit()
        cursor.execute("SELECT * FROM autopost_settings WHERE user_id = %s", (user_id,))
...
@dp.message(F.text == "🚀 Настройки автопостинга", F.chat.type == ChatType.PRIVATE)
async def btn_autopost_settings(message: Message, state: FSMContext):
    await state.clear()
    if not has_active_autopost_subscription(message.from_user.id):
        await message.answer(
            "❌ У вас нет активной подписки на автопостинг. Оформите её на сайте во вкладке "
            "«Платные услуги».",
            reply_markup=get_main_keyboard(message.from_user.id)
        )
        return
    settings = _get_or_create_autopost_settings(message.from_user.id)
    txt, kb = _build_autopost_menu_text_kb(settings)
    await message.answer(txt, reply_markup=kb, parse_mode="HTML")


@dp.callback_query(F.data == "ap_menu")
async def ap_menu(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.clear()
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    txt, kb = _build_autopost_menu_text_kb(settings)
    await callback.message.edit_text(txt, reply_markup=kb, parse_mode="HTML")


@dp.callback_query(F.data == "ap_add_photo")
async def ap_add_photo(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(AutopostSettingsStates.waiting_photos)
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="✅ Готово", callback_data="ap_photos_done"))
    kb.row(InlineKeyboardButton(text="🗑️ Очистить фото", callback_data="ap_photos_clear"))
    await callback.message.edit_text(
        "📸 Отправьте одну или несколько фотографий для вашего объявления (по одной сообщением).\n"
        "Когда закончите — нажмите «Готово».",
        reply_markup=kb.as_markup()
    )


@dp.message(AutopostSettingsStates.waiting_photos, F.photo)
async def ap_receive_photo(message: Message, state: FSMContext):
    settings = _get_or_create_autopost_settings(message.from_user.id)
    photos = settings.get("photos") or []
    if len(photos) >= 9:
        await message.answer("⚠️ Максимум 9 фотографий. Нажмите «Готово».")
        return
    photos.append(message.photo[-1].file_id)
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET photos = %s, updated_at = now() WHERE user_id = %s",
        (json.dumps(photos), message.from_user.id)
    )
    conn.commit()
    conn.close()
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text="✅ Готово", callback_data="ap_photos_done"))
    kb.row(InlineKeyboardButton(text="🗑️ Очистить фото", callback_data="ap_photos_clear"))
    await message.answer(f"✅ Фото добавлено ({len(photos)}/9). Отправьте ещё или нажмите «Готово».", reply_markup=kb.as_markup())


@dp.callback_query(F.data == "ap_photos_clear")
async def ap_photos_clear(callback: CallbackQuery):
    await callback.answer("Фото очищены")
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET photos = '[]'::jsonb, updated_at = now() WHERE user_id = %s",
...
@dp.callback_query(F.data == "ap_photos_done")
async def ap_photos_done(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    await ap_menu(callback, state)


@dp.callback_query(F.data == "ap_add_text")
async def ap_add_text(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await state.set_state(AutopostSettingsStates.waiting_text)
    await callback.message.edit_text(
        "📝 Отправьте текст вашего объявления одним сообщением.",
    )


@dp.message(AutopostSettingsStates.waiting_text, F.text)
async def ap_receive_text(message: Message, state: FSMContext):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET text = %s, updated_at = now() WHERE user_id = %s",
...
@dp.callback_query(F.data == "ap_time_menu")
async def ap_time_menu(callback: CallbackQuery):
    await callback.answer()
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    start_h = settings.get("start_hour") or 0
    end_h = settings.get("end_hour") or 0
    kb = InlineKeyboardBuilder()
    kb.row(InlineKeyboardButton(text=f"🔽 Начало: {start_h:02d}:00 (нажмите +1ч)", callback_data="ap_time_start"))
    kb.row(InlineKeyboardButton(text=f"🔼 Конец: {end_h:02d}:00 (нажмите +1ч)", callback_data="ap_time_end"))
    kb.row(InlineKeyboardButton(text="⬅️ Назад", callback_data="ap_menu"))
    await callback.message.edit_text(
        "🕒 <b>Временной промежуток публикаций (Московское время)</b>\n\n"
        "Каждое нажатие на кнопку добавляет 1 час. При изменении начала конец "
        "сбрасывается на то же время.",
        reply_markup=kb.as_markup(),
        parse_mode="HTML"
    )


@dp.callback_query(F.data == "ap_time_start")
async def ap_time_start(callback: CallbackQuery):
    await callback.answer()
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    start_h = ((settings.get("start_hour") or 0) + 1) % 24
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET start_hour = %s, end_hour = %s, updated_at = now() WHERE user_id = %s",
        (start_h, start_h, callback.from_user.id)
    )
    conn.commit()
    conn.close()
    await ap_time_menu(callback)


@dp.callback_query(F.data == "ap_time_end")
async def ap_time_end(callback: CallbackQuery):
    await callback.answer()
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    end_h = ((settings.get("end_hour") or 0) + 1) % 24
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET end_hour = %s, updated_at = now() WHERE user_id = %s",
        (end_h, callback.from_user.id)
    )
    conn.commit()
    conn.close()
    await ap_time_menu(callback)
...
@dp.callback_query(F.data == "ap_accept")
async def ap_accept(callback: CallbackQuery):
    if not has_active_autopost_subscription(callback.from_user.id):
        await callback.answer("❌ Подписка неактивна", show_alert=True)
        return
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    if not (settings.get("text") or "").strip() and not (settings.get("photos") or []):
        await callback.answer("⚠️ Добавьте текст и/или фотографии перед запуском", show_alert=True)
        return
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE autopost_settings SET enabled = TRUE, updated_at = now() WHERE user_id = %s",
        (callback.from_user.id,)
    )
    conn.commit()
    conn.close()
    await callback.answer("✅ Автопостинг запущен!", show_alert=True)
    settings = _get_or_create_autopost_settings(callback.from_user.id)
    txt, kb = _build_autopost_menu_text_kb(settings)
    await callback.message.edit_text(txt, reply_markup=kb, parse_mode="HTML")
...
async def autopost_scheduler():
    """Фоновая задача автопостинга: публикует объявления подписчиков по расписанию
    (не чаще раза в 1.5ч, максимум 5 раз в сутки, пауза >=30 сообщений в чате)
    и следит за истечением подписок."""
    while True:
        try:
            now = get_moscow_time()
            today = now.date()
            ...
            # Истёкшие подписки — отключаем и уведомляем
            cursor.execute(
                "SELECT user_id FROM autopost_subscriptions WHERE active = TRUE AND expires_at <= %s",
                (now,)
            )
            expired_users = [r["user_id"] for r in cursor.fetchall()]
            for uid in expired_users:
                cursor.execute("UPDATE autopost_subscriptions SET active = FALSE WHERE user_id = %s", (uid,))
                cursor.execute("UPDATE autopost_settings SET enabled = FALSE WHERE user_id = %s", (uid,))
                conn.commit()
                try:
                    await bot.send_message(
                        uid,
                        "⏳ <b>Подписка на автопостинг истекла</b>\n\n"
                        "Автопостинг ваших объявлений остановлен. Чтобы продолжить публикации, "
                        "продлите подписку во вкладке «Платные услуги» на сайте.",
                        parse_mode="HTML"
                    )
                except Exception:
                    pass

            # Активные и включённые настройки автопостинга с действующей подпиской
            cursor.execute(
                """SELECT s.* FROM autopost_settings s
                   JOIN autopost_subscriptions sub ON sub.user_id = s.user_id
                   WHERE s.enabled = TRUE AND sub.active = TRUE AND sub.expires_at > %s""",
                (now,)
            )
            ...
            for row in candidates:
                ...
                posts_day_date = row.get("posts_day_date")
                if posts_day_date != today:
                    posts_today = 0

                if posts_today >= 5:
                    continue

                start_h = row.get("start_hour") or 0
                end_h = row.get("end_hour") or 0
                hour = now.hour
                if start_h == end_h:
                    in_window = True
                elif start_h < end_h:
                ...
                if last_post_at and (now - last_post_at) < timedelta(minutes=90):
                    continue

                last_counter = row.get("last_post_counter") or 0
                if last_post_at and (_autopost_msg_counter - last_counter) < 30:
                    continue
                ...
                try:
                    if photos:
                        if len(photos) == 1:
                            await bot.send_photo(CHAT_ID, photos[0], caption=text or None)
                        else:
                    ...
                    cursor.execute(
                        """UPDATE autopost_settings
                           SET last_post_at = %s, last_post_counter = %s,
                               posts_today = %s, posts_day_date = %s, updated_at = now()
                           WHERE user_id = %s""",
                        (now, _autopost_msg_counter, posts_today + 1, today, user_id)
                    )
                    conn.commit()
                except Exception as e:
                    logger.error(f"Ошибка автопостинга для пользователя {user_id}: {e}")

            conn.close()
        except Exception as e:
            logger.error(f"Ошибка autopost_scheduler: {e}")
...
async def handle_message(message: Message):
    ...
    # Счётчик сообщений в чате — используется автопостингом объявлений,
    # чтобы выдерживать паузу минимум в 30 сообщений между постами.
    global _autopost_msg_counter
    _autopost_msg_counter += 1
    ...
    if has_trigger_word:
        ...
        # warn_user() сам отправляет сообщение о выданном предупреждении в чат,
        # поэтому отдельное дублирующее сообщение здесь больше не отправляем.
    ...
    if text.startswith('/'):
        ...
        bot_commands = [
            ...
            '/ban', '/tban', '/unban', '/cc', '/admin_add', '/admin_remove', '/admin_quest', '/vol_quest',
...
async def cleanup_expired_data():
    while True:
        ...
        try:
            ...
            cursor.execute("DELETE FROM warns WHERE expires_at <= %s", (current_time,))
            
            # Деактивируем истекшие муты
            cursor.execute(
                "UPDATE mutes SET is_active = FALSE WHERE expires_at <= %s AND is_active = TRUE",
                (current_time,)
            )
            
            # Деактивируем истекшие баны
            cursor.execute(
                "UPDATE bans SET is_active = FALSE WHERE expires_at <= %s AND is_active = TRUE",
                (current_time,)
            )
        ...
        await asyncio.sleep(300 if had_error else 3600)

# ============================================================
#   ПУБЛИКАЦИЯ НОВЫХ КОМНАТ «ПРОСМОТР ФИЛЬМОВ» В ЧАТ
# ============================================================

async def movie_room_announcer():
    """Периодически проверяет очередь movie_room_announcements (её пополняет
    сайт при создании публичной — не приватной — комнаты просмотра) и сам
    публикует сообщение в общий чат от имени бота."""
    while True:
        conn = None
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute(
                "SELECT id, room_id, title, platform_label, owner_username "
                "FROM movie_room_announcements WHERE announced = FALSE ORDER BY id ASC LIMIT 20"
            )
            pending = cursor.fetchall()

            for row in pending:
                ann_id, room_id, title, platform_label, owner_username = row
                text = (
                    "🎬 <b>Новая комната просмотра!</b>\n"
                    f"Видео: {title}\n"
                    f"Платформа: {platform_label}\n"
                    f"ID комнаты: <code>{room_id}</code>\n"
                    f"Создал: @{owner_username}"
                )
                try:
                    await bot.send_message(CHAT_ID, text, parse_mode="HTML")
                except Exception as e:
                    logger.error(f"Не удалось опубликовать комнату {room_id}: {e}")
                    continue  # оставляем announced=FALSE, попробуем в следующий проход

                cursor.execute(
                    "UPDATE movie_room_announcements SET announced = TRUE WHERE id = %s",
                    (ann_id,),
                )
                conn.commit()

        except Exception as e:
            logger.error(f"Ошибка в movie_room_announcer: {e}")
        finally:
            if conn is not None:
                conn.close()
...
🚫 1. Нарушение правил отправки объявлений (НПОО)
   ...
   💎 Участники с тегом «УОО» рядом с именем (платная услуга «Урезанные ограничения на отправку объявлений» на сайте) живут по льготным лимитам: до 8 объявлений в день, не чаще 1 раза в час, и достаточно 10 сообщений после объявления вместо 30. Пока тег активен — считать НПОО нужно по этим цифрам.
...
📵 13. Продажа аккаунтов социальных сетей
   • Запрещена продажа, покупка и обмен аккаунтами любых социальных сетей (Telegram, Instagram, VK и т.д.)
   • Продажа игровых аккаунтов разрешена
   🔥 Нарушение: мут на 7 дней → бан при повторных нарушениях

📵 14. Продажа сим-карт и финансовых доступов
   • Запрещена продажа, покупка и обмен номерами телефонов и доступами к сим-картам
   • Запрещена продажа, покупка и обмен банковскими картами
   • Запрещена продажа, покупка и обмен доступами к брокерским и иным финансовым счетам
   🔥 Нарушение: перманентный бан"""

ADMIN_RULES_TEXT = """👮 Правила для администраторов 👮

• Запрещено злоупотребление полномочиями: мут/бан/варн без причины или в личных целях
• Каждое наказание обязательно сопровождается указанием причины
• Запрещено разглашение внутренней информации (жалобы, переписки модерации, личные данные пользователей) третьим лицам
• Запрещено использование служебного положения для личной выгоды (вымогательство, продажа привилегий, скидок «по знакомству»)
• Логин и пароль от сайта — строго личные, передача третьим лицам запрещена

🔥 Нарушение: предупреждение администратору → снятие роли при повторном нарушении"""

ORDERS_ADMIN_RULES_TEXT = """📦 Правила для админов на выдаче заказов 📦

• Зона ответственности — исключительно подтверждение и выдача заказов
• Запрещено использовать модераторские команды (мут/бан/варн) — таких прав у роли нет
• Запрещено вмешиваться в конфликты, жалобы и споры — при обращении направлять к модератору/администратору
• Выдача заказа — только после подтверждения оплаты/сделки, «в долг» и «по знакомству» запрещены
• Каждая выдача обязательно фиксируется (кому, что, когда)
• Запрещены грубость и споры с покупателем при выдаче

🔥 Нарушение: предупреждение → снятие роли, при повторе — бан"""
...
💼 Комиссия гаранта: 5% от суммы сделки"""
...
async def _do_send_info():
    ...
    cursor.execute("SELECT message_id FROM periodic_messages WHERE chat_id = %s", (CHAT_ID,))
    ...
    cursor.execute("DELETE FROM periodic_messages WHERE chat_id = %s", (CHAT_ID,))
    conn.commit()
    ...
    for msg_id in new_ids:
        cursor.execute(
            "INSERT INTO periodic_messages (message_id, chat_id) VALUES (%s, %s)",
            (msg_id, CHAT_ID)
        )
    conn.commit()
    conn.close()
...
def get_admin_panel_keyboard():
    ...
    builder.row(InlineKeyboardButton(text="❌ Закрыть", callback_data="adm:close"))
    return builder.as_markup()


def get_product_categories_keyboard(with_back: bool = True):
    """Inline-клавиатура со списком категорий товаров"""
    builder = InlineKeyboardBuilder()
    for i, cat in enumerate(PRODUCT_CATEGORIES):
        builder.row(InlineKeyboardButton(text=cat, callback_data=f"adm:cat:{i}"))
    if with_back:
        builder.row(InlineKeyboardButton(text="◀️ Назад", callback_data="adm:back_panel"))
    return builder.as_markup()


def get_category_keyboard(cat_index: int):
    """Inline-клавиатура внутри категории"""
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="➕ Добавить товар", callback_data=f"adm:addprod:{cat_index}"))
    builder.row(InlineKeyboardButton(text="🗑 Удалить товар", callback_data=f"adm:delprod:{cat_index}"))
    builder.row(InlineKeyboardButton(text="◀️ Назад к категориям", callback_data="adm:products"))
    return builder.as_markup()


def get_product_list_for_delete(cat_index: int):
    """Inline-клавиатура со списком товаров для удаления"""
    category = PRODUCT_CATEGORIES[cat_index]
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name FROM shop_products WHERE category = %s AND is_active = 1 ORDER BY id DESC",
        (category,)
    )
    ...
    if products:
        for prod_id, prod_name in products:
            short_name = prod_name[:30] + "…" if len(prod_name) > 30 else prod_name
            builder.row(InlineKeyboardButton(
                text=f"🗑 {short_name}",
                callback_data=f"adm:delconfirm:{prod_id}:{cat_index}"
            ))
    builder.row(InlineKeyboardButton(text="◀️ Назад", callback_data=f"adm:cat:{cat_index}"))
    return builder.as_markup(), len(products)


# ВАЖНО для веб-сервера (web.py / server.py):
# 1. GET /api/products — добавьте в каждый товар поле "photos": список file_id из shop_product_photos
#    SELECT file_id FROM shop_product_photos WHERE product_id=? ORDER BY sort_order
# 2. GET /api/product-photo/{product_id}/{photo_index} — отдавать фото по индексу из shop_product_photos
#    (ранее был только /api/product-photo/{product_id} — он остаётся для совместимости, отдаёт фото с sort_order=0)

def add_product_to_db(category: str, name: str, description: str, photo_file_id: str, added_by: int, extra_photos: list = None) -> int:
    """Добавляет товар в БД и возвращает его ID. extra_photos — список доп. file_id."""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO shop_products (category, name, description, photo_file_id, added_by) VALUES (%s, %s, %s, %s, %s) RETURNING id",
        (category, name, description, photo_file_id, added_by)
    )
    product_id = cursor.fetchone()["id"]
    # Сохраняем все фото в отдельную таблицу (первое + доп.)
    all_photos = [photo_file_id] + (extra_photos or [])
    for i, fid in enumerate(all_photos):
        cursor.execute(
            "INSERT INTO shop_product_photos (product_id, file_id, sort_order) VALUES (%s, %s, %s)",
            (product_id, fid, i)
        )
    conn.commit()
    conn.close()
...
def delete_product_from_db(product_id: int):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE shop_products SET is_active = 0 WHERE id = %s", (product_id,))
    conn.commit()
    conn.close()
...
@dp.callback_query(F.data == "adm:back_panel")
async def adm_back_panel(callback: CallbackQuery):
    if not await is_bot_admin(callback.from_user.id):
...
async def adm_products(callback: CallbackQuery):
    """Показать список категорий товаров"""
    if not await is_bot_admin(callback.from_user.id):
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    await callback.message.edit_text(
...
@dp.callback_query(F.data.startswith("adm:cat:"))
async def adm_category(callback: CallbackQuery):
    """Открыть категорию"""
    if not await is_bot_admin(callback.from_user.id):
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    cat_index = int(callback.data.split(":")[2])
    category = PRODUCT_CATEGORIES[cat_index]

    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(*) AS c FROM shop_products WHERE category = %s AND is_active = 1",
        (category,)
    )
    count = cursor.fetchone()["c"]
    conn.close()
    ...
    await callback.message.edit_text(
        f"📂 <b>{category}</b>\n\n"
        f"Товаров в категории: <b>{count}</b>\n\n"
        f"Выберите действие:",
        parse_mode="HTML",
        reply_markup=get_category_keyboard(cat_index)
    )
    await callback.answer()


@dp.callback_query(F.data.startswith("adm:addprod:"))
async def adm_start_add_product(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(cat_index=cat_index, category=category)
    await callback.message.answer(
...
@dp.callback_query(F.data.startswith("adm:delprod:"))
async def adm_delete_product_list(callback: CallbackQuery):
    ...
    cat_index = int(callback.data.split(":")[2])
    category = PRODUCT_CATEGORIES[cat_index]
    kb, count = get_product_list_for_delete(cat_index)
...
async def adm_delete_confirm(callback: CallbackQuery):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT name FROM shop_products WHERE id = %s", (product_id,))
    row = cursor.fetchone()
    conn.close()
    ...
    await callback.answer(f"✅ Товар «{row[0]}» удалён.", show_alert=True)

    # Обновляем список
    kb, count = get_product_list_for_delete(cat_index)
    if count == 0:
        await callback.message.edit_text(
            f"📂 <b>{PRODUCT_CATEGORIES[cat_index]}</b>\n\nВсе товары удалены.",
            parse_mode="HTML",
            reply_markup=get_category_keyboard(cat_index)
        )
    else:
...
async def main():
    ...
    asyncio.create_task(pin_queue_monitor())
    asyncio.create_task(autopost_scheduler())
    asyncio.create_task(monitor_expired_punishments())
    asyncio.create_task(send_periodic_info())
    asyncio.create_task(movie_room_announcer())
