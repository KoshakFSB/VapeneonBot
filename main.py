...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    args = command.args
    user_id = None
    duration = MUTE_DURATION  # значение по умолчанию
    reason = "Не указана"
    ...
    if message.reply_to_message:
        ...
        if message.reply_to_message.from_user.is_bot:
            await message.answer("❌ Нельзя мутить ботов.")
            return
        
        # Если есть аргументы после команды
        if args:
            ...
            # Парсим время
            parsed_duration = parse_time(time_str)
            if parsed_duration:
        ...
        # Если не ответ на сообщение, то парсим аргументы
        if not args:
            await message.answer("❌ Укажите пользователя и время: /tmute @user 1d [причина]")
            return

        parts = args.split(maxsplit=2)  # максимум 3 части: user, время, причина
        ...
        # Проверяем, что это не бот (быстрая проверка, но не критично если не получится)
        try:
            user = await bot.get_chat(user_id)
            if user.is_bot:
                await message.answer("❌ Нельзя мутить ботов.")
...
async def show_user_reviews(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Проверяем, откуда пришел запрос (из чата или из ЛС)
    if callback.message.chat.type == ChatType.PRIVATE:
        # В ЛС - редактируем сообщение
        await callback.message.edit_text(
            text,
            parse_mode="HTML",
        ...
        # В чате - отправляем новое сообщение и удаляем старое
