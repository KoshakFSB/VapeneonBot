...
async def get_user_id_from_message(text: str) -> Optional[int]:
    ...
    if text.isdigit():
        user_id = int(text)
        logger.info(f"get_user_id_from_message: получен числовой ID: {user_id}")
        return user_id
    
    # Иначе пробуем получить по юзернейму
    try:
        user = await bot.get_chat(f"@{text}")
        logger.info(f"get_user_id_from_message: получен ID по юзернейму @{text}: {user.id}")
        return user.id
    except Exception as e:
...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    - Ответ на сообщение (ВСЕГДА приоритет)
    - ID пользователя в аргументах команды
    - @username в аргументах команды
    Возвращает (user_id, reason) или None, если пользователь не определён.
    
    ПРИОРИТЕТ: ответ на сообщение ВСЕГДА важнее аргументов команды!
    
    Форматы:
    1. Ответ на сообщение: /warn причина
    2. ID пользователя: /warn 123456789 причина
    3. @username: /warn @username причина
    """
    user_id = None
    reason = "Не указана"
    
    # СЛУЧАЙ 1: ВСЕГДА проверяем ответ на сообщение (максимальный приоритет)
    if message.reply_to_message:
        ...
        # Если аргументов нет, используем причину по умолчанию
        else:
            reason = "Не указана"
            
        logger.info(f"resolve_user_reference: получен ID из ответа: {user_id}, причина: {reason}")
        return (user_id, reason)
    
    # СЛУЧАЙ 2: Если нет ответа на сообщение, пробуем аргументы команды
    if command_args and command_args.strip():
        ...
        if user_id:
            logger.info(f"resolve_user_reference: получен ID из аргументов: {user_id}, причина: {reason}")
            return (user_id, reason)
    
    # Если ни один способ не сработал
    logger.warning(f"resolve_user_reference: не удалось определить пользователя. args: {command_args}")
    return None
...
async def resolve_user_only(message: Message, command_args: str = None) -> Optional[int]:
    ...
    Форматы:
    1. Ответ на сообщение: /warns
    2. ID пользователя: /warns 123456789
    3. @username: /warns @username
    """
    # СЛУЧАЙ 1: ВСЕГДА проверяем ответ на сообщение (максимальный приоритет)
    if message.reply_to_message:
        user_id = message.reply_to_message.from_user.id
        logger.info(f"resolve_user_only: получен ID из ответа: {user_id}")
        return user_id
    
    # СЛУЧАЙ 2: Если нет ответа на сообщение, пробуем аргументы команды
    if command_args and command_args.strip():
        user_identifier = command_args.strip().split()[0]  # Берём только первый аргумент
        user_id = await get_user_id_from_message(user_identifier)
        if user_id:
            logger.info(f"resolve_user_only: получен ID из аргументов: {user_id}")
            return user_id
    
    logger.warning(f"resolve_user_only: не удалось определить пользователя. args: {command_args}")
    return None
...
async def cmd_warn(message: Message, command: CommandObject):
    ...
    logger.info(f"cmd_warn: пользователь {message.from_user.id} выдаёт предупреждение {user_id} с причиной: {reason}")
...
async def cmd_mute(message: Message, command: CommandObject):
    ...
    user_id, reason = result
    logger.info(f"cmd_mute: пользователь {message.from_user.id} мутит {user_id} с причиной: {reason}")
...
async def cmd_tmute(message: Message, command: CommandObject):
    ...
    logger.info(f"cmd_tmute: пользователь {message.from_user.id} мутит {user_id} на {time_str} с причиной: {reason}")
...
async def cmd_unmute(message: Message, command: CommandObject):
    ...
    await unmute_user(message.chat.id, user_id, message.message_thread_id)
...
async def cmd_ban(message: Message, command: CommandObject):
    ...
    user_id, reason = result
    logger.info(f"cmd_ban: пользователь {message.from_user.id} банит {user_id} с причиной: {reason}")
...
async def cmd_tban(message: Message, command: CommandObject):
    ...
    logger.info(f"cmd_tban: пользователь {message.from_user.id} банит {user_id} на {time_str} с причиной: {reason}")
...
async def cmd_unban(message: Message, command: CommandObject):
    ...
    await unban_user(message.chat.id, user_id, message.message_thread_id)
