...
⚠️ <b>6. Беспричинное упоминание администраторов</b>
...
<i>Исключение: администраторы могут упоминать друг друга для рабочих вопросов</i>
...
APPEAL_MESSAGE = """
⚖️ <b>Система обжалования наказаний</b> ⚖️

🛡️ <b>Если вас наказали необоснованно</b> - вы имеете право подать жалобу!

📋 <b>Как подать жалобу:</b>

<u>🔹 На обычного пользователя:</u>
...
🤝 <b>Мы за справедливую модерацию!</b>
"""
...
last_appeal_message_id = None

async def send_rules_and_order_message():
    global last_rules_message_id, last_order_message_id, last_appeal_message_id
    ...
    try:
        ...
        if last_appeal_message_id:
            try:
                await bot.delete_message(CHAT_ID, last_appeal_message_id)
            except:
                pass
        ...
        # Добавляем сообщение о системе жалоб
        appeal_msg = await bot.send_message(CHAT_ID, APPEAL_MESSAGE, parse_mode="HTML")
        last_appeal_message_id = appeal_msg.message_id
...
def get_active_complaints() -> List[Dict[str, Any]]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        """SELECT id, user_id, admin_username, description, complaint_text, evidence, 
...
def get_complaint_by_id(complaint_id: int) -> Optional[Dict[str, Any]]:
    ...
    if result:
        return {
            "id": result[0],
            "user_id": result[1],
            "admin_username": result[2],
            "description": result[3],
            "complaint_text": result[4],
            "evidence": result[5],
            "created_at": result[6],
            "status": result[7],
            "handled_by": result[8],
            "handling_result": result[9]
        }
    return None
...
def get_complaints_keyboard() -> InlineKeyboardMarkup:
    ...
    for complaint in complaints:
        ...
        admin_username = complaint['admin_username']
        description = complaint['description']
        
        # Обрезаем длинные тексты для кнопки
...
def get_complaint_actions_keyboard(complaint_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard.row(
        InlineKeyboardButton(
            text="⚠️ Варн за некорректную жалобу",
...
async def cmd_start(message: Message):
    ...
    if is_user_blocked(message.from_user.id):
        await message.answer(
            ...
            "Для разблокировки свяжитесь с администраторами.",
            parse_mode="HTML"
        )
        return
...
async def cmd_appeal(message: Message):
    ...
    await message.answer(APPEAL_MESSAGE, parse_mode="HTML")
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Ищем ID жалобщика в тексте сообщения - ИСПРАВЛЕННЫЙ РЕГУЛЯРНЫЙ ВЫРАЖЕНИЕ
...
async def process_evidence(message: Message, state: FSMContext):
    ...
    complaint_message = (
        ...
        f"👤 <b>Жалобщик:</b> {data['username']}\n"
        f"👮 <b>Администратор:</b> {data['admin_username']}\n"
        f"📋 <b>Тип жалобы:</b> {data['description']}\n"
        f"📝 <b>Описание:</b>\n{data['complaint_text']}\n"
    ...
    if sent_count > 0:
        ...
        for admin_id in ADMIN_IDS:
            try:
                await bot.send_message(
                    chat_id=admin_id,
                    text=f"💬 <b>Действия по жалобе #{complaint_id} от {data['username']}</b>",
                    reply_markup=keyboard.as_markup(),
                    parse_mode="HTML"
                )
            except Exception as e:
    ...
    if sent_count > 0:
        await message.answer(
            f"✅ Ваша жалоба на администратора {data['admin_username']} успешно отправлена!\n\n"
            f"<b>Краткая информация:</b>\n"
            f"• ID жалобы: #{complaint_id}\n"
            f"• Тип: {data['description']}\n"
            f"• Адресат: {data['admin_username']}\n"
...
async def start_warn_false_report(callback: types.CallbackQuery, state: FSMContext):
    """Начинает процесс выдачи варна за ложную жалобу"""
    if not await is_bot_admin(callback.from_user.id):
        await callback.answer("❌ У вас нет прав для обработки жалоб.")
        return
    ...
    await state.update_data(complaint_id=complaint_id)
    await callback.message.answer(
        "🚫 <b>Выдача варна за ложную жалобу</b>\n\n"
        f"Жалоба #${complaint_id} от {complaint['username']}\n"
        "Укажите причину выдачи варна за ложную жалобу:",
        parse_mode="HTML"
    )
...
async def start_warn_incorrect_report(callback: types.CallbackQuery, state: FSMContext):
    """Начинает процесс выдачи варна за некорректную жалобу"""
    if not await is_bot_admin(callback.from_user.id):
        await callback.answer("❌ У вас нет прав для обработки жалоб.")
        return
    ...
    await state.update_data(complaint_id=complaint_id)
    await callback.message.answer(
        "⚠️ <b>Выдача варна за некорректную жалобу</b>\n\n"
        f"Жалоба #${complaint_id} от {complaint['username']}\n"
        "Укажите причину выдачи варна за некорректную жалобу:",
        parse_mode="HTML"
    )
...
async def process_false_report_reason(message: Message, state: FSMContext):
    """Обрабатывает причину варна за ложную жалобу"""
    data = await state.get_data()
    complaint_id = data['complaint_id']
    complaint = get_complaint_by_id(complaint_id)
    ...
    # Выдаем предупреждение пользователю в чате
    warn_success = await warn_user(CHAT_ID, complaint['user_id'], f"Ложная жалоба: {reason}")
    
    # Уведомляем жалобщика
    try:
        await bot.send_message(
            chat_id=complaint['user_id'],
            text=f"🚫 <b>Вам выдано предупреждение за ложную жалобу</b>\n\n"
                 f"🆔 <b>ID жалобы:</b> #{complaint_id}\n"
                 f"👮 <b>Администратор:</b> {complaint['admin_username']}\n"
                 f"📝 <b>Причина:</b> {reason}\n\n"
                 f"<i>Пожалуйста, будьте внимательнее при подаче жалоб.</i>",
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Не удалось уведомить жалобщика {complaint['user_id']}: {e}")
    ...
    # Уведомляем администратора
    status_text = "✅ Предупреждение выдано" if warn_success else "⚠️ Предупреждение не выдано (ошибка)"
    
    await message.answer(
        f"🚫 <b>Варн за ложную жалобу выдан</b>\n\n"
        f"🆔 <b>ID жалобы:</b> #{complaint_id}\n"
        f"👤 <b>Жалобщик:</b> {complaint['username']}\n"
        f"📝 <b>Причина:</b> {reason}\n"
...
async def process_incorrect_report_reason(message: Message, state: FSMContext):
    """Обрабатывает причину варна за некорректную жалобу"""
    data = await state.get_data()
    complaint_id = data['complaint_id']
    complaint = get_complaint_by_id(complaint_id)
    ...
    # Выдаем предупреждение пользователю в чате
    warn_success = await warn_user(CHAT_ID, complaint['user_id'], f"Некорректная жалоба: {reason}")
    
    # Получаем текущие предупреждения пользователя
    user_warns = get_user_warns(complaint['user_id'], CHAT_ID)
    warn_count = len(user_warns)
    
    # Проверяем, нужно ли блокировать пользователя (5 или более предупреждений)
    if warn_count >= 5:
        block_user(complaint['user_id'], "5 предупреждений за некорректные жалобы", message.from_user.id)
        block_message = "\n\n🚫 <b>Пользователь заблокирован в боте за 5 предупреждений!</b>"
    else:
        block_message = f"\n\n📊 <b>Текущее количество предупреждений:</b> {warn_count}/5"
    
    # Уведомляем жалобщика
    try:
        block_notice = "\n\n🚫 <b>Вы заблокированы в боте за 5 предупреждений!</b>" if warn_count >= 5 else ""
        await bot.send_message(
            chat_id=complaint['user_id'],
            text=f"⚠️ <b>Вам выдано предупреждение за некорректную жалобу</b>\n\n"
                 f"🆔 <b>ID жалобы:</b> #{complaint_id}\n"
                 f"👮 <b>Администратор:</b> {complaint['admin_username']}\n"
                 f"📝 <b>Причина:</b> {reason}\n"
                 f"📊 <b>Всего предупреждений:</b> {warn_count}/5{block_notice}\n\n"
                 f"<i>Пожалуйста, ознакомьтесь с правилами подачи жалоб.</i>",
            parse_mode="HTML"
        )
    except Exception as e:
        logger.error(f"Не удалось уведомить жалобщика {complaint['user_id']}: {e}")
    ...
    await message.answer(
        f"⚠️ <b>Варн за некорректную жалобу выдан</b>\n\n"
        f"🆔 <b>ID жалобы:</b> #{complaint_id}\n"
        f"👤 <b>Жалобщик:</b> {complaint['username']}\n"
        f"📝 <b>Причина:</b> {reason}\n"
        f"📊 <b>Статус:</b> {status_text}{block_message}",
        parse_mode="HTML",
        reply_markup=get_complaints_keyboard()
    )
