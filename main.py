...
@dp.message(ProductStates.waiting_for_name, F.chat.type == ChatType.PRIVATE)
async def adm_product_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Название: <b>{name}</b>\n\nТеперь введите <b>описание</b> товара (цена, объём, вкус и т.д.):",
        parse_mode="HTML"
    )


@dp.message(ProductStates.waiting_for_description, F.chat.type == ChatType.PRIVATE)
...
@dp.message(ProductStates.waiting_for_photo, F.photo, F.chat.type == ChatType.PRIVATE)
async def adm_product_photo(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Отправьте ещё фото (до 9 штук), или нажмите <b>/done</b> чтобы завершить добавление товара.",
        parse_mode="HTML"
    )


@dp.message(ProductStates.waiting_for_photo, F.chat.type == ChatType.PRIVATE)
async def adm_product_photo_wrong(message: Message):
    """Если прислали не фото"""
    await message.answer("⚠️ Пожалуйста, отправьте именно <b>фото</b> (изображение) товара:", parse_mode="HTML")


@dp.message(ProductStates.waiting_for_more_photos, F.photo, F.chat.type == ChatType.PRIVATE)
async def adm_product_more_photos(message: Message, state: FSMContext):
    """Принимать дополнительные фото"""
    data = await state.get_data()
    extra = data.get("extra_photos", [])
    if len(extra) >= 9:
        await message.answer("⚠️ Максимум 9 дополнительных фото. Отправьте /done для завершения.")
        return
    ...
    await message.answer(
        ...
        f"Отправьте ещё или нажмите <b>/done</b>.",
        parse_mode="HTML"
    )


@dp.message(ProductStates.waiting_for_more_photos, Command("done"), F.chat.type == ChatType.PRIVATE)
async def adm_product_done(message: Message, state: FSMContext):
    """Завершить добавление товара"""
    data = await state.get_data()
...
@dp.message(ProductStates.waiting_for_more_photos, F.chat.type == ChatType.PRIVATE)
async def adm_product_more_wrong(message: Message):
    """Если прислали не фото в режиме доп. фото"""
    await message.answer(
        "⚠️ Отправьте <b>фото</b> или нажмите <b>/done</b> для завершения.",
        parse_mode="HTML"
    )
