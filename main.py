...
async def process_safe_partner(message: Message, state: FSMContext):
    ...
    try:
        # Важно: передаём username с @ для get_chat
        ...
        logger.info(f"Найден пользователь: id={partner_id}, username=@{actual_username}")
        ...
        if "user not found" in error_msg or "not found" in error_msg:
            await message.answer(
                ...
                f"Возможные причины:\n"
                f"• Пользователь не запускал бота (не писал /start)\n"
                f"• Допущена опечатка в username\n"
                f"• Пользователь удалил аккаунт\n\n"
                f"📌 <b>Решение:</b>\n"
                f"1. Попросите партнёра написать боту: <code>/start</code>\n"
            ...
            await message.answer(
                f"❌ <b>Пользователь @{partner_username} заблокировал бота.</b>\n\n"
                f"Попросите партнёра разблокировать бота и написать /start.\n\n"
                f"🔄 Введите username снова:",
                parse_mode="HTML"
            )
        else:
            await message.answer(
                f"❌ <b>Ошибка при поиске пользователя:</b>\n"
                f"<code>{str(e)[:200]}</code>\n\n"
                f"Попробуйте ввести username ещё раз:",
                parse_mode="HTML"
            )
        return
    ...
    await message.answer(
        ...
        f"<i>Создайте группу, добавьте в неё второго участника и бота, "
        f"затем отправьте ссылку-приглашение.</i>",
        parse_mode="HTML"
    )
...
async def sd_initiate_payment(callback: CallbackQuery):
    ...
    try:
        ...
        payment_params = {
            ...
            "comment": (deal.get("description") or "")[:100],
            "need-fio": "false",
            "need-email": "false",
            "need-phone": "false",
            "need-address": "false",
        ...
        await callback.message.edit_text(
            ...
            f"💼 <b>Комиссия гаранта:</b> {guarantor_fee:.2f} руб.\n\n"
    ...
    try:
        ...
        if YOO_MONEY_ACCESS_TOKEN:
            async with _aiohttp.ClientSession() as session:
                ...
                async with session.get(f"{YOO_BASE_URL}/operation-history", headers=headers, params=params) as resp:
                    if resp.status == 200:
...
async def sd_withdraw_request(callback: CallbackQuery, state: FSMContext):
    """Запрос на вывод средств из баланса гаранта"""
    await callback.answer()
    user_id = callback.from_user.id
    ...
    await state.set_state(SafeDealWithdrawStates.waiting_for_wallet)
    await state.update_data(sd_withdraw_amount=balance)
    
    await callback.message.edit_text(
        f"💰 <b>Заявка на вывод средств</b>\n\n"
        f"Доступно для вывода: {balance:.2f} руб.\n\n"
        f"Введите номер кошелька ЮMoney (11–16 цифр):",
        parse_mode="HTML"
    )


@dp.message(SafeDealWithdrawStates.waiting_for_wallet)
async def sd_process_wallet(message: Message, state: FSMContext):
    """Обработка ввода кошелька для вывода"""
    wallet = message.text.strip()
    if not re.match(r'^\d{11,16}$', wallet):
        await message.answer("❌ Неверный формат. Введите 11–16 цифр:")
        return
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, 'pending', ?, ?)
    """, (user_id, amount, datetime.now(), wallet))
    conn.commit()
    conn.close()
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                ...
                f"👤 Пользователь: @{message.from_user.username or 'No username'} (ID: {user_id})\n"
    ...
    await message.answer(
        f"✅ Заявка на вывод {amount:.2f} руб. на кошелёк {wallet} принята!\n\n"
