...
def init_db():
    ...
    # Восстановление повреждённых таблиц (malformed database schema / invalid rootpage)
    try:
        ...
        for (table_name,) in tables:
            try:
                ...
                if "malformed" in str(e) or "invalid rootpage" in str(e):
                    logger.warning(f"Повреждённая таблица '{table_name}' — удаляем и пересоздаём")
...
# Основная функция
async def run_web():
    """Запускает FastAPI (web.py) через uvicorn в том же event loop."""
    import uvicorn
...
async def main():
    logger.info("Запуск бота + веб-панели...")
    ...
    # Запускаем бота и веб-панель параллельно в одном процессе
