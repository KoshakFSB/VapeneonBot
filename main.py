...
async def cmd_report(message: Message, command: CommandObject):
    ...
    if message.reply_to_message:
        # Жалоба через ответ на сообщение
        ...
        # Жалоба через упоминание пользователя
        args = command.args.split(maxsplit=1) if command.args else []
        if not args:
    ...
    # Экранируем reason для callback_data (убираем пробелы и специальные символы)
    safe_reason = re.sub(r'[^a-zA-Z0-9а-яА-Я]', '_', reason)[:30]  # Ограничиваем длину
    ...
    report_text = (
        f"🚨 <b>Новая жалоба</b>\n\n"
        f"👤 <b>Жалоба от:</b> {reporter_mention}\n"
        f"⚠️ <b>На пользователя:</b> {reported_mention}\n"
        f"📝 <b>Причина:</b> {reason}\n"
    )
    
    if reported_message_id:
        report_text += f"📎 <b>Сообщение:</b> <a href='https://t.me/c/{str(CHAT_ID)[4:]}/{reported_message_id}'>ссылка</a>"
    ...
    keyboard.row(
        InlineKeyboardButton(
            ...
            callback_data=f"warn_{reported_user_id}_{safe_reason}"
        ),
        InlineKeyboardButton(
            text="🔇 Мут", 
            callback_data=f"mute_{reported_user_id}_{safe_reason}"
        )
    )
    keyboard.row(
        InlineKeyboardButton(
            text="🚫 Бан", 
            callback_data=f"ban_{reported_user_id}_{safe_reason}"
        ),
        InlineKeyboardButton(
            text="❌ Отклонить", 
            callback_data=f"dismiss_{reported_user_id}"
        )
    )
    ...
    if sent_count > 0:
        await message.answer(
            f"✅ Ваша жалоба на {reported_mention} отправлена {sent_count} администраторам.",
            parse_mode="HTML"
        )
    else:
        await message.answer(
            "❌ Не удалось отправить жалобу администраторам.",
            parse_mode="HTML"
        )

@dp.callback_query(F.data.startswith(("warn_", "mute_", "ban_", "dismiss_")))
async def handle_report_callback(callback: types.CallbackQuery):
    """Обработчик действий по жалобам"""
    data = callback.data
    admin_id = callback.from_user.id
    
    if not await is_bot_admin(admin_id):
    ...
    try:
        if data.startswith("warn_"):
            ...
            parts = data.split('_', 2)  # Разделяем только на 2 части
            user_id = int(parts[1])
            reason = parts[2] if len(parts) > 2 else "Жалоба"
            
            await warn_user(CHAT_ID, user_id, reason)
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)}",
                reply_markup=None  # Убираем клавиатуру после действия
            )
            ...
            await mute_user(CHAT_ID, user_id, MUTE_DURATION, reason)
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)}",
                reply_markup=None
            )
            ...
            await ban_user(CHAT_ID, user_id, None, reason)
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)}",
                reply_markup=None
            )
            ...
            await callback.message.edit_text(
                f"❌ {callback.message.text}\n\n👮 Отклонено: {await get_user_mention(admin_id)}",
                reply_markup=None
            )
