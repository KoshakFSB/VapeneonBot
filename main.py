...
async def cmd_report(message: Message, command: CommandObject):
    ...
    report_text = (
        f"🚨 <b>Новая жалоба из чата</b>\n\n"
        f"👤 <b>Жалоба от:</b> {reporter_mention}\n"
        f"⚠️ <b>На пользователя:</b> {reported_mention}\n"
        f"📝 <b>Причина:</b> {reason}\n"
        f"💬 <b>Чат:</b> {message.chat.title if message.chat.title else 'Групповой чат'}\n"
    )
    ...
    # Отправляем жалобу в группу вместо личных сообщений администраторам
    success = False
    try:
        report_msg = await message.chat.send_message(
            report_text,
            parse_mode="HTML",
        ...
        # Удаляем сообщение с жалобой через 5 минут
        await asyncio.sleep(300)
        await delete_message(message.chat.id, report_msg.message_id)
        
    except Exception as e:
        logger.error(f"Не удалось отправить жалобу в чат: {e}")
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Проверяем права администратора в указанном чате
        if not await is_chat_admin(admin_id, chat_id):
            await callback.answer("❌ У вас нет прав администратора в этом чате.")
            return
        ...
        if action == "warn":
            ...
            action_text = "выдано предупреждение"
            ...
            action_text = f"мут на {duration_str}"
