...
async def parse_command_args_v2(message: Message, command: CommandObject, required_args: int = 0, has_time: bool = False) -> Optional[Dict]:
    ...
    Возвращает словарь с user_id, reason, duration (если есть время).
    """
    result = {
        'user_id': None,
        'reason': 'Не указана',
    ...
    if message.reply_to_message:
        ...
        # Парсим аргументы при ответе
        args = command.args.strip().split()
        
        if has_time and len(args) >= 1:
            # Для команд с временем: /tmute 1d Причина
            time_str = args[0]
    ...
    # Определяем, что является идентификатором пользователя
    ...
    if user_identifier.isdigit():
        ...
        try:
            ...
            logger.error(f"Ошибка получения пользователя по юзернейму: {e}")
    ...
    if has_time:
        ...
        result['reason'] = ' '.join(args[2:]) if len(args) > 2 else 'Не указана'
    else:
