...
async def cmd_admin_quest(message: Message):
    """Запускает набор администраторов — публикует анкету с кнопкой подачи заявки"""
    if not await is_owner(message.from_user.id):
        await message.answer("❌ Только владелец может запускать набор администраторов.")
        return
