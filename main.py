...
RULES_INTERVAL_HOURS = float(os.getenv("RULES_INTERVAL_HOURS", "2.5"))
...
async def send_rules_and_order_message():
    ...
    try:
        # Проверяем текущее время в московском часовом поясе - не отправляем с 00:00 до 06:00 MSK
        ...
        if night_start <= current_time <= night_end:
            logger.info(f"Ночное время MSK ({current_time}) - пропускаем отправку правил")
            return
...
async def rules_scheduler():
    """Планировщик для отправки правил каждые 2.5 часа (кроме ночного времени 00:00-06:00 MSK)"""
    while True:
        try:
            # Проверяем текущее время в московском часовом поясе - не отправляем с 00:00 до 06:00 MSK
            current_time = get_moscow_time().time()
            ...
            if not (night_start <= current_time <= night_end):
                await send_rules_and_order_message()
                logger.info("Правила и заказ отправлены по расписанию (MSK)")
            else:
                logger.info(f"Ночное время MSK ({current_time}) - пропускаем отправку правил")
...
async def cmd_clear_chat(message: Message, command: CommandObject):
    ...
    if not command.args:
        await message.answer(
            ...
            "Пример: <code>/cc 100</code> - удалит 100 сообщений",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        if count <= 0:
            ...
            return
        if count > 1000:  # Ограничение для безопасности
            await message.answer("❌ Максимальное количество сообщений для удаления - 1000.")
            return
    except ValueError:
    ...
    confirm_msg = await message.answer(
        f"🧹 <b>Начинаю очистку {count} сообщений...</b>\n\n"
    ...
    try:
        deleted_count = 0
        skipped_count = 0
        current_message_id = message.message_id - 1  # Начинаем с сообщения перед командой
        ...
        # Удаляем сообщения пока не достигнем нужного количества или не дойдем до начала чата
        while (current_message_id > 0 and 
               deleted_count + skipped_count < count and 
               deleted_count + skipped_count < 1000):
            
            try:
                # Проверяем, не является ли сообщение закрепленным
                if pinned_message_id and current_message_id == pinned_message_id:
                    skipped_count += 1
                ...
                try:
                    chat_message = await bot.get_chat_message(message.chat.id, current_message_id)
                    
                    # Пропускаем сообщения от каналов
                    if chat_message.sender_chat and chat_message.sender_chat.type == ChatType.CHANNEL:
                        skipped_count += 1
                        current_message_id -= 1
                        continue
                        
                    # Пропускаем сообщения от ботов (кроме самого бота)
                    if (chat_message.from_user and chat_message.from_user.is_bot and 
                        chat_message.from_user.id != (await bot.get_me()).id):
                        skipped_count += 1
                        current_message_id -= 1
                        continue
                        
                except Exception as e:
                    # Если не можем получить информацию о сообщении, все равно пытаемся удалить
                    pass
            ...
            if (deleted_count + skipped_count) % 50 == 0:
                try:
        ...
        if pinned_message_id:
            result_text += f"• 📌 Сохранено закрепленное сообщение\n"
        
        result_text += "<i>Пропущены сообщения от каналов, закрепленные сообщения и сообщения ботов</i>"
        ...
        # Удаляем сообщение с результатами через 30 секунд
...
async def main():
    ...
    # Проверяем время в московском часовом поясе при запуске - не отправляем правила если ночное время MSK
    current_time = get_moscow_time().time()
    night_start = datetime.strptime("00:00", "%H:%M").time()
    night_end = datetime.strptime("06:00", "%H:%M").time()
    
    if not (night_start <= current_time <= night_end):
        logger.info("Дневное время MSK - отправляем правила при запуске")
        await send_rules_and_order_message()
    else:
        logger.info(f"Ночное время MSK ({current_time}) - пропускаем отправку правил при запуске")
