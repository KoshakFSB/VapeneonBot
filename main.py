...
async def safe_deal_create_redirect(callback: CallbackQuery):
    """Перенаправление на создание сделки"""
    await callback.answer("Функция создания сделок доступна в боте-гаранте.", show_alert=True)


@dp.callback_query(F.data == "safe_deal_my_deals")
async def safe_deal_my_deals_redirect(callback: CallbackQuery):
    """Перенаправление на мои сделки"""
    await callback.answer("Функция просмотра сделок доступна в боте-гаранте.", show_alert=True)
...
async def safe_deal_balance_redirect(callback: CallbackQuery):
    ...
    await callback.answer("Функция просмотра баланса доступна в боте-гаранте.", show_alert=True)


@dp.callback_query(F.data == "safe_deal_reviews")
async def safe_deal_reviews_redirect(callback: CallbackQuery):
    """Перенаправление на отзывы"""
    await callback.answer("Функция просмотра отзывов доступна в боте-гаранте.", show_alert=True)


@dp.callback_query(F.data == "safe_deal_service_review")
async def safe_deal_service_review_redirect(callback: CallbackQuery):
    """Перенаправление на отзыв о сервисе"""
    await callback.answer("Функция отзыва о сервисе доступна в боте-гаранте.", show_alert=True)
...
async def send_periodic_info():
    """Отправка правил, информации о заказах и инструкции по жалобам в чат каждые 3 часа."""
    # Ждём 10 секунд после запуска, чтобы бот успел инициализироваться
    await asyncio.sleep(10)
    while True:
        try:
            # Отправляем три сообщения без указания темы (темы отключены)
            ...
            # Ждём 3 часа до следующей отправки
