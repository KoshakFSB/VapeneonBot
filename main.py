...
async def cmd_report(message: Message, command: CommandObject):
    ...
    admin_ids = list(set(get_all_admins() + ADMIN_IDS))  # Убираем дубликаты
    
    # Отправляем жалобу всем администраторам в ЛС
    success_count = 0
    for admin_id in admin_ids:
        try:
            # Пропускаем самого себя и невалидные ID
            ...
            # Удаляем сообщение с жалобой через 10 минут
    ...
    # Подтверждение пользователю (удаляется через 10 секунд)
    if success_count > 0:
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        # Обрабатываем разные форматы действий
        ...
        # Проверяем права администратора - используем chat_id из callback
