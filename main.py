...
async def cmd_report(message: Message, command: CommandObject):
    ...
    keyboard = InlineKeyboardBuilder()
    keyboard.row(
        InlineKeyboardButton(text="⚠️ Предупреждение", callback_data=f"warn_{reported_user_id}_{reason}"),
        InlineKeyboardButton(text="🔇 Мут", callback_data=f"mute_{reported_user_id}_{reason}")
    )
    keyboard.row(
        InlineKeyboardButton(text="🚫 Бан", callback_data=f"ban_{reported_user_id}_{reason}"),
        InlineKeyboardButton(text="❌ Отклонить", callback_data=f"dismiss_{reported_user_id}")
    )
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    if data.startswith("warn_"):
        ...
        parts = data.split('_')
        user_id = int(parts[1])
        reason = '_'.join(parts[2:]) if len(parts) > 2 else "Жалоба"
