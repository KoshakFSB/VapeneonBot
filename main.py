...
async def resolve_user_reference(message: Message, command_args: str = None) -> Optional[Tuple[int, str]]:
    ...
    if message.reply_to_message:
        ...
        logger.info(f"Found user via reply: {user_id}")
        
        # Проверяем, что это не бот
        if message.reply_to_message.from_user.is_bot:
            await message.answer("❌ Нельзя выдавать наказания ботам.")
            return None
        # Если есть аргументы, используем их как причину
        if command_args:
            reason = command_args
    ...
    # Затем проверяем аргументы команды
        ...
        logger.info(f"Processing command_args: {command_args}")
        
        # Разбиваем аргументы, сохраняя остаток как причину
        args_parts = command_args.strip().split(maxsplit=1)
        logger.info(f"args_parts: {args_parts}")
        
        if len(args_parts) >= 1:
            user_part = args_parts[0].strip()
            logger.info(f"user_part: {user_part}")
            
            # Проверяем различные форматы
            if user_part.isdigit():
                user_id = int(user_part)
                logger.info(f"user_id from digit: {user_id}")
            elif user_part.startswith('@'):
                try:
                    user = await bot.get_chat(user_part)
                    user_id = user.id
                    logger.info(f"user_id from username: {user_id}")
                except Exception as e:
                    logger.error(f"Error getting user by username: {e}")
                    await message.answer(f"❌ Пользователь {user_part} не найден.")
                    return None
            else:
                # Может быть ID без символов
                try:
                    user_id = int(user_part)
                    logger.info(f"user_id from int conversion: {user_id}")
                except ValueError:
            ...
            try:
                ...
                logger.error(f"Error checking if user is bot: {e}")
                # Если не можем получить информацию, всё равно продолжаем
                pass
