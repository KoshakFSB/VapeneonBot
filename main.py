...
async def cmd_report(message: Message, command: CommandObject):
    ...
    # Упрощаем callback_data - используем только ID и тип действия
    callback_data = f"report:{reported_user_id}:{message.message_id}"
    ...
    keyboard.row(
        InlineKeyboardButton(
            ...
            callback_data=f"warn:{reported_user_id}"
        ),
        InlineKeyboardButton(
            text="🔇 Мут 1д", 
            callback_data=f"mute1:{reported_user_id}"
        )
    )
    keyboard.row(
        InlineKeyboardButton(
            text="🔇 Мут 7д", 
            callback_data=f"mute7:{reported_user_id}"
        ),
        InlineKeyboardButton(
            text="🚫 Бан", 
            callback_data=f"ban:{reported_user_id}"
        )
    )
    keyboard.row(
        InlineKeyboardButton(
            text="❌ Отклонить", 
            callback_data=f"dismiss:{reported_user_id}"
        )
    )
    
    # Отправляем жалобу в чат для администраторов (вместо личных сообщений)
    try:
        report_msg = await bot.send_message(
            CHAT_ID,  # Отправляем в основной чат
            report_text,
            parse_mode="HTML",
        ...
        await message.answer(
            f"✅ Ваша жалоба на {reported_mention} отправлена администраторам.",
            parse_mode="HTML"
        )
...
async def handle_report_callback(callback: types.CallbackQuery):
    ...
    # Проверяем права в чате, а не в боте
    if not await is_chat_admin(admin_id, callback.message.chat.id):
    ...
    try:
        ...
        if action == "warn":
            await warn_user(CHAT_ID, user_id, "Жалоба от пользователя")
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)} - выдано предупреждение",
                reply_markup=None
            )
            ...
            await mute_user(CHAT_ID, user_id, duration, "Жалоба от пользователя")
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)} - мут на {duration_str}",
                reply_markup=None
            )
            
        elif action == "ban":
            await ban_user(CHAT_ID, user_id, None, "Жалоба от пользователя")
            await callback.message.edit_text(
                f"✅ {callback.message.text}\n\n👮 Принято: {await get_user_mention(admin_id)} - бан",
                reply_markup=None
            )
