...
def _repair_db_if_needed(db_path: str):
    ...
    try:
        ...
        # Быстрая проверка: пробуем прочитать sqlite_master
    ...
    logger.warning("Обнаружена повреждённая БД — запускаем восстановление через dump...")
    ...
    try:
        ...
        logger.info(f"БД восстановлена. Старая сохранена как {backup_path}")
    except Exception as e:
...
async def admin_quest_process_age(message: Message, state: FSMContext):
    ...
    await message.answer("Вопрос 3/3: Сколько времени в день вы готовы уделять слежке за чатом?")
...
async def admin_quest_process_time(message: Message, state: FSMContext):
    ...
    summary = (
        ...
        f"3️⃣ Время на слежку: {data.get('quest_time')}"
    )
...
async def admin_quest_process_time_invalid(message: Message):
    await message.answer("Пожалуйста, ответьте текстом. Вопрос 3/3: Сколько времени в день вы готовы уделять слежке за чатом?")
