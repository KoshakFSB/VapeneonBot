...
async def parse_command_args_v2(message: Message, command: CommandObject, required_args: int = 0, has_time: bool = False) -> Optional[Dict]:
    ...
    # Проверяем, является ли первый аргумент временем (для случаев, когда пользователь указан в reply)
    # Но здесь нет reply, значит первый аргумент - пользователь
    ...
    # Парсим остальные аргументы в зависимости от наличия времени
    if has_time:
