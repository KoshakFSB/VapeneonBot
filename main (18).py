...
async def cmd_start_with_reviews(message: Message, command: CommandObject):
    ...
    if command.args == "onboarding":
        ...
        await message.answer("✅ Ты уже прошёл обучение! Добро пожаловать.", reply_markup=get_main_keyboard())
        return
...
async def btn_complain_admin(message: Message, state: FSMContext):
    if is_user_blocked(message.from_user.id):
        await message.answer("🚫 Вы заблокированы в боте.", reply_markup=get_main_keyboard())
        return
...
async def btn_view_complaints(message: Message):
    if not await is_bot_admin(message.from_user.id):
        await message.answer("❌ У вас нет прав для просмотра жалоб.", reply_markup=get_main_keyboard())
        return
    complaints = get_active_complaints()
...
async def btn_start_onboarding(message: Message, state: FSMContext):
    ...
    if is_onboarding_completed(user_id):
        await message.answer("✅ Ты уже прошёл обучение! Если хочешь повторить — напиши /start.", reply_markup=get_main_keyboard())
        return
    remaining = get_onboarding_cooldown_remaining(user_id)
    if remaining:
