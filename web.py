...
async def lifespan(app: FastAPI):
    _init_db()
    try:
        _ensure_services_tables()
    except Exception as e:
        log.error(f"Ошибка создания таблиц платных услуг: {e}")
    asyncio.create_task(_reconcile_pending_service_orders())
    asyncio.create_task(_uoo_expiry_checker())
    log.info("Сайт VapeNeon запущен на :8080")
    yield

app = FastAPI(title="VapeNeon", lifespan=lifespan)

# ВРЕМЕННЫЙ маршрут для получения OAuth-кода от ЮMoney.
# Удалите этот блок после того, как получите access_token.
@app.get("/yoomoney-oauth-catch")
async def _yoomoney_oauth_catch(code: str = None):
    if not code:
        return HTMLResponse("<h2>Код не найден в параметрах запроса (?code=...)</h2>")
    return HTMLResponse(f"<h2>Ваш code:</h2><pre style='font-size:18px;user-select:all'>{code}</pre>"
                         f"<p>Скопируйте его и используйте в запросе на обмен на access_token.</p>")
...
async def tg_lift_mute(chat_id: int, user_id: int) -> bool:
    """Снимает мут через прямой вызов Bot API (полные права на отправку сообщений)."""
    if not BOT_TOKEN:
        return False
    permissions = {
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            data = resp.json()
            if not data.get("ok"):
                log.error(f"restrictChatMember (снятие мута) не удалось: {data}")
            return bool(data.get("ok"))
        except Exception as e:
            log.error(f"Ошибка снятия мута через Bot API: {e}")
            return False


async def tg_set_member_tag(chat_id: int, user_id: int, tag: str) -> bool:
    """Ставит (или снимает, если tag='') тег обычному участнику через Bot API
    setChatMemberTag (Bot API 9.5+). Бот должен быть админом чата с правом
    can_manage_tags. Не выдаёт участнику никаких прав — это просто подпись
    рядом с именем."""
    if not BOT_TOKEN:
        return False
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            data = resp.json()
            if not data.get("ok"):
                log.error(f"setChatMemberTag не удалось (user={user_id}, tag='{tag}'): {data}")
            return bool(data.get("ok"))
        except Exception as e:
            log.error(f"Ошибка setChatMemberTag через Bot API: {e}")
            return False


async def tg_lift_ban(chat_id: int, user_id: int) -> bool:
    """Снимает бан через прямой вызов Bot API."""
    if not BOT_TOKEN:
        return False
    async with httpx.AsyncClient(timeout=10) as client:
        try:
            ...
            data = resp.json()
            if not data.get("ok"):
                log.error(f"unbanChatMember (снятие бана) не удалось: {data}")
            return bool(data.get("ok"))
        except Exception as e:
            log.error(f"Ошибка снятия бана через Bot API: {e}")
            return False
...
def optional_user(request: Request) -> Optional[dict]:
    """Как require_user, но не требует авторизации — возвращает None, если
    пользователь не вошёл на сайт. Используется услугами, которые можно
    заказать и без входа (например, «Закрепление объявления»)."""
    token = request.cookies.get("vn_user_session")
    return get_user_session(token) if token else None
...
PIN_PRICE_PER_HOUR = 20.0
AUTOPOST_TARIFFS = {
    "1d": {"days": 1, "price": 10.0, "label": "1 день"},
    "7d": {"days": 7, "price": 50.0, "label": "7 дней"},
    "30d": {"days": 30, "price": 300.0, "label": "30 дней"},
}
BLOCK_TARIFFS = {
    # "days" — сколько дней действия наказания покрывает тариф.
    # None означает "покрывает любой срок, включая бессрочное наказание".
    # Используется, чтобы нельзя было снять длинный мут/бан дешёвым тарифом.
    "mute_1d":  {"kind": "mute", "price": 15.0,  "label": "Мут — 1 день",    "days": 1},
    "mute_2d":  {"kind": "mute", "price": 27.0,  "label": "Мут — 2 дня",     "days": 2},
    "mute_3d":  {"kind": "mute", "price": 41.0,  "label": "Мут — 3 дня",     "days": 3},
    "mute_7d":  {"kind": "mute", "price": 95.0,  "label": "Мут — неделя",    "days": 7},
    "ban_30d":  {"kind": "ban",  "price": 150.0, "label": "Бан — 30 дней",   "days": 30},
    "ban_perm": {"kind": "ban",  "price": 275.0, "label": "Бан — навсегда",  "days": None},
    "warn_1":   {"kind": "warn", "price": 8.0,   "label": "Снятие 1 варна",  "days": None},
}

# «УОО» — урезанные ограничения на отправку объявлений. Пока подписка активна,
# участнику ставится тег «УОО» рядом с именем в чате (без прав админа) —
# админы по нему вручную применяют льготные лимиты вместо стандартных.
UOO_TAG_TEXT = "УОО"
UOO_TARIFFS = {
    "1d":  {"days": 1,  "price": 8.0,   "label": "1 день"},
    "7d":  {"days": 7,  "price": 50.0,  "label": "7 дней"},
    "30d": {"days": 30, "price": 156.0, "label": "30 дней"},
}


def _remaining_days(expires_at) -> Optional[int]:
    """Сколько дней (округляя вверх) осталось до окончания наказания.
    None, если наказание бессрочное (expires_at is NULL)."""
...
def _cheapest_sufficient_tariff(kind: str, remaining_days: Optional[int]):
    """Находит самый дешёвый тариф выбранного вида, который покрывает остаток наказания."""
    candidates = [
        (key, t) for key, t in BLOCK_TARIFFS.items()
        if t["kind"] == kind and (
            t["days"] is None or (remaining_days is not None and t["days"] >= remaining_days)
        )
    ]
    if not candidates:
        return None
    candidates.sort(key=lambda kt: kt[1]["price"])
    return candidates[0]


CHAT_INVITE_LINK = os.getenv("CHAT_INVITE_LINK", "https://t.me/vapeneonVP")

YOO_MONEY_ACCOUNT = os.getenv("YOO_MONEY_ACCOUNT", "")
YOO_MONEY_ACCESS_TOKEN = os.getenv("YOO_MONEY_ACCESS_TOKEN", "")


def _ensure_services_tables():
    with db() as conn:
        conn.execute("""
...
def _resolve_guest_contact(contact: str) -> tuple:
    """Для покупки услуг без авторизации: превращает введённый пользователем
    @username или числовой Telegram ID в (user_id, username).
    Для username ищет в bot_users — там оказывается любой, кто хоть раз
    написал боту (например, /start). Бросает HTTPException, если не нашли."""
    contact = (contact or "").strip().lstrip("@")
    if not contact:
        raise HTTPException(400, "Укажите ваш Telegram username или ID (либо войдите на сайт)")
    if contact.isdigit():
        return int(contact), ""
    with db() as conn:
        bu = one(conn, "SELECT * FROM bot_users WHERE lower(username)=%s", (contact.lower(),))
    if not bu:
        raise HTTPException(
            400,
            "Не нашли такого пользователя. Сначала откройте бота в Telegram и нажмите «Старт», "
            "затем попробуйте снова (или войдите на сайт)."
        )
    return bu["user_id"], bu.get("username") or contact


def _parse_pin_message_id(link: str) -> Optional[int]:
    """Достаёт ID сообщения из ссылки вида t.me/c/123/456 или t.me/username/456."""
    import re as _re
    m = _re.search(r"(\d+)/?$", (link or "").strip())
    return int(m.group(1)) if m else None


async def _yoomoney_check_label(label: str, expected_amount: float) -> bool:
    """Проверяет, поступил ли платёж с данной меткой (label) через ЮMoney API."""
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.post(
                ...
                headers=headers,
                data={"records": 50, "details": "true"},
            )
            if resp.status_code != 200:
                log.error(f"YooMoney API вернул {resp.status_code}: {resp.text[:300]}")
                return False
            data = resp.json()
        ...
        log.error(f"Ошибка проверки оплаты ЮMoney (label={label}): {e}")
        return False


def _build_quickpay_url(amount: float, label: str, target: str, comment: str) -> str:
...
class PinOrderIn(BaseModel):
    hours: float
    message_link: str
    # Указывается, только если пользователь не авторизован на сайте: его
    # Telegram @username (должен был хоть раз написать боту /start) или
    # числовой Telegram ID.
    contact: Optional[str] = None


class AutopostOrderIn(BaseModel):
    tariff: str


class UnblockOrderIn(BaseModel):
    tariff: str


@app.get("/api/services/config")
async def services_config():
    """Отдаёт актуальные тарифы для фронтенда."""
    return {
        "pin_price_per_hour": PIN_PRICE_PER_HOUR,
        "autopost_tariffs": AUTOPOST_TARIFFS,
        "block_tariffs": BLOCK_TARIFFS,
        "uoo_tariffs": UOO_TARIFFS,
    }


@app.post("/api/services/pin/order")
async def create_pin_order(body: PinOrderIn, request: Request):
    # Услуга «Закрепление объявления» доступна и без авторизации на сайте:
    # если пользователь вошёл — берём его из сессии, иначе распознаём его
    # по указанному username/ID (contact).
    u = optional_user(request)
    if u:
        target_user_id, target_username = u["tg_id"], u.get("username", "")
    else:
        target_user_id, target_username = _resolve_guest_contact(body.contact)
    _ensure_services_tables()

    if body.hours <= 0 or body.hours > 720:
        raise HTTPException(400, "Некорректное количество часов")
    link = (body.message_link or "").strip()
    if not link or "t.me" not in link:
        raise HTTPException(400, "Укажите корректную ссылку на сообщение-объявление в Telegram")
    if _parse_pin_message_id(link) is None:
        raise HTTPException(400, "Не удалось распознать ссылку на сообщение")

    amount = round(float(body.hours) * PIN_PRICE_PER_HOUR, 2)

    with db() as conn:
        row = one(conn, """
            INSERT INTO paid_services_orders (service_type, user_id, username, amount, label, params, status)
            VALUES ('pin', %s, %s, %s, 'pending_label', %s, 'pending')
            RETURNING id
        """, (target_user_id, target_username, amount, json.dumps({"hours": body.hours, "message_link": link})))
        order_id = row["id"]
        label = f"service_pin_{order_id}"
        conn.execute("UPDATE paid_services_orders SET label=%s WHERE id=%s", (label, order_id))

    payment_url = _build_quickpay_url(
        amount, label,
        target=f"Закрепление объявления на {body.hours:g} ч.",
        comment=f"Услуга: закрепление объявления, заказ #{order_id}",
    )
    return {"order_id": order_id, "amount": amount, "payment_url": payment_url}


@app.post("/api/services/pin/check/{order_id}")
async def check_pin_order(order_id: int, request: Request):
    # Без авторизации право проверять заказ подтверждается знанием order_id
    # (аналог гостевого чек-аута); если пользователь всё же вошёл — дополнительно
    # сверяем, что заказ действительно его.
    u = optional_user(request)
    with db() as conn:
        order = one(conn, "SELECT * FROM paid_services_orders WHERE id=%s AND service_type='pin'", (order_id,))
    if not order:
        raise HTTPException(404, "Заказ не найден")
    if u and order["user_id"] != u["tg_id"]:
        raise HTTPException(403, "Это не ваш заказ")

    if order["status"] == "paid":
        return {"status": "paid"}

    confirmed = await _yoomoney_check_label(order["label"], float(order["amount"]))
    if not confirmed:
    ...
    message_id = _parse_pin_message_id(message_link)

    with db() as conn:
        conn.execute(
            "UPDATE paid_services_orders SET status='paid', paid_at=now() WHERE id=%s",
            (order_id,)
        )
        conn.execute("""
            INSERT INTO pin_queue (order_id, user_id, chat_id, message_id, message_link, hours, status)
            VALUES (%s, %s, %s, %s, %s, %s, 'queued')
        """, (order_id, order["user_id"], CHAT_ID, message_id, message_link, hours))

    await tg_send(
        order["user_id"],
        "✅ <b>Оплата получена!</b>\n\n"
        f"Ваша заявка на закрепление объявления на <b>{hours:g} ч.</b> принята.\n"
        "Если сейчас в чате уже что-то закреплено платно — вы встанете в очередь, "
        "и закрепление начнётся автоматически сразу после освобождения места."
    )
    return {"status": "paid"}


@app.post("/api/services/autopost/order")
async def create_autopost_order(body: AutopostOrderIn, request: Request):
    u = require_user(request)
    _ensure_services_tables()

    tariff = AUTOPOST_TARIFFS.get(body.tariff)
    if not tariff:
    ...
    with db() as conn:
        row = one(conn, """
            INSERT INTO paid_services_orders (service_type, user_id, username, amount, label, params, status)
            VALUES ('autopost', %s, %s, %s, 'pending_label', %s, 'pending')
            RETURNING id
        """, (u["tg_id"], u.get("username", ""), amount, json.dumps({"tariff": body.tariff, "days": tariff["days"]})))
        order_id = row["id"]
        label = f"service_autopost_{order_id}"
        conn.execute("UPDATE paid_services_orders SET label=%s WHERE id=%s", (label, order_id))

    payment_url = _build_quickpay_url(
        amount, label,
        target=f"Подписка на автопостинг ({tariff['label']})",
        comment=f"Услуга: автопостинг {tariff['label']}, заказ #{order_id}",
    )
    return {"order_id": order_id, "amount": amount, "payment_url": payment_url}


@app.post("/api/services/autopost/check/{order_id}")
async def check_autopost_order(order_id: int, request: Request):
    u = require_user(request)
    with db() as conn:
        order = one(conn, "SELECT * FROM paid_services_orders WHERE id=%s AND service_type='autopost'", (order_id,))
    if not order:
        raise HTTPException(404, "Заказ не найден")
    if order["user_id"] != u["tg_id"]:
        raise HTTPException(403, "Это не ваш заказ")
    ...
    with db() as conn:
        ...
        existing = one(conn, "SELECT * FROM autopost_subscriptions WHERE user_id=%s", (u["tg_id"],))
        now = datetime.now()
        if existing and existing["active"] and existing["expires_at"] and existing["expires_at"] > now:
            new_expiry = existing["expires_at"] + timedelta(days=days)
        else:
            new_expiry = now + timedelta(days=days)
        conn.execute("""
            INSERT INTO autopost_subscriptions (user_id, username, expires_at, active, notified_expiry)
            VALUES (%s, %s, %s, TRUE, FALSE)
            ON CONFLICT (user_id) DO UPDATE SET
                username=excluded.username, expires_at=excluded.expires_at,
                active=TRUE, notified_expiry=FALSE
        """, (u["tg_id"], u.get("username", ""), new_expiry))

    await tg_send(
        u["tg_id"],
        "✅ <b>Подписка на автопостинг активирована!</b>\n\n"
        f"Действует до: <b>{new_expiry.strftime('%d.%m.%Y %H:%M')}</b> (МСК)\n\n"
        "В боте появилась кнопка «🚀 Настройки автопостинга» — откройте её, чтобы "
        "загрузить фото, текст объявления и выбрать время публикаций."
    )
    return {"status": "paid"}


class UooOrderIn(BaseModel):
    tariff: str


async def _apply_uoo(order: dict, days: int) -> "datetime":
    """Продлевает/создаёт подписку «УОО» и ставит тег «УОО» участнику в чате.
    Возвращает дату окончания действия услуги."""
    user_id = order["user_id"]
    username = order.get("username", "")
    with db() as conn:
        existing = one(conn, "SELECT * FROM uoo_subscriptions WHERE user_id=%s", (user_id,))
        now = datetime.now()
        if existing and existing["active"] and existing["expires_at"] and existing["expires_at"] > now:
            new_expiry = existing["expires_at"] + timedelta(days=days)
        else:
            new_expiry = now + timedelta(days=days)
        conn.execute("""
            INSERT INTO uoo_subscriptions (user_id, username, expires_at, active)
            VALUES (%s, %s, %s, TRUE)
            ON CONFLICT (user_id) DO UPDATE SET
                username=excluded.username, expires_at=excluded.expires_at, active=TRUE
        """, (user_id, username, new_expiry))

    await tg_set_member_tag(CHAT_ID, user_id, UOO_TAG_TEXT)
    return new_expiry


@app.post("/api/services/uoo/order")
async def create_uoo_order(body: UooOrderIn, request: Request):
    """«Урезанные ограничения на отправку объявлений» — только для авторизованных
    пользователей сайта."""
    u = require_user(request)
    _ensure_services_tables()

    tariff = UOO_TARIFFS.get(body.tariff)
    if not tariff:
        raise HTTPException(400, "Неизвестный тариф")
    ...
    with db() as conn:
        row = one(conn, """
            INSERT INTO paid_services_orders (service_type, user_id, username, amount, label, params, status)
            VALUES ('uoo', %s, %s, %s, 'pending_label', %s, 'pending')
            RETURNING id
        """, (u["tg_id"], u.get("username", ""), amount, json.dumps({"tariff": body.tariff, "days": tariff["days"]})))
        order_id = row["id"]
        label = f"service_uoo_{order_id}"
        conn.execute("UPDATE paid_services_orders SET label=%s WHERE id=%s", (label, order_id))

    payment_url = _build_quickpay_url(
        amount, label,
        target=f"Урезанные ограничения на объявления ({tariff['label']})",
        comment=f"Услуга: УОО {tariff['label']}, заказ #{order_id}",
    )
    return {"order_id": order_id, "amount": amount, "payment_url": payment_url}


@app.post("/api/services/uoo/check/{order_id}")
async def check_uoo_order(order_id: int, request: Request):
    u = require_user(request)
    with db() as conn:
        order = one(conn, "SELECT * FROM paid_services_orders WHERE id=%s AND service_type='uoo'", (order_id,))
    if not order:
        raise HTTPException(404, "Заказ не найден")
    if order["user_id"] != u["tg_id"]:
        raise HTTPException(403, "Это не ваш заказ")
    ...
    new_expiry = await _apply_uoo(order, days)

    await tg_send(
        u["tg_id"],
        "✅ <b>Услуга «УОО» активирована!</b>\n\n"
        f"Вам выдан тег «{UOO_TAG_TEXT}» в чате — теперь для вас действуют льготные лимиты "
        "на отправку объявлений: до 8 в день, не чаще раза в час, и достаточно 10 сообщений "
        "после объявления вместо 30.\n\n"
        f"Действует до: <b>{new_expiry.strftime('%d.%m.%Y %H:%M')}</b> (МСК). "
        "По истечении срока тег будет снят автоматически."
    )
    return {"status": "paid"}


def _get_active_mute(conn, user_id: int):
    return one(conn, """
        SELECT * FROM mutes WHERE user_id=%s AND chat_id=%s AND is_active=TRUE
        ORDER BY issued_at DESC LIMIT 1
    """, (user_id, CHAT_ID))


def _get_active_ban(conn, user_id: int):
    return one(conn, """
        SELECT * FROM bans WHERE user_id=%s AND chat_id=%s AND is_active=TRUE
        ORDER BY issued_at DESC LIMIT 1
    """, (user_id, CHAT_ID))


def _get_active_warns_count(conn, user_id: int) -> int:
    r = one(conn, "SELECT COUNT(*) AS c FROM warns WHERE user_id=%s AND chat_id=%s AND expires_at > now()",
            (user_id, CHAT_ID))
    return int(r["c"]) if r else 0


@app.get("/api/services/my-punishments")
async def my_punishments(request: Request):
    """Текущие активные наказания пользователя — для подсветки недостаточных тарифов на фронте."""
    u = require_user(request)
    _ensure_services_tables()

    with db() as conn:
        mute = _get_active_mute(conn, u["tg_id"])
        ban = _get_active_ban(conn, u["tg_id"])
        warns_count = _get_active_warns_count(conn, u["tg_id"])

    mute_remaining = _remaining_days(mute.get("expires_at")) if mute else None
    ban_remaining = _remaining_days(ban.get("expires_at")) if ban else None

    return {
        "mute": {"active": bool(mute), "remaining_days": mute_remaining},
        "ban": {"active": bool(ban), "remaining_days": ban_remaining},
        "warns": {"active": warns_count > 0, "count": warns_count},
    }


@app.post("/api/services/unblock/order")
async def create_unblock_order(body: UnblockOrderIn, request: Request):
    u = require_user(request)
    _ensure_services_tables()

    tariff = BLOCK_TARIFFS.get(body.tariff)
    if not tariff:
        raise HTTPException(400, "Неизвестный тариф")
    ...
    with db() as conn:
        if tariff["kind"] == "mute":
            mute = _get_active_mute(conn, u["tg_id"])
            if not mute:
                raise HTTPException(400, "У вас нет активного мута в чате")
            remaining = _remaining_days(mute.get("expires_at"))
            tariff_days = tariff.get("days")
            if tariff_days is not None and (remaining is None or remaining > tariff_days):
                suggestion = _cheapest_sufficient_tariff("mute", remaining)
                if suggestion:
                    _, sug_t = suggestion
                    raise HTTPException(
                        400,
                        f"Этот тариф не покрывает оставшийся срок мута "
                        f"({'бессрочно' if remaining is None else f'{remaining} дн.'}). "
                        f"Выберите тариф «{sug_t['label']}» или выше."
                    )
                raise HTTPException(400, "Ни один тариф не покрывает оставшийся срок мута. Обратитесь к администратору.")
        elif tariff["kind"] == "ban":
            ban = _get_active_ban(conn, u["tg_id"])
            if not ban:
                raise HTTPException(400, "У вас нет активного бана в чате")
            remaining = _remaining_days(ban.get("expires_at"))
            tariff_days = tariff.get("days")
            if tariff_days is not None and (remaining is None or remaining > tariff_days):
                suggestion = _cheapest_sufficient_tariff("ban", remaining)
                if suggestion:
                    _, sug_t = suggestion
                    raise HTTPException(
                        400,
                        f"Этот тариф не покрывает оставшийся срок бана "
                        f"({'бессрочно' if remaining is None else f'{remaining} дн.'}). "
                        f"Выберите тариф «{sug_t['label']}» или выше."
                    )
                raise HTTPException(400, "Ни один тариф не покрывает оставшийся срок бана. Обратитесь к администратору.")
        elif tariff["kind"] == "warn":
            if _get_active_warns_count(conn, u["tg_id"]) < 1:
                raise HTTPException(400, "У вас нет активных варнов")

    amount = tariff["price"]
    with db() as conn:
        row = one(conn, """
            INSERT INTO paid_services_orders (service_type, user_id, username, amount, label, params, status)
            VALUES ('unblock', %s, %s, %s, 'pending_label', %s, 'pending')
            RETURNING id
        """, (u["tg_id"], u.get("username", ""), amount, json.dumps({"tariff": body.tariff, "kind": tariff["kind"]})))
        order_id = row["id"]
        label = f"service_unblock_{order_id}"
        conn.execute("UPDATE paid_services_orders SET label=%s WHERE id=%s", (label, order_id))

    payment_url = _build_quickpay_url(
        amount, label,
        target=f"Снятие блокировки: {tariff['label']}",
        comment=f"Услуга: снятие блокировки ({tariff['label']}), заказ #{order_id}",
    )
    return {"order_id": order_id, "amount": amount, "payment_url": payment_url}


async def _apply_unblock(order: dict) -> bool:
    """Выполняет снятие блокировки после подтверждённой оплаты. Возвращает True, если применено."""
    params = order.get("params") or {}
    ...
    if kind == "mute":
        with db() as conn:
            mute = _get_active_mute(conn, user_id)
            if mute:
                conn.execute("UPDATE mutes SET is_active=FALSE WHERE id=%s", (mute["id"],))
        await tg_lift_mute(CHAT_ID, user_id)
        await tg_send(
            user_id,
            "✅ <b>Оплата получена!</b>\n\n"
            "Мут в чате снят досрочно — можете снова писать сообщения."
        )
        return True

    elif kind == "ban":
        with db() as conn:
            ban = _get_active_ban(conn, user_id)
            if ban:
                conn.execute("UPDATE bans SET is_active=FALSE WHERE id=%s", (ban["id"],))
        await tg_lift_ban(CHAT_ID, user_id)
        await tg_send(
            user_id,
            "✅ <b>Оплата получена!</b>\n\n"
            "Бан в чате снят досрочно. Возвращайтесь:\n"
            f"{CHAT_INVITE_LINK}"
        )
        return True
        ...
        with db() as conn:
            oldest = one(conn, """
                SELECT id FROM warns WHERE user_id=%s AND chat_id=%s AND expires_at > now()
                ORDER BY issued_at ASC LIMIT 1
            """, (user_id, CHAT_ID))
            if oldest:
                conn.execute("DELETE FROM warns WHERE id=%s", (oldest["id"],))
            remaining = _get_active_warns_count(conn, user_id)
        await tg_send(
            user_id,
            "✅ <b>Оплата получена!</b>\n\n"
            "Один варн снят.\n"
            f"Осталось активных варнов: <b>{remaining}</b>"
        )
        return True
...
@app.post("/api/services/unblock/check/{order_id}")
async def check_unblock_order(order_id: int, request: Request):
    u = require_user(request)
    with db() as conn:
        order = one(conn, "SELECT * FROM paid_services_orders WHERE id=%s AND service_type='unblock'", (order_id,))
    if not order:
        raise HTTPException(404, "Заказ не найден")
    if order["user_id"] != u["tg_id"]:
        raise HTTPException(403, "Это не ваш заказ")
    ...
    with db() as conn:
        conn.execute("UPDATE paid_services_orders SET status='paid', paid_at=now() WHERE id=%s", (order_id,))

    await _apply_unblock(order)
    return {"status": "paid"}


async def _reconcile_pending_service_orders():
    """Фоновая подстраховка: раз в минуту сама проверяет все незавершённые
    заказы на платные услуги через ЮMoney — на случай, если пользователь
    оплатил, но не вернулся на сайт нажать «Я оплатил» (или закрыл вкладку)."""
    await asyncio.sleep(15)
    while True:
        try:
            with db() as conn:
                pending = rows(conn, """
                    SELECT * FROM paid_services_orders
                    ...
                    if order["service_type"] == "pin":
                        ...
                        with db() as conn:
                            ...
                            conn.execute("""
                                INSERT INTO pin_queue (order_id, user_id, chat_id, message_id, message_link, hours, status)
                                VALUES (%s, %s, %s, %s, %s, %s, 'queued')
                            """, (order["id"], order["user_id"], CHAT_ID, message_id, message_link, hours))
                        await tg_send(
                            order["user_id"],
                            "✅ <b>Оплата получена!</b>\n\n"
                            f"Ваша заявка на закрепление объявления на <b>{hours:g} ч.</b> принята.\n"
                        ...
                        log.info(f"[services] Автопроверка нашла оплату заказа pin #{order['id']}")

                    elif order["service_type"] == "autopost":
                        days = int(params.get("days", 1))
                        with db() as conn:
                            conn.execute(
                                "UPDATE paid_services_orders SET status='paid', paid_at=now() WHERE id=%s",
                            ...
                            existing = one(conn, "SELECT * FROM autopost_subscriptions WHERE user_id=%s", (order["user_id"],))
                            now = datetime.now()
                            if existing and existing["active"] and existing["expires_at"] and existing["expires_at"] > now:
                                new_expiry = existing["expires_at"] + timedelta(days=days)
                            else:
                            ...
                            conn.execute("""
                                ...
                                ON CONFLICT (user_id) DO UPDATE SET
                                    username=excluded.username, expires_at=excluded.expires_at,
                                    active=TRUE, notified_expiry=FALSE
                            """, (order["user_id"], order.get("username", ""), new_expiry))
                        await tg_send(
                            order["user_id"],
                            "✅ <b>Подписка на автопостинг активирована!</b>\n\n"
                            f"Действует до: <b>{new_expiry.strftime('%d.%m.%Y %H:%M')}</b> (МСК)\n\n"
                            "В боте появилась кнопка «🚀 Настройки автопостинга»."
                        )
                        log.info(f"[services] Автопроверка нашла оплату заказа autopost #{order['id']}")

                    elif order["service_type"] == "unblock":
                        with db() as conn:
                            conn.execute(
                                "UPDATE paid_services_orders SET status='paid', paid_at=now() WHERE id=%s",
                                (order["id"],)
                        ...
                        await _apply_unblock(order)
                        log.info(f"[services] Автопроверка нашла оплату заказа unblock #{order['id']}")

                    elif order["service_type"] == "uoo":
                        days = int(params.get("days", 1))
                        with db() as conn:
                            conn.execute(
                                "UPDATE paid_services_orders SET status='paid', paid_at=now() WHERE id=%s",
                        ...
                        await tg_send(
                            order["user_id"],
                            "✅ <b>Услуга «УОО» активирована!</b>\n\n"
                            f"Вам выдан тег «{UOO_TAG_TEXT}» в чате — действуют льготные лимиты на "
                            "объявления.\n\n"
                            f"Действует до: <b>{new_expiry.strftime('%d.%m.%Y %H:%M')}</b> (МСК)."
                        )
                        log.info(f"[services] Автопроверка нашла оплату заказа uoo #{order['id']}")

                except Exception as e:
                    log.error(f"[services] Ошибка автопроверки заказа #{order['id']}: {e}")

        except Exception as e:
            log.error(f"[services] Ошибка фоновой сверки платежей: {e}")

        await asyncio.sleep(60)


async def _uoo_expiry_checker():
    """Раз в минуту снимает тег «УОО» и деактивирует подписку тем, у кого
    истёк срок услуги «Урезанные ограничения на отправку объявлений»."""
    await asyncio.sleep(20)
    while True:
        try:
            with db() as conn:
                expired = rows(conn, """
                    SELECT * FROM uoo_subscriptions
            ...
            for sub in expired:
                try:
                    with db() as conn:
                        conn.execute(
                            "UPDATE uoo_subscriptions SET active=FALSE WHERE user_id=%s",
                            (sub["user_id"],)
                        )
                    await tg_set_member_tag(CHAT_ID, sub["user_id"], "")
                    await tg_send(
                        sub["user_id"],
                        f"⏳ <b>Срок услуги «{UOO_TAG_TEXT}» истёк</b>\n\n"
                        "Тег снят, для вас снова действуют стандартные лимиты на отправку "
                        "объявлений. Продлить услугу можно во вкладке «Платные услуги» на сайте."
                    )
                    log.info(f"[services] Тег УОО снят по истечении срока: user={sub['user_id']}")
                except Exception as e:
                    log.error(f"[services] Ошибка снятия тега УОО для user={sub.get('user_id')}: {e}")

        except Exception as e:
            log.error(f"[services] Ошибка проверки истёкших подписок УОО: {e}")
