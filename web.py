...
# ─── ТЕСТОВЫЙ ВХОД (только для разработки/тестов) ────────────────────────────
# Включается переменной окружения ENABLE_TEST_LOGIN=1. Не ставьте её на проде.
ENABLE_TEST_LOGIN = os.getenv("ENABLE_TEST_LOGIN") == "1"
...
async def test_login(response: Response):
    ...
    session_token = secrets.token_urlsafe(32)
    save_user_session(session_token, {"username": "test_user", "tg_id": 999999999})
    redirect = RedirectResponse(url="/")
    redirect.set_cookie(
        "vn_user_session", session_token,
...
ALLOWED_DOCS = {
    "договор_оферты.md",
    "условия_возврата.md",
    "политика_персональных_данных.md",
}
...
async def create_pin_order(body: PinOrderIn, request: Request):
    ...
    with db() as conn:
        row = one(conn, """
            ...
            RETURNING id
        """, (u["tg_id"], u.get("username", ""), amount, json.dumps({"hours": body.hours, "message_link": link})))
        order_id = row["id"]
...
async def check_pin_order(order_id: int, request: Request):
    ...
    with db() as conn:
        ...
        """, (order_id, u["tg_id"], CHAT_ID, message_id, message_link, hours))
