...
async def tg_ban_user(tg_id: int, reason: str, issued_by: str):
    ...
    text = (
        ...
        f"Обжалование через: <a href='https://t.me/{os.getenv(\"BOT_USERNAME\",\"TheVapeNeonBot\")}'>бота</a>"
    )
...
async def get_users(request: Request, q: str = ""):
    ...
    for u in data:
        ...
        u["site_banned"] = one(conn,"SELECT COUNT(*) as c FROM site_bans WHERE lower(username)=? AND is_active=1 AND (expires_at IS NULL OR expires_at>datetime('now'))",(u.get("username","").lower(),))["c"] > 0
    conn.close()
    return data
