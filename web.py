...
async def handle_user_report(request: Request, rid: int, body: UserReportActionIn):
    ...
    conn.execute("""
        UPDATE user_reports SET status=?, handled_by=?, handled_action=?, handled_at=datetime('now')
        WHERE id=?
...
async def grant_complaint_review(body: dict):
    ...
    existing = one(conn, "SELECT * FROM site_admins WHERE lower(username)=? AND is_active=1", (username,)) if username else None
    if not existing and tg_id:
        existing = one(conn, "SELECT * FROM site_admins WHERE tg_id=? AND is_active=1", (tg_id,))
    if not existing:
        conn.close()
        raise HTTPException(404, "Admin not found")
    conn.execute(
        "UPDATE site_admins SET can_review_admin_complaints=1 WHERE id=?",
        (existing["id"],)
    )
    conn.commit()
    conn.close()
...
async def revoke_complaint_review(body: dict):
    ...
    if not existing:
        ...
        raise HTTPException(404, "Admin not found")
    conn.execute(
        "UPDATE site_admins SET can_review_admin_complaints=0 WHERE id=?",
        (existing["id"],)
    )
    conn.commit()
    conn.close()
...
async def deactivate_site_admin(body: dict):
    ...
    conn.execute(
        "UPDATE site_admins SET is_active=0, password_hash='' WHERE id=?",
        (existing["id"],)
    )
    conn.commit()
    conn.close()
...
def _ensure_products_table():
    """Убедиться что таблицы товаров существуют"""
    with db() as conn:
        conn.execute("""
...
async def get_products(category: str = "", response: Response = None):
    ...
    with db() as conn:
        if category:
            result = rows(conn,
                ...
                "FROM shop_products WHERE is_active=1 AND category=? ORDER BY id DESC",
                (category,)
            )
        else:
            result = rows(conn,
    ...
    with db() as conn:
        for p in result:
            ...
            p["photos"] = [row[0] for row in photos]
...
async def get_product_photo(product_id: int):
    ...
    _ensure_products_table()
    with db() as conn:
        row = one(conn, "SELECT photo_file_id FROM shop_products WHERE id=? AND is_active=1", (product_id,))
    if not row:
...
async def get_product_photo_by_index(product_id: int, photo_index: int):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT file_id FROM shop_product_photos WHERE product_id=? ORDER BY sort_order LIMIT 1 OFFSET ?",
            (product_id, photo_index)
        ).fetchone()
        if not row:
            # Фолбэк: старое поле photo_file_id (для товаров без записей в shop_product_photos)
            fallback = one(conn, "SELECT photo_file_id FROM shop_products WHERE id=? AND is_active=1", (product_id,))
...
async def debug_dbinfo():
    ...
    Показывает, какой файл БД реально видит процесс сайта."""
    abs_path = os.path.abspath(DB_PATH)
    info = {"resolved_path": abs_path, "cwd": os.getcwd(), "exists": os.path.exists(abs_path)}
    if info["exists"]:
        st = os.stat(abs_path)
        ...
        try:
            with db() as conn:
                row = one(conn, "SELECT COUNT(*) as c FROM shop_products WHERE is_active=1")
                info["active_products_seen"] = row["c"] if row else None
                reports = rows(conn, "SELECT id, status, handled_by, handled_action, handled_at FROM user_reports WHERE id IN (60,61)")
                info["reports_60_61"] = reports
        except Exception as e:
