...
def _repair_db_if_needed():
    ...
    log.warning("Обнаружена повреждённая БД — запускаем восстановление через dump...")
    ...
    try:
        ...
        log.info(f"БД восстановлена. Старая сохранена как {backup_path}")
    except Exception as e:
        log.error(f"Ошибка при восстановлении БД: {e} — создаём пустую БД")
        for c in [src, dst]:
            try:
                if c: c.close()
            except Exception:
...
async def login(body: LoginIn, response: Response):
    ...
    is_https = os.getenv("SITE_URL", "").startswith("https")
    response.set_cookie(
        "vn_session", token,
        httponly=True,
        samesite="lax",
...
async def logout(request: Request, response: Response):
    ...
    if token:
        _del_session(token)
    is_https = os.getenv("SITE_URL", "").startswith("https")
    response.delete_cookie("vn_session", path="/", samesite="lax", secure=is_https)
    return {"ok": True}
...
async def poll_user_token(token: str, response: Response):
    ...
    if data["confirmed"]:
        ...
        USER_SESSIONS[session_token] = {"username": data["username"], "tg_id": data["tg_id"]}
...
async def my_complaints(request: Request):
    ...
    if not token or token not in USER_SESSIONS:
        raise HTTPException(401, "Требуется авторизация")
    uname = USER_SESSIONS[token]["username"].lower().lstrip("@")
    conn = db()
    data = rows(conn, """
